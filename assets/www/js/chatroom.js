...
if(chatroomData.identifire == "chatroom" )
{
   ...
   if(chatroomData.chatroomApiKey !== ""){
          var config = {
              apiKey: chatroomData.chatroomApiKey,
              authDomain: chatroomData.chatroomAuthDomain,
              databaseURL: chatroomData.value,
              projectId: "",
              storageBucket: "",
              messagingSenderId: ""
          };
          ...
          try {
              firebase.app('chatroom');
          }
          catch (e) {
             chatroomFirebase= firebase.initializeApp(config, "chatroom");
          }
      }
...
if(localStorage.getItem("chatUserImage"+pageIdentifie) == null)
{
	...
	{
		Appyscript.openChat(chatroomData.identifire,chatroomData.skypeId);
	}
}
...
Appyscript.chatroomOpen = function(value){
       ...
       if( chatroomData.chatroomApiKey!=""){
            if(!chatroomFirebase.auth().currentUser){
                 chatroomFirebase.auth().signInAnonymously().then(function(user) {
                     console.log("logged in");
                 }).catch(function(error) {
                   var errorCode = error.code;
                   var errorMessage = error.message;
                   console.log(error.message);
          Appyscript.alert(data.languageSetting.firebase_fail_msg,function(){
                                         if(mainView.activePage.name == "chatroom-inner") {
...
//        			}
        localStorage.setItem("chatUrl"+pageIdentifie, value);
	        ...
	        if(chatroomData.chatroomApiKey!=""){
             chatDataRef = chatroomFirebase.database().ref(chatroomData.name);
            }else{
...
//    navigator.device.capture.captureImage(updateChatImage1,null, options);
        navigator.camera.getPicture(updateChatImage1, null, {
            quality: 50,
