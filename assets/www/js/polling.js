...
Appyscript.openPollPage=function(){
    ...
    pollServiceURL=webserviceUrl+'Poll.php';
...
Appyscript.onPageInit('polling-Page',function(page){
    ...
    $$("#resultLanguageSetting").html(pageData.languageSettings.result)
    $$("#voteLanguageSetting").html(pageData.languageSettings.vote)
	...
	*/
    ...
    var listTemplate = '{{#question}}<div class="swiper-slide data-{{@index}}" id="form-{{queId}}" showStatus="{{showStatus}}"><div class="polling-area"><p class="question {{@global.styleAndNavigation.heading[0]}} {{@global.styleAndNavigation.heading[1]}}" style="color:{{@global.styleAndNavigation.heading[2]}}">{{question}} <div class="question-media">{{#if media}}<img src="images/transparent-bg.png" data-background="{{media}}" class="swiper-lazy">   <div class="preloader"></div>{{/if}}{{#if videourl}}<a onclick="Appyscript.playYoutubeWatch(\'{{videourl}}\',\'Polling\');" style="background:{{@global.styleAndNavigation.button[2]}};color:{{@global.styleAndNavigation.button[3]}}"  class="video-btn iconz-video"></a>{{/if}}{{#if image_url}}<img src="images/transparent-bg_1x1.png" data-background="{{httpsUrl image_url}}" class="swiper-lazy"><div class="preloader"></div>{{/if}}{{#if audiourl}}<audio controls controlsList="nodownload" id="audio1" style="display: table; margin: 0 auto;" onclick="this.play();"> <source src="{{audiourl}}" type="audio/ogg"><source src="{{audiourl}}" type="audio/mpeg"> </audio>{{/if}}</div></p> <ul style="border-color:{{@global.styleAndNavigation.borderColor}}"> {{#if multiple}}{{#answers}}<li   style="border-color:{{@global.styleAndNavigation.borderColor}}" onclick="pollingClick(this)"> <label class="label-checkbox item-content"> <input type="checkbox" name="myRadio-{{queId}}" id="{{ansId}}" value="{{answer}}"> <div class="item-media"> <i class="icon icon-form-checkbox"></i> {{#if image_url}}<img src="images/transparent-bg_1x1.png" data-background="{{httpsUrl image_url}}" class="swiper-lazy">   <div class="preloader"></div>{{/if}} </div> <div class="item-inner">  <div class="item-title">{{answer}}</div> </div> </label> </li> {{/answers}} {{else}}{{#answers}} <li style="border-color:{{@global.styleAndNavigation.borderColor}}" onclick="pollingClick(this)"> <label class="label-radio item-content">  <input type="radio" name="myRadio-{{queId}}" id="{{ansId}}" value="{{answer}}"> <div class="item-media"> <i class="icon icon-form-radio"></i> {{#if image_url}}<img src="images/transparent-bg_1x1.png" data-background="{{httpsUrl image_url}}" class="swiper-lazy">   <div class="preloader"></div>{{/if}} </div> <div class="item-inner">  <div class="item-title">{{answer}}</div></div> </label></li> {{/answers}} {{/if}}</ul> </div></div>{{/question}}';
    $$('.bottom').show();
...
//AppyTemplate.global.result = pageData.languageSettings.result;
Appyscript.getVote = function()
{
    ...
    if(pooling_prev_jsondata != null && pooling_prev_jsondata[question[1]]!=null){
     Appyscript.hideIndicator();
            Appyscript.alert(pageData.languageSettings.alreadyVote, appnameglobal_allpages);
        return true;
    }else if(ansId=='' || ansId == undefined)
      {
       Appyscript.hideIndicator();
        Appyscript.alert(pageData.languageSettings.selectAtleastOne, appnameglobal_allpages);
        Appyscript.hideIndicator();
        return true;
      }
...
function offlineChart()
{
    ...
    if(pageData.generalSetting.showpollresult == true)
         {
         ...
         {
                Appyscript.hideIndicator();
                Appyscript.alert(pageData.languageSettings.poll_result_text, appnameglobal_allpages);
                return;
         }
	...
	localStorage.setItem("question", $$("#"+voteid+" .question").text());
        ...
        var wrapText = function (context, text, x, y, maxWidth, lineHeight) {
            ...
            for(var n = 0; n < words.length; n++) {
                ...
                if (testWidth > maxWidth && n > 0) {
                   if(localStorage.lang=="sa"){
                        var canvas = document.getElementById('pie');
                        context.fillText(line, canvas.width-20, y);
                    }else{
        ...
        if(localStorage.lang=="sa"){
        var screenWidthForRect = canvas.width-30;
        var screenWidthForLabel = canvas.width-50;
             }
        ...
        wrapText(ctx, question, 5, questionHeight, x*2-10, questionHeight);
          ...
          for(var i=0; i<data.length; i++) {
            ...
            endAngle = calculateEnd(data, i, total);
              ...
              ctx.fill();
              if(localStorage.lang=="sa"){
                   ctx.rect(screenWidthForRect, i * 30+(x-15)*2, 12, 12);
               }else{
              ...
              if(localStorage.lang=="sa"){
                 ctx.fillText( ansText(ctx,data[i].label,x*2) + " - " + data[i].value + " (" + percent + "%)", screenWidthForLabel, i * 30+(x-10)*2);
              }else{
