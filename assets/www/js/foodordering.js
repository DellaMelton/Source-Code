...
Appyscript.onPageInit('foodordering-page',function(page)
                      ...
                      foodUrl=webserviceUrl+'FoodOrder.php';
...
function foodSwiperBind(index)
{
    ...
    if(isOnline())
    {
        ...
        var postdata='{"method":"catListingWithSubCategory","appId":"'+appid+'","catId":"'+swiperObj.attr("data-id")+'","type":"cat", "count":"60", "pageNo":"'+swiperObj.attr("data-page")+'","sort":"0","emailId":"'+localStorage.getItem("email")+'","sortCatAlpha":"'+sortCatAlpha+'"}';
...
function foodLayoutScrolling(index)
{
    ...
    swiperObj.on('infinite', function()
                 ...
                 {
                 $$(".infinite-scroll-preloader").show();
                 var postdata='{"method":"catListingWithSubCategory","appId":"'+appid+'","catId":"'+dataID+'","type":"cat", "count":"60", "pageNo":"'+dataPage+'","sort":"0","emailId":"'+localStorage.getItem("email")+'","sortCatAlpha":"'+sortCatAlpha+'"}';
...
Appyscript.foodCategory = function(a)
{
    ...
    var postdata='{"method":"catListingWithSubCategory","appId":"'+appid+'","catId":"'+catID+'","type":"cat", "count":"60", "pageNo":"1","sort":"0","emailId":"'+localStorage.getItem("email")+'","sortCatAlpha":"'+sortCatAlpha+'"}';
...
Appyscript.onPageInit('foodordering-Category',function(page)
                      ...
                      infiniteScroll.on('infinite', function()
                                        ...
                                        var postdata='{"method":"catListingWithSubCategory","appId":"'+appid+'","catId":"'+catID+'","type":"cat", "count":"60", "pageNo":"'+dataPage+'","sort":"0","emailId":"'+localStorage.getItem("email")+'","sortCatAlpha":"'+sortCatAlpha+'"}';
                                        console.log("catListingWithSubCategory  postdata "+postdata);
...
Appyscript.foodSubcatProductList = function(a)
{
    ...
    if(isOnline())
    {
        var postdata='{"method":"catListingWithSubCategory","appId":"'+appid+'","catId":"'+subcatid+'","type":"cat", "count":"60", "pageNo":"1","sort":"0","emailId":"'+localStorage.getItem("email")+'","sortCatAlpha":"'+sortCatAlpha+'"}';
        console.log("catListingWithSubCategory postdata "+postdata);
...
Appyscript.onPageInit('foodordering-ContinueCheckout',function(page)
                     ...
                     deliveryMinDuration_value= foodData.otherRules.deliveryMinDuration;
...
function foodProceedToPay(a)
{
    ...
    if(pageData.orderwithoutprice == 0){
    ...
    {
        Appyscript.alert("Sorry! minimum order can not be less then "+ minPrice);
        return;
    }
...
function foodConfirmPayment(a,tabName){
    var foodData=JSON.parse(localStorage.getItem("fooddata"));
    if(pageData.orderwithoutprice == 1 && parseFloat(foodData.grandTotal) == 0)
    {
       Appyscript.paymentRegistrationInfo("zeroPayment",'food');
       return
    }
    ...
    else
    {
        ...
        if(len>0)
        {
            if(timeViewFlag ==false)
            {
                ...
                var k = new Date();
                var c = k.setMinutes(k.getMinutes() + parseInt(deliveryMinDuration_value));

                  new Date(c);
                  new Date(c).toTimeString()
                var timevalue=new Date(c).toLocaleTimeString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true });

                 console.log('edrftgyuiol;'+ timevalue );
                 foodData.deliveryTime = timevalue;
...
function foodPaymentMethod(aa)
{
    ...
    if(isOnline())
    {
//       if(pageData.orderwithoutprice == 1){
//      // foodConfirmPayment("","pickup");s
//          return;
//       }

    //asdasdsdas
        Appyscript.showIndicator();
        var baseurl=webserviceUrl+'FoodOrder.php';
        var postdata='{"method":"foodPaymentMethod","appId":"'+appid+'"}';
...
function foodPaymentRegistrationInfo(paymentTypeObject)
{
    ...
    if(paymentType == "zeropayment"){
        paymentMethodKey="zeropayment";
    }else{
...
//    var paymentMethodKey=a.attr("data-key");
 var paymentMethodKey;
    ...
    if(isOnline())
    {
        ...
        var baseurl=webserviceUrl+'FoodOrder.php';  //'http://angularml.pbodev.info//webservices/Ecomm.php';
        var postData;
...
function foodOpenPaypalViewForPayment(billingDataPar,payPalDataPar)
{
    ...
    var htmlForm= Appyscript.getPayPalHtml("", payPalData.paypalId, payPalData.totalAmount,
                                           ...
                                           site_url+"/paypalmobile/notify/orderId/"+"/lang/"+data.appData.lang+payPalData.orderId+"/appId/"+appid);
...
function foodOpenPayFastViewForPayment(billingDataPar,payfastDataPar){
    ...
    var htmlForm= Appyscript.getPayFastHtml("", payFastData.merchantId,payFastData.merchantKey, payFastData.totalAmount, payFastData.currency, payFastData.orderId,site_url+"/paypalmobile/payfast-success",site_url+"/paypalmobile/payfast-cancel"+"/lang/"+data.appData.lang,site_url+"/paypalmobile/payfast-notify/appId/"+app_id+"/orderId/"+payFastData.orderId,payFastData.userName,payFastData.userEmail,payFastData.productDetailName,payFastData.productDetailDescrip);
    Appyscript.hideIndicator();
...
function  foodCreditCardPaymentThroughStripe(billingData,shippingData,paymentTypeObject,productList,creditCardDetailPar)
{
    ...
    else
    {
          ...
          foodProductListForGloble=JSON.parse(localStorage.getItem("foodpaydata"));
          if(_checkStripeCall)
           {
           foodSubmitOrderByStripe('','');
            _checkStripeCall=false;
           }
...
function foodSubmitOrderByStripe(paymentId,customer_id)
{
    ...
    if(isOnline())
    {
        ...
        $$.ajax({
                ...
                else (success.toLowerCase()=="failure")
                {
    /*Appyscript.alert(something_went_wrong_please_try_again , appnameglobal_allpages);*/
