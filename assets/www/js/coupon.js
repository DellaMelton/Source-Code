...
function CoupanDetails(index) {
    ...
    if (pageData.list[index].show_expire == 0) {
...
function coupanredeemvalue() {
    ...
    if (couponserch == couponCode) {
        ...
        var postdata = '{"method":"redeemCoupon","appId":"' + app_id + '","redeemFrom":"couponPage","userId":"' + userIdValue + '","uniquecodedata":"' + uniquecodedata + '","deviceId":"' + coupondeviceid + '","pageIdentifier":"' + pageIdentifie + '"}';
...
function coupanredeemshowmessage() {
    ...
    Appyscript.confirmation(AppyTemplate.global.pageLanguageSetting.redeem_confirmation, '', AppyTemplate.global.pageLanguageSetting.redeem, AppyTemplate.global.pageLanguageSetting.cancel,
        function() {
            ...
            var coupondeviceid = Appyscript.getDeviceId();
            app_id = localStorage.getItem("appid");
            var postdata = '{"method":"redeemCoupon","appId":"' + app_id + '","uniquecodedata":"' + uniquecodedata + '","redeemFrom":"couponPage","userId":"' + userIdValue + '","deviceId":"' + coupondeviceid + '","pageIdentifier":"' + pageIdentifie + '"}';
            var baseurl = webserviceUrl + 'Coupon.php';
            ...
            if (filledInPixels > 50) {
                ...
                setTimeout(function() {
                    ...
                    if (handleWeb == true) {
                        var coupondeviceid = Appyscript.getDeviceId();
                        app_id = localStorage.getItem("appid");
                        var postdata = '{"method":"redeemCoupon","appId":"' + app_id + '","uniquecodedata":"' + uniquecodedata + '","deviceId":"' + coupondeviceid + '", "redeemFrom":"couponPage","userId":"' + userIdValue + '","text":"' + coupon_scratch_text + '","image":"' + scratchImageGlobal + '","coupon_unlucky":"' + coupon_unlucky + '","coupon_type":"' + coupon_type + '" ,"pageIdentifier":"' + pageIdentifie + '" }';
                        var baseurl = webserviceUrl + 'Coupon.php';
                        ...
                        $$.ajax({
                            ...
                            success: function(jsondata, textStatus) {
                                ...
                                if (textStatus == 200 || textStatus == 'success') {
                                    var postdata = '{"method":"scratchCoupon","appId":"' + app_id + '","pageId":"' + pageIdentifie + '","couponCode":"' + uniquecodedata + '","couponType":"' + coupon_type + '","scratchStatus":"' + coupon_unlucky + '","deviceId":"' + coupondeviceid + '","deviceType":"android","couponScratchText":"' + coupon_scratch_text + '","userId":"' + userIdValue + '" }';
                                    console.log("scratchCoupon postdata :  " + postdata + "   baseurl  " + baseurl);
                                    $$.ajax({
                                        url: baseurl,
                                        data: Appyscript.validateJSONData(postdata),
                                        type: "post",
                                        ...
                                        success: function(jsondata, textStatus) {
                                            var jsonData = JSON.parse(jsondata);
                                            console.log("finddata   " + JSON.stringify(jsondata));
                                            if (textStatus == 200 || textStatus == 'success') {} else {
                                                Appyscript.alert(something_went_wrong_please_try_again, appnameglobal_allpages);
                                            }
                                        },
                                        error: function(error) {
                    ...
                    if (coupon_unlucky == 1) {
...

                        {
                            ...
                            var coupondeviceid = Appyscript.getDeviceId();
                            app_id = localStorage.getItem("appid");
                            var postdata = '{"method":"redeemCoupon","appId":"' + app_id + '","uniquecodedata":"' + uniquecodedata + '","redeemFrom":"couponPage","userId":"' + userIdValue + '","deviceId":"' + Appyscript.getDeviceId() + '","deviceId":"' + coupondeviceid + '","pageIdentifier":"' + pageIdentifie + '"}';
                            var baseurl = webserviceUrl + 'Coupon.php';
...
/*****************************************************************************************/
// Coupon QR code scanning START
/*****************************************************************************************/

Appyscript.qrCodeCoupon = function() {
    var fromloyality = 'couponCard';
    var data = JSON.stringify(pageData);
    data = JSON.parse(data);
    openScannerCoupon(fromloyality, data.pageTitle);

}

function openScannerCoupon(scanios, scanand) {
    if (Appyscript.device.android) {
        AppyPieNative.validateLoyaltyViaScanner("couponCard", scanand);
    } else {
...
function QrScannerCouponSuccess(a) {
    qrCodeValue = a.split(':');
    qrCodeValue = qrCodeValue[1];

    console.log("code==" + qrCodeValue.trim() + "  couponCode==" + couponCode);
    ...
    if (qrCodeValue.trim() == couponCode) {
        var coupondeviceid = Appyscript.getDeviceId();
        app_id = localStorage.getItem("appid");
        var postdata = '{"method":"redeemCoupon","appId":"' + app_id + '","redeemFrom":"couponPage","userId":"' + userIdValue + '","uniquecodedata":"' + uniquecodedata + '","deviceId":"' + coupondeviceid + '","pageIdentifier":"' + pageIdentifie + '"}';
        var baseurl = webserviceUrl + 'Coupon.php';
        console.log("kkkkk RedeemCoupon  " + baseurl);
        console.log("kkkkk2222 RedeemCoupon  " + postdata);
        $$.ajax({
            url: baseurl,
            data: Appyscript.validateJSONData(postdata),
            type: "post",
