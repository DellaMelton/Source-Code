...
loadmoreyoutube=false;
	 listIndex=$$(a).attr("index");
	  videoName=$$(a).attr("videoName");
	  catIndex=$$(".categories-list li.swiper-slide-active a").attr("index");
			 ...
			 if(VideoIdentifire=='custom')
			     {
			      ...
			      if(clickedVideoUrl.indexOf("rtsp:")>-1 || clickedVideoUrl.indexOf("m3u8")>-1||clickedVideoUrl.indexOf(".mp4")>-1 )
			      {
			      if(!videolisttitle)
				 ...
				 else
				 {
				  if(!videolisttitle)
                 	  ...
                 	  if(clickedVideoUrl.indexOf(".m4v") > -1 ){
                         Appyscript.openWebView(clickedVideoUrl,"",pageData.pageTitle);
                 	  }else{
                 	     checkIfFileExistsorNot_Video(clickedVideoUrl);
                 	  }
                 }
			   }
                   ...
                   {
                    jsonOfCatlist.pageTitle=pageData.pageTitle;
                     if(clickedVideoUrl.indexOf("https://player.vimeo.com/external/")!=-1 || clickedVideoUrl.indexOf("https://www.twitch.tv/")!=-1)
			   ...
			   else if(VideoIdentifire== 'youtube')
			   {
					   ...
					   {
					   youtubeUrl=catlistArray[catIndex].list[listIndex].username;
					   parseYoutubeChannelList('https://www.googleapis.com/youtube/v3/search?channelId='+youtubeUrl+'&mime=true&part=snippet&maxResults=50&order=date&key='+data.googlePlacesApiKey, typeOfYoutubeVideo);
					   }
					   else if(typeOfYoutubeVideo=='user')
					   {
						parseYoutubeUserList('https://www.googleapis.com/youtube/v3/channels?part=contentDetails&maxResults=50&order=date&forUsername=' + youtubeUrl + '&key='+data.googlePlacesApiKey);
					   }
					   else if(typeOfYoutubeVideo=='playlist')
					   {
						parseYoutubeChannelList('https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=50&order=date&playlistId=' + youtubeUrl + '&key='+data.googlePlacesApiKey);
					   }
					   else
					   {
						 ...
						 parseYoutubeChannelList('https://www.googleapis.com/youtube/v3/search?part=snippet&q=' + youtubeUrl + '&mime=true&part=snippet&maxResults=50&key='+data.googlePlacesApiKey);
					   }
			  ...
			  if(clickedVideoUrl.indexOf("https://www.facebook.com") != "-1")
                     {
                         clickedVideoUrl=clickedVideoUrl;
                     }
                     else{
                         clickedVideoUrl="https://www.facebook.com/"+clickedVideoUrl;
                     }
...
var videoLink=$$(a).attr("videoId");
    ...
    if(isFrom=="RssMedia")
	{
		 ...
		 console.log("===== videotitle rss feed " + title);
		...
		/* if(videoLink.indexOf(".mp3")!=-1 || videoLink.indexOf(".pls")!=-1)
								  ...
								  || videoLink.indexOf(".3gp") > -1 || videoLink.indexOf(".mpg") > -1)
		 {
		     ...
		     if(videoLink.indexOf("vimeo") > -1||videoLink.indexOf("venue")>-1 ||videoLink.indexOf("amazingfacts")>-1 || videoLink.indexOf("podbean")>-1)
                     		 {
		 ...
		 else
		 {
	        ...
	        var  catIndex=$$(".categories-list li.swiper-slide-active a").attr("index");
             if((pageData.list.length == 1) && (pageData.list[0].list.length == 1)) {
			   ...
			   var catlist=catlistArray[catIndex];
			   var catSublist=catlistArray[catIndex].list;
			   catSublist[0].description = $$(a).attr('description');
			   catSublist[0].name = $$(a).attr('title');
			   var jsonOfCatlist=JSON.stringify(catSublist);
			    VideoIdentifire=catlist.identifire;
			   var clickedVideoUrl=$$(a).attr("videoId");
			   /*pageData.title= $$(a).attr('title');*/


		   AppyPieNative.playDefaultVideo(videoLink,jsonOfCatlist,listIndex,title,pageData.setting.openInNativeBrowser,pageData.setting.enableShare,pageData.setting.enableCastscreen,pageData.setting.enableAutoPlay,shareLayoutValue);
		   }

		   //Appyscript.openVideoStream(videoLink,"0",pageData.pageTitle,pageData.setting.openInNativeBrowser,pageData.setting.enableShare,pageData.setting.enableCastscreen,pageData.setting.enableAutoPlay);
		 }

		 else if(videoLink.indexOf("vimeo") > -1||videoLink.indexOf("venue")>-1 ||videoLink.indexOf("amazingfacts")>-1 || videoLink.indexOf("podbean")>-1)
         		 {
         		  Appyscript.openWebView(videoLink,pageData.pageTitle,"video");

         		 }

         		 else if(videoLink.indexOf("dailymotion") > -1 || videoLink.indexOf(".html")>-1)
                          		 {
                               Appyscript.openWebView(videoLink,pageData.pageTitle,"video");
...
if(isNaN(username))
{
var match = username.match(/\//g);
    var match = username.match(new RegExp("/", 'g'));
   ...
   var facebookOauthAccessToken = 'https://graph.facebook.com/oauth/access_token?client_id='+data.appData.fbAppId+'&client_secret='+data.appData.fbAppSecret+'&redirect_uri=https://www.snappy.appypie.com&grant_type=client_credentials';
              $.ajax({
                     ...
                     AcessToken = jsonData['access_token']
                     Fburl='https://graph.facebook.com/'+getFBPageNameFromURL(username)+'/videos?access_token='+AcessToken+'&fields=from,title,embed_html,created_time,picture,description';
                     newfacebookParse(Fburl);
                     //var postURL = 'https://graph.facebook.com/'+photoData.albumName.split("___")[0]+'/photos?fields=picture,source,count&limit='+facebookcount+'&access_token='+AcessToken;
                     },
...
function newfacebookParse(Fburl)
{
if(isOnline())
{
...
Appyscript.showIndicator();
				...
				success: function(data, textStatus ){
				 $$.get("pages/video-facebook.html", function (template) {
                  ...
                  var tempObj = Object.assign(data);
                         ...
                         if(data.data == undefined)
                           {
                            ...
                            console.log("567890-"+JSON.stringify(data))
                   ...
                   "layout":pageData.styleAndNavigation.layout
                       ...
                       {
                       Appyscript.hideIndicator();
                       				Appyscript.alert(pageData.languageSetting.oops_no_data_found,localStorage.getItem("appName"));

                       }
				   });
				},error: function(error) {
				Appyscript.hideIndicator();
				Appyscript.alert(pageData.languageSetting.oops_no_data_found,data.appData.appName);
   ...
   if(isOnline())
	...
	Appyscript.showIndicator();
		$$.ajax({
			   ...
			   parseYoutubeChannelList('https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=50&playlistId=' + obj.items[0].contentDetails.relatedPlaylists.uploads + '&key='+data.googlePlacesApiKey);
			   }
			   else{
               Appyscript.hideIndicator();
	...
	Appyscript.showIndicator();
		$$.ajax({
			   ...
			   success: function(json) {
				...
				var dateFormat = object.items;

				dateFormat.sort(function(a,b){
...
console.log("==== playlistid_new_api in  parseYoutubeChannelListLoadMore : " + playlistid_new_api);
   ...
   {
   var loadmoreurl = "https://www.googleapis.com/youtube/v3/playlistItems?&part=snippet&pageToken="+loadMorepageToken+"&maxResults=50&order=date&playlistId="+playlistidyoutube+"&key="+data.googlePlacesApiKey
   console.log("===== loadmoreurl user 432543254: " + loadmoreurl);
          ...
          {
              var loadmoreurl = "https://www.googleapis.com/youtube/v3/playlistItems?&part=snippet&pageToken="+loadMorepageToken+"&maxResults=50&order=date&playlistId="+youtubeUrl+"&key="+data.googlePlacesApiKey
  ...
  // var loadmoreurl = "https://www.googleapis.com/youtube/v3/playlistItems?&part=snippet&pageToken="+loadMorepageToken+"&maxResults=50&order=date&playlistId="+youtubeUrl+"&key=AIzaSyA3KxUIBgANTtrwXWJ_qpt4DC4b9kYmAYs"
   var loadmoreurl ="https://www.googleapis.com/youtube/v3/search?channelId="+youtubeUrl+"&mime=true&part=snippet&pageToken="+loadMorepageToken+"&maxResults=50&key="+data.googlePlacesApiKey;
...
Appyscript.playFacebookVideo = function(a){
    ...
    if(VideoId.indexOf("http")!=-1)
    {
     if(VideoId.indexOf("vimeo.com")!=-1)
      ...
      Appyscript.openWebView("https://player.vimeo.com/video/"+VideoId,$$(a).attr("title"),"video");
    }
     else{
...
var baseVimeoUrl="https://vimeo.com/";
            ...
            console.log("===== subVimeoUrl : " + subVimeoUrl);
             if(parseInt(subVimeoUrl)) {
                            ...
                            isNumber =1;
                               ...
                               Appyscript.openWebView("https://player.vimeo.com/video/"+subVimeoUrl,videoName,"video");
...
function getFBPageNameFromURL(pageValue){
	...
	var match = fbname.match(/\//g);
    var match = fbname.match(new RegExp("/", 'g'));
    var count = match ? match.length : 0;

        if(fbname.indexOf("https://www.facebook.com/video/embed?video_id=") != "-1")
        {
        returnUrl=fbname.replace("https://www.facebook.com/video/embed?video_id=",'');
        }
         else  if(fbname.indexOf("https://www.facebook.com/pg/") != "-1")
             {
              fbname=fbname.replace("https://www.facebook.com/pg/",'');
                if(fbname.indexOf("videos") != "-1")
                {
                 returnUrl=fbname.replace("/videos/",'');
                }
           ...
           {
             returnUrl=fbname.replace("https://www.facebook.com/",'');
           }


       else
        {
         returnUrl=fbname.substring(fbname.lastIndexOf("/")+1,fbname.length);
        }
console.log("==== fbname 12333 : " + returnUrl);
return returnUrl;
}
...
Appyscript.onPageBeforeInit('video-UserPage2',function(page){if(singleVideoPage) {if(AppyTemplate.global.style.layout == "bottom") {
 ...
 AppyTemplate.registerHelper('videoDate',function (getData) {
 //                            var a = new Date(getData);
 //                            return  a.getFullYear() + "-" + (a.getMonth() + 1) + "-" + a.getDate() + " " + a.getHours() + ":" + a.getMinutes()
 //
 //
                             var today = new Date(getData);
 ...
 //
 //                            }
 //                            //PC convert military time to standard time
 //                            var time = today.getHours() + ":" + minutes + ": 00" ;
 //                            time = time.split(':');
 ...
 //                            return  mm + "/" + dd + "/" + today.getFullYear() + " " + timeValue
 //                            //END
 //
 //                            //return  today.getFullYear() + "-" + mm + "-" + dd + " " + today.getHours() + ":" + minutes





 //                          var _dateArray = date.split("/");
                             var _defaultDateFormat=data.defaultDateFormat.toLowerCase();
                             var _monthArr=data.monthLang;
                             var _convertedDate;

                             var _dateValue = today.getDate();
                             var _monthValue = today.getMonth()+1;
                             var _yearValue = today.getFullYear();
                             ...
                             return _convertedDate + " "+(new Date(getData).toLocaleTimeString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true })); ;
...
function checkIfFileExistsorNot_Video(customVideoURL)
{
    video_DL_Link = encodeURI(customVideoURL);
    fileName_Video = video_DL_Link.substr(video_DL_Link.lastIndexOf('/') + 1);
    //path_Video = fileSystemGlobal.root.toNativeURL();
    path_Video = cordova.file.externalDataDirectory;
    path_Video = path_Video+data.appData.appId+"_video/" + fileName_Video ;

    window.resolveLocalFileSystemURL(path_Video, fileExistsYes_Video, fileDoesNotExistNO_Video);

  //  fileSystemGlobal.root.getFile("customvideo/"+fileName_Video, { create: false }, fileExistsYes_Video, fileDoesNotExistNO_Video);
}

function fileExistsYes_Video(fileEntry)
{
 //  var jsonString = '{"catName": "  ", "catIcon": "", "catIconType": "","identifire": "","list":[{"name":"","value":"' + path_Video.replace('file://','') + '","description":"","uploadedOn":"","iconName":"","iconType":""}],"pageTitle":""}';

  // window.location = "openvideoplayer:" + jsonString + "$,$" + "" + "$,$" + "" + fileName_Video;
 AppyPieNative.playLiveSteamVideo(path_Video.replace('file://',''), "", "", fileName_Video, "", "", "", "", "");
//alert("SDF");
}

function fileDoesNotExistNO_Video()
{
  //  Appyscript.showIndicator();
//    downloadFileYes_Video(video_DL_Link,fileName_Video);
    var data=pageData;
    var catlistArray=data.list;
    var catlist=catlistArray[catIndex];

    catlist.pageTitle=videoName;
    catlist.videoId=data.videoId;
    catlist.enableCastscreen=data.enableCastscreen;
    catlist.enableShare=data.setting.enableShare.toString();
    catlist.openInNativeBrowser=data.openInNativeBrowser;
    catlist.enableAutoPlay=data.setting.enableAutoPlay;

    var jsonOfCatlist=JSON.stringify(catlist);
   // window.location="openvideoplayer:"+jsonOfCatlist+"$,$"+listIndex+"$,$"+videoName+"$,$"+pageData.styleAndNavigation.shareLayout;
   if(AppyPieNative.getBuildVersion()<24)
   {
    Appyscript.openVideoStream(video_DL_Link,jsonOfCatlist,listIndex,fileName_Video,pageData.setting.openInNativeBrowser,pageData.setting.enableShare,pageData.setting.enableCastscreen,pageData.setting.enableAutoPlay,shareLayoutValue);
   }
   else{
     AppyPieNative.playDefaultVideo(video_DL_Link,jsonOfCatlist,listIndex,fileName_Video,pageData.setting.openInNativeBrowser,pageData.setting.enableShare,pageData.setting.enableCastscreen,pageData.setting.enableAutoPlay,shareLayoutValue);
   }
}
//  download file =====
function downloadFileYes_Video(URL,name)
{
    if(isOnline())
    {
        ...
        directoryEntry.getDirectory(folderName, {
                                    create: true,
                                    exclusive: false
                                    }, onDirectorySuccess_Video, onDirectoryFail_Video);

        var rootdir = fileSystemGlobal.root;
      ...
      // var fp = fileSystemGlobal.root.toNativeURL();
            ...
            fp = fp +data.appData.appId+"_video/"+name;
        // download function call
        filetransferYes_Video(video_DL_Link, fp);

        function onDirectorySuccess_Video(parent) {
            // Directory created successfuly
        }

        function onDirectoryFail_Video(error) {
...
function filetransferYes_Video(download_link, fp)
{
    var fileTransfer = new FileTransfer();
    // File download function with URL and local path
    fileTransfer.download(decodeURIComponent(download_link), fp,function(entry)
    {
...
AppyTemplate.registerHelper('defaultImage', function(value) {
       var value = value || "" ;
        if(value.indexOf("youtu")>-1){
            return "style=background-image:url("+pageData.defaultImage.ytDefault+")"
        }else{
            return "style=background-image:url("+pageData.defaultImage.defaultImage+")"
        }

});


AppyTemplate.registerHelper('defaultImageSRC', function(value) {
       var value = value || "" ;
        if(value.indexOf("youtu")>-1){

            return "src="+pageData.defaultImage.ytDefault
        }else{
            return "src="+pageData.defaultImage.defaultImage
        }
});
