...
function openPocketTools(pageIndex, pageIdentifier,headerTitle){
    ...
    switch(pageIdentifier)
    {
        ...
        case 'barcodereader':
            ...
            barcodeResultPageIndex=pageIndex;
...
function BarCodeScannerResult(a) {
     if(isOnline())
        ...
        var databaseURL=pageData.list[barcodeResultPageIndex].barcode_url;
        var databaseName=pageData.list[barcodeResultPageIndex].barcode_db;
        var database_Secret=pageData.list[barcodeResultPageIndex].Secretkey;
        var requestPath= databaseURL + "/"+databaseName+".json"

        if(database_Secret!="" && database_Secret != undefined && database_Secret != null){
                    requestPath = requestPath +"?auth="+database_Secret
        }
...
function checkScannerValues(responseData,a){
    Appyscript.showIndicator();
        flag=true;
        var _jsondata = JSON.parse(responseData);
        $$.each(_jsondata, function( key, value) {
                if(flag){
                    var childData = value;
                    if(childData.bar_code_no==a){
                        ...
                        else
                        {
                            ...
                            Appyscript.openWebView(childData.url,headerTitleQRScanner ,'fromscanner');
                            Appyscript.hideIndicator();
                        }
                        flag=false;
...
{
    Appyscript.openWebView("https://apps.appypie.com/media/testapp/pokemap?latitude="+localStorage.getItem("localLatitude")+"&longitude="+localStorage.getItem("localLongitude"),a11);
}
...
Appyscript.OpenstockExchangepage = function(pagename, stockpageId) {
    if (isOnline()) {
        Appyscript.showIndicator();
        $$.ajax({
            url: "https://cloud.iexapis.com/stable/stock/market/batch?symbols=aapl,NKE,msft,,GOOG,DAX&types=quote&range=1m&last=5&token=sk_2409c82919a64235b775ed923527f90e",
            type: 'get',
            async: true,
            success: function(data) {
               var DocData = {};
                               ...
                               stockArr.push(DocData[i].quote)
                               }
                               console.log("stockmarket::::   "+JSON.stringify(DocData));
                               var data = {
                                   stockArr :stockArr
                               }
                $$.get("pages/stockmarket.html", function(template) {
...
function stockMarketDetailPage(a){
    var stockSymbol = $$(a).attr("stockName");
    if (isOnline()) {
            Appyscript.showIndicator();
            $$.ajax({
                url: "https://cloud.iexapis.com/stable/stock/market/batch?symbols="+stockSymbol+"&types=quote&range=1m&last=5&token=sk_2409c82919a64235b775ed923527f90e",
                type: 'get',
                async: true,
                success: function(data) {
                    var DocData = {};
...
//                    DocData = JSON.parse(data);
//                    if (stockSymbol == "MSFT") {
//                        StockData = DocData.MSFT;
//                    } else if (stockSymbol == "NKE") {
//                        StockData = DocData.NKE;
//                    } else if (stockSymbol == "AAPL") {
//                        StockData = DocData.AAPL;
//                    } else if (stockSymbol == "GOOG") {
//                        StockData = DocData.GOOG;
//                    } else if (stockSymbol == "DAX") {
//                        StockData = DocData.DAX;
//                    }
                    var stockDataArr = []

                    for(var i in DocData){
                    stockDataArr.push(DocData[i].quote)
                    }
                  //  console.log("stockmarket::::   "+JSON.stringify(DocData));
                    var data = {
                    stockDataArr :stockDataArr
                    }
                    console.log("stockmarkesymbollt:::   "+JSON.stringify(StockData));

                    $$.get("pages/stockmarket-detail.html", function(template) {
                        var compiledTemplate = AppyTemplate.compile(template);
                        var html = compiledTemplate(data);
                        mainView.router.load({
                            content: html,
...
function openPlusCodePage(title, index) {
    ...
    $$.get("pages/pluscode-map.html", function (template) {
        ...
        if(data.appData.layout == "slidemenu3d" || data.appData.layout == "slidemenu"){
            mainView.router.reloadContent(html);
        }else{
            mainView.router.load({
                content: html,
...
function findPlusCode() {
    if (isNaN(plusCodeLat) || isNaN(plusCodeLat)) {
        Appyscript.alert("Current location is not available right now. please search your location");
    }
    ...
    if (!isOnline()) {
        ...
        $$.ajax({
            ...
            success: function(result) {
                ...
                if (result.status == "OK") {
                    if (isGlobalPlusCodeRequest) {
                        var latLang = result.plus_code.geometry.location.lat + "," + result.plus_code.geometry.location.lng
                        goToPlusCodeCallback(plusCode, latLang);
                        isGlobalPlusCodeRequest = false;
                    } else {
                ...
                if (result.status == "INVALID_REQUEST") {
                    ...
                    apiname = "Google";
                    serviceFailedNotify(data.status, apiname, 1);
                    Appyscript.alert(result.error_message, appnameglobal_allpages);
                }
            },
            error: function(result) {
                ...
                Appyscript.alert(something_went_wrong_please_try_again, appnameglobal_allpages);
                apiname = "Google";
                serviceFailedNotify(result.status, apiname, 0);
            }
        });

    }
}
function newValueForThis(){
...
var globalPageName="";
function openPlusCodePageGlobal(pageName) {
    globalPageName=pageName;
    isGlobalPlusCodeRequest=true;
    $$.get("pages/pluscode-map.html", function (template) {
        var compiledTemplate = AppyTemplate.compile(template);
        var plusCodeData = {
            pageTitle: pageData.languageSetting.PLUS_CODE,
            getPlusCode: pageData.languageSetting.GET_PLUS_CODE,
            searchTitle: pageData.languageSetting.CURRENT_LOCATION
        };
        var html = compiledTemplate(plusCodeData);
        mainView.router.load({
            content: html,
...
function goToPlusCodeCallback(plusCode,latLng){
    if(globalPageName=="storePage"){
        goToStorePageCallBack(plusCode,latLng);
    }else if(globalPageName=="foodcourt"){
        foodGoToStorePageCallBack(plusCode,latLng);
    }
}
