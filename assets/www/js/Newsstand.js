    ...
    if (localStorage.getItem("email")) {
        ...
        if (paymentType == "paypal_express") {
            ...
            if (recuringAlbumPrice == "album") {
			  ...
			  if((mytype == "oneTime" && index != "0")|| albumPrice == "" || albumPrice === "undefinded")
				{
				descTxt = pageData.termconditionOneTime;
                    var id = itemIdIphone + "$$" + newsstandEditionId + "$$one$$" + iphoneInappSharedSecretKey;
                  //  inappNative(id);
                  paymentView.push({text: ''+pageData.newsstandLanguageText.newsstandBuyThis +' '+ pricePerItemCurrency + pricePerItem+'', click:function(){customAlertNewstand.hide();
                                                       inappNative(id);}})
...
//                        });
                        ...
                        paymentView.push({text: ''+pageData.newsstandLanguageText.newsstandBuyBundle +' '+ albumCurrency + albumPrice+'', click:function(){customAlertNewstand.hide();
                                                                             inappNative(id1);}})
//                        Appyscript.modal({
//                                         title: pageData.newsstandLanguageText.newsstandChoosePayment,
//                                         verticalButtons: true,
...
//                    });
                ...
                if (recuringAlbumPrice == "recuring") {
                    ...
                    if (mytype == "oneTime") {
                        ...
                        paymentView.push({text: ''+pageData.newsstandLanguageText.newsstandBuyThis +' '+ pricePerItemCurrency+ pricePerItem+'', click:function(){customAlertNewstand.hide();
                         ...
                         descTxt = pageData.termconditionSubscription;
                        mytype = "sub";
                        var button = [];
                        if (subscriptionPriceMonthly) {
							if(subscriptionPriceMonthly==''){
                            ...
                            button.push({
                             ...
                             paymentView.push({text: ''+pageData.newsstandLanguageText.newsstandBuyMonthly +' '+ subscriptionCurrencyMonthly + subscriptionPriceMonthly+'', click:function(){customAlertNewstand.hide();
                                                                         inappNative(id1);}})
                        }
                        if (subscriptionPriceYearly) {
                            ...
                            paymentView.push({text: ''+pageData.newsstandLanguageText.newsstandBuyYearly +' '+ subscriptionCurrencyYearly + subscriptionPriceYearly+'', click:function(){customAlertNewstand.hide();
                                                                         inappNative(id2);}})
                        }
                        button.push({
                            text: 'cancel',
            ...
            // New Code
                if(!customAlertNewstand) {
                            customAlertNewstand = customPopupNewstand({
                                                      text: descTxt,
                                                      buttons: paymentView
                                                      });
                        }
                        customAlertNewstand.show();
        }
        Appyscript.hideIndicator();
    } else {
...
var customPopupNewstand = function(data) {
    var thisPopup = this;
    $$(".custom-popup").remove();
...
function inappNative(a) {
    ...
    Appyscript.paymentViaGoogleIAP(itemIdIphone, iphoneInappSharedSecretKey, timePeriod, "NewsStands");
...
function paypal_express(subscriptionBundleId, editionBundleId, recuringAlbumPrice, albumPrice, albumCurrency, newsstandEditionId, subscriptionPriceMonthly, subscriptionCurrencyMonthly, previousEditionMonthly, subscriptionPriceYearly, subscriptionCurrencyYearly, previousEditionYearly, pricePerItem, pricePerItemCurrency, mytype) {
    ...
    var paypalUrlFormHtml = '<!-- URL --><input name="cancel_return" type="hidden" value="' + site_url + '/paypalmobile/newsstand-cancel"><input type="hidden" name="notify_url" value="' + site_url + '/paypalmobile/iap-purchase-notify/appId/' + localStorage.getItem("appid") + '/orderId/' + localStorage.getItem("unicDeviceIdWithTime") + '/subscriptionPeriod/' + subscriptionPeriod + '/pageId/' + pageIdentifie + '"/><input type="hidden" name="return" value="' + site_url + '/paypalmobile/newsstand-success" /><!-- Display the payment button. --><input type="image" src="https://www.paypalobjects.com/webstatic/mktg/logo/pp_cc_mark_37x23.jpg" name="submit" id="submit" alt="PayPal - The safer, easier way to pay online!"><img alt="" border="0" src="https://www.sandbox.paypalobjects.com/en_US/i/scr/pixel.gif" width="1" height="1"></form><script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.1/jquery.min.js"></script><script>function ClickButtonPaypal(){$(\'#submit\').trigger(\'click\');}</script></body></html>';
...
function inappSuccess(a, b) {
    ...
    if (a == 'success') {
        ...
        if (paymentTypeFlagPaypal == "oneTimeAll") {
            ...
            if ("subscription-monthly-all".includes(paymentTypeFlagPaypal)) {
                paymentTypeFlagPaypal = "subscription-monthly";
                console.log("cart data-----" + JSON.stringify(cart));
                var cartData = JSON.stringify(cart);
                ...
                $$.ajax({
                    ...
                    success: function(data) {
                        ...
                        var vimeoHdUrl;
                            ...
                            if(typeof vimeoData.download == "undefined" && typeof vimeoData.download == undefined){
                                                     vimeoHdUrl= encodeURI(vimeoData.download[1].link);
                                                     return;
                                                  }else{
                                                         vimeoHdUrl= encodeURI(vimeoData.link);
                                                         Appyscript.openWebView(vimeoHdUrl,pageData.pageTitle,"video", "true", "");
                                                         return;
                                                  }
...
Appyscript.showIndicator();
    ...
    if(a)
    {
       url = b;
            filetype = a;
    		audio_coverimg = c;
     if(filetype=="YouTubeVideo" || filetype == 'Website' ||filetype == 'Video' || pageData.iapAdvanceSettings.offline_support=="Off"
                                                               )
                                {
                                             ...
                                             if (!isOnline()) {
                                                    ...
                                                    return
                                                       ...
                                                       filetype = a;
                                					   audio_coverimg = c;
                                                       fileExistswithurl(filetype,url,audio_coverimg, dFileName);
...
function fileExists1(fileEntry){
...
function fileExistswithurl(filetype, url, audio_coverimg, dFileName) {
    Appyscript.showIndicator();
    setTimeout(function() {

        if (filetype == "resource") {
            AppyPieNative.openImageGallary(url, 0, "", "", "", "", data.appData.appName,"","");
        } else {
            filepathFP = url;
            if (filetype == 'Website') {
                if (filepathFP.includes(".mp4") || filepathFP.includes(".m4v")) {
                    ...
                    Appyscript.loadWebData(url, pageData.pageTitle, Appyscript.isBottom(pageData.pageTitle));
                }
            }
            if (filetype == 'Image') {
                ...
                if (filepathFP.includes(".mp3")) {
                    ...
                    Appyscript.playAudioUrls(jsonString, "rss", "", "channalNameValue", "yes", pageData.pageTitle, pageData.pageId);
                }
                if (filepathFP.includes(".pdf")) {
                ...
                if (filepathFP.includes("jpg") || filepathFP.includes("jpeg") || filepathFP.includes("png") || filepathFP.includes("gif")) {
                    AppyPieNative.openImageGallary(url, 0, "", "", "", "On", data.appData.appName," ",pageData.iapAdvanceSettings.auto_enable_share);
                }
                if (filepathFP.includes(".epub")) {
...
function fileExists(fileEntry) {
    ...
    console.log("File " + fileEntry.fullPath + " exists!");
        ...
        window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, fileSystempath, fileSystemFail);
        var filepathFP = '';
        function fileSystempath(fileSystem) {
            var download_link = encodeURI(url);
            fileName = download_link.substr(download_link.lastIndexOf('/') + 1);
    ...
    setTimeout(function() {
        ...
        if(pageData.iapAdvanceSettings.offline_support=="Off")
        {
          filepathFP = url;
        }
            ...
            if (filetype == 'Audio') {
                ...
                if (dFileName == 'undefined' || dFileName == null || dFileName == '' || dFileName == 'null' || dFileName == undefined)
                    ...
                    jsonString = jsonString + "@@--@@" + dFileName + "#####" + filepathFP + "#####" + audio_coverimg + "#####" + "";
                Appyscript.playAudioUrls(jsonString, "rss", "", "channalNameValue", "1", pageData.pageTitle, pageData.pageId);
            }
            if (filetype == 'Video') {
                AppyPieNative.playLiveSteamVideo(url, "", "", pageData.pageTitle, "", "", "", "", "");
            }
    ...
    function onDirectoryFails(error) {
        ...
        //        console.log("orderproductpaymenttype", orderproductpaymenttype, orderproductorderId);
             ...
             Appyscript.hideIndicator();
				...
				for (var i = 0; i < arrayProductId.length; i++) {
					...
					$$("#"+arrayProductId[i]).attr("onclick",'Appyscript.itemFile_inApp("'+$$("#"+arrayProductId[i]).attr("newsfullfiletypegen")+'","'+$$("#"+arrayProductId[i]).attr("newsfullfile")+'","'+$$("#"+arrayProductId[i]).attr("newsimageurl")+'", "'+$$("#"+arrayProductId[i]).attr("newsfullfiletitle")+'",this,"'+$$("#"+arrayProductId[i]).attr("newsfullfiletypegen")+'","'+"full"+'")');
