...
Appyscript.newsDetailsPageRss = function(a){
        ...
        var index = $$(a).attr("index");
         var checkDescription_img=false
         for(i=0;i<rssData.item.length;i++)
                    {
                    if(rssData.item[i].description){
                    rssData.item[i].description = rssData.item[i].description.replace("page", " ");
                    rssData.item[i].description = rssData.item[i].description.replace("opendeeplink ", "opendeeplinkpage");
                     if(rssData.item[i].description.indexOf('<img') != -1)
                      {
                       checkDescription_img =true;

                      }
                       if(rssData.item[i].enclosures.length && checkDescription_img==false)
                       {
                       rssData.item[i].imageval= rssData.item[i].enclosures[0].url;
                            if(rssData.item[i].imageval.indexOf("http")!="-1")
                                              {
                                            rssData.item[i].imageval = site_url + "/images/no-image.jpg";
                     ...
                     if(rssData.item[i].description.indexOf('<img') != -1) {
                     rssData.item[i].description = $$('<div>'+rssData.item[i].description+'</div>').find("img").eq(0).attr("src");

                    if(rssData.item[i].description.indexOf("http")!="-1")
                      {
                       rssData.item[i].imageval = site_url + "/images/no-image.jpg";
                          console.log("    rssData.item[i].imageval====="+   rssData.item[i].imageval)
                      }
...
Appyscript.newsDetailsPage = function(a){
    ...
    var newsId = $$(a).attr("data-id");
      newsshareVal = $$(a).attr("data-share");
        ...
        for(var i=0;i<newsIDSORT.length;i++)
        {
            ...
            if(catId =="search")
            {
                ...
                var json ='{"method":"searchNewsDetail","appId":"'+appid+'","pageIdentifire":"'+pageIdentifie+'","lang":"'+data.appData.lang+'","keyword":"'+keyword+'","page":"1","count":"200"}';
                      ...
                      var json ='{"method":"categoryNewsDetail","appId":"'+appid+'","pageIdentifire":"'+pageIdentifie+'","lang":"'+data.appData.lang+'","catId":"'+catId+'","page":"1","count":"100"}';
            }
    }

console.log("12132124124====="+json);
...
//            jsonData.langSetting = pageData.languageSetting;
            ...
            var getList = jsonData.list || [];
            if(getnewsarticalpublicprivate == "public"){
                getList = getList.filter(function(v){
                        return v.accessStatus == "public"
                 })
                jsonData.list =getList;

                index = getList.findIndex(function(v){
                    return v.id == newsId
                })
...
Appyscript.newsShare = function(a) {
 if(pageData.settings.enableShareContent == 1 || pageData.settings.enableShare == 1){

        Appyscript.shareAppWithoutlive(newsshareVal);
    }
    else{
           return
           // Appyscript.shareAppWithoutlive(newsshareVal);
       ...
       pageData.homecategories[index].item = data.feed.entries;
             ...
             $$.each(pageData.homecategories[index].item, function(i, v) {
                      ...
                      v.imageUrl = $$('<div>'+v.content+'</div>').find("img").eq(0).attr("src");
                       ...
                       if(v.imageUrl.indexOf("https")==-1)
                                             {
                                             v.imageUrl = site_url + "/images/no-image.jpg";
                ...
                if(v.enclosures && v.enclosures.length!=0) {

                   v.imageUrl =v.enclosures[0].url
                    if(v.imageUrl)
                    {
                    if(v.imageUrl.indexOf("https")==-1)
                       {
...
function parserssfeedwitoutext(url,index,menubar,catName){
    ...
    Appyscript.showIndicator();
var wsUrl = "https://snappy.appypie.com/services/utility-soap#rssfeed";
...
/* function for opening news detail on receiving push notification for news*/
function getNewsDetailById(newsDetailData){
   var wsUrl = webserviceUrl+"News.php";
            console.log("getNewsDetailById wsUrl : "+wsUrl);
            var appid = localStorage.getItem("appid");
            var newsDetail = newsDetailData.split("#pushCoupon#");
            console.log("getNewsDetailById newsDetail : "+newsDetail);
            pageIdentifie = newsDetail[1];
            var json ='{"method":"getNewsDetailsPush","appId":"' + data.appData.appId + '","pageIdentifire":"'+pageIdentifie+'","newsId":"' + newsDetail[0] + '"}';

            console.log("getNewsDetailById Request : "+json);
          ...
          if(isOnline())
            ...
            $$.ajax({
                    ...
                    success: function(jsonData, textStatus ){
                    Appyscript.hideIndicator();
                    var rssData=  JSON.parse(jsonData);
                    console.log("News Detail Info : "+ jsonData);
                    console.log("News Detail Info : "+ JSON.stringify(jsonData));
                     AppyTemplate.global.pageLanguageSetting = JSON.parse(jsonData).newsPageData.languageSetting;
                     AppyTemplate.global.styleAndNavigation = JSON.parse(jsonData).newsPageData.styleAndNavigation;
                     AppyTemplate.global.setting = JSON.parse(jsonData).newsPageData.settings;
                     pageData = rssData.newsPageData
                    ...
                    $$.get("pages/news-details.html", function (template)
                                   ...
                                   var html = compiledTemplate(rssData);
                                   mainView.router.load({content: html, animatePages: true});
                                    if(rssData.newsPageData.settings.enableShare == 0)
                                    {
                                     $$(".share-btn").hide();
                                    }
                                    if(rssData.newsPageData.settings.enableComments == 0)
                                    {
                                     $$(".comments-btn").hide();
                                    }
...
//                     {
//                        $$(this).attr('onclick','openNewsLinkPage(this)');
//                     });
//
//                       $$(page.container).find(".news-content").find('a').each(function() {
//                             var tempblogulr = $$(this).attr('href');
//                                 $$(this).attr('onclick', 'openNewsLinkPage("' + tempblogulr + '")');
//                         });
//                         setTimeout(function() {
...
function openNewsLinkPage(a){

    Appyscript.openWebView(a, pageData.pageTitle);
}
...
function openNewsLink(a){
    var getObj = $$(a).attr("data-link")
    Appyscript.openWebView(getObj, pageData.pageTitle);
}