...
Appyscript.onPageInit('quote-page',function(page){
                      ...
                      firebaseUrl=url+urlname+".json";
                       ...
                       if(pageData.databaseUrlSecretKey!="" && pageData.databaseUrlSecretKey != undefined){
                          firebaseUrl = firebaseUrl +"?auth="+pageData.databaseUrlSecretKey
                       }
...
function getQuoteOnSwipe(flag){
    ...
    var date = new Date(activeDate.replace(/-/g, "/"));
...
//                        quoteArray.push({"index":key,"quoteText":value.quoteText,"authorName":value.authorName,"displayDate":value.date,"mediaurl":value.mediaurl,"addMediaStatus":value.addMediaStatus,"addMediaType":value.addMediaType,"mediaresource":value.mediaresource,"arrayKey":i++,"addMediaSelectedType":value.addMediaSelectedType});
                       ...
                       if(currentDate.setHours(0, 0, 0, 0) >= new Date(value.date).setHours(0, 0, 0, 0) || value.date==""){
...
function displayQuote(activeDate){
    ...
    if(pageData.quotePreviousDaySetting=="Off"){ // For Date Wise
        ...
        if(indexElement.length>0){
            ...
            appendQuote(todayIndex,"quoteExist");
        }else{

            var getRandomIndex=getRandomInt(0,totalLengthofQuoteArray);
            appendQuote(getRandomIndex,"noQuote");
        }
...
function appendQuote(getRandomIndex,quoteFlag){
    Appyscript.hideIndicator();
    ...
    if(pageData.quotePreviousDaySetting=="Off"){
         currentDate=dailyQuoteDate.split("-");
    }else{
         currentDate=activeDate.split("-");
    }
    ...
    var dateFormatAccServer=dateFormat.replace("yyyy",currentDate[0]).replace("dd",currentDate[2]).replace("mm",currentDate[1]).replace("MM",theMonths[parseInt(currentDate[1])]).replace("DD",currentDate[2]);
        ...
        //ShowNoQuote
      //showRandomPreviousDate
      //ShowCustomQuote
        if(pageData.quoteShow=="ShowNoQuote" && quoteFlag=="noQuote"){
          $$("#quoteText").html('<div class="clr"></div><div class="animated '+animateType+'"><div class="msg-code" style="top:40vh"><div class="msg-container"><span class="icon appyicon-no-data" style="color: '+pageData.styleAndNavigation.heading[2]+'; border-color: '+pageData.styleAndNavigation.heading[2]+';"><div class="clr quote-time '+pageData.styleAndNavigation.heading[1]+'" style="text-align:center !important;"><span></span>'+dateFormatAccServer+'</div></span></div></div></div>');
          return false;
        }
        else if(pageData.quoteShow=="ShowCustomQuote" && quoteFlag=="noQuote"){
              $$("#quoteText").html('<div class="animated '+animateType+'"><div class="quote-time '+pageData.styleAndNavigation.heading[1]+'"><span></span>'+dateFormatAccServer+'</div><div class="quote-wrapper"><p id="quoteContent" class="'+pageData.styleAndNavigation.list[1]+'"><i class="icon-quote-left-1"></i>'+pageData.customText+'<i class="icon-quote-right-1"></i></p><div class="row"><div class="quote-share"><i class="icon-share-1" onclick=shareQuote("'+pageData.quoteShow+'");></i></div></div></div></div>');
          return false;
        }
        ...
        case "Yes":
            ...
            if(quoteArray[getRandomIndex].addMediaType=="video"){
                ...
                if(mediaURL==""){
                    ...
                    var imgURL="'"+mediaURL+"'";
                    mediaTypeHTML='<img onclick="openGalleryImage('+imgURL+')" src="'+mediaURL+'" class="lazy"/>';
                }
...
function shareQuote(getRandomIndex){
    ...
    if(getRandomIndex=="ShowCustomQuote"){
        var html = pageData.customText;
...
function getListOfFavourite(){
    ...
    myDB.transaction(function (transaction) {
                     ...
                     transaction.executeSql('SELECT * FROM m_dailyQuote', [],  function (tx, results) {
                                            ...
                                            //                                            </div>
                                            //                                            </div>
                                            $$("#favouriteQuote_txt").html('<div class="clr"></div><div class="msg-code" style="top:40vh"><div class="msg-container"><span class="icon appyicon-no-data" style="color: '+pageData.styleAndNavigation.heading[2]+'; border-color: '+pageData.styleAndNavigation.heading[2]+';"><b class="error_text">No records found.</b></span></div></div>');
