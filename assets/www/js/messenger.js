...
var maxFileSizeMessenger = (47*1024*1024);
...
var tempRepLock = 0;
var everyWhereArea = 6371 * 2;

var updateTimeFrequency = 1;
...
Appyscript.initializeMessenger = function () {
    ...
    if (isOnline()) {
                    ...
                    $$.ajax({
                        ...
                        success: function (jsonData, textStatus) {
                            ...
                            var filterData = areaCode.value.filter(function (item) {
                               var countryName = "";
                               if(jsonData.results != undefined && jsonData.results[0].address_components != undefined) {
                               jsonData.results[0].address_components.forEach(function(item,ind){
        ...
        messengerpageData.locationEnabled = true;
        messengerpageData.gpsEnabled = true;
        messengerpageData.friendSearchArea = everyWhereArea;
        ...
        if (localStorage.getItem("email") == null || localStorage.getItem("email") == '') {
            ...
            let storageKey = pageData.list[0].storageBucket == "" ? pageData.list[0].apiKey : pageData.list[0].storageBucket;
                            var config = {
            ...
            try{
                ...
                //Firebase Error Parse Error Fatal
                Appyscript.hideIndicator();
                var _langKey =pageData.languageSetting.firebase_key_error || "Cannot parse Firebase url."
                Appyscript.alert(_langKey,appnameglobal_allpages)
                return false;
            }
...
Appyscript.onPageInit("messenger-home", function (page) {

 if(localStorage.getItem("gpsStatus") != undefined ) {
    messengerpageData.gpsEnabled = localStorage.getItem("gpsStatus");

  }

    locChatLocationManager.gpsSetting();
});
...
function reloadChatList(reloadFrom) {
                                        ...
                                        locChatManager.updateChatDataModel().then(function(){
                                             ...
                                             <div class="item-title ">{{this.name}}</div>
                                                                                  <div {{#if this.userId}} id="messagedatehome{{this.userId}}" {{else}} id="messagedatehome{{this.groupId}}" {{/if}}  class="item-after {{@global.styleAndNavigation.content[0]}}  {{@global.styleAndNavigation.content[1]}}"  style="color:{{@global.styleAndNavigation.timeColor}};line-height: 25px;">{{messagereceiveddate this.date "home"}}</div>
                                             </div>
                                             ...
                                             <div class="item-after badge-icon">{{#isBlankData this.unread}} {{else}} <span  class="badge bg-yellow " style="background:{{@global.styleAndNavigation.badge[1]}}; color:{{@global.styleAndNavigation.badge[0]}}">{{this.unread}}</span> {{/isBlankData}}</div>
                                              ...
                                              {{#js_compare "this.distance == undefined"}}


                                              {{else}}
                                              <div class="item-title {{@global.styleAndNavigation.content[0]}} {{@global.styleAndNavigation.content[1]}}" style="color:{{@global.styleAndNavigation.content[2]}};padding-top: 10px;">{{compareDistance distance locality}}</div>
                                              {{/js_compare}}
                                    ...
                                    AppyTemplate.registerHelper('messagereceiveddate', function (value, from) {
            ...
            if (filterData[0]) {
                if (filterData2[0].name != filterData[0].name) {
                    ...
                    messengerpageData.recentContact.push({
                        ...
                        "lat":messengerpageData.allMessage[i].lat,
                        "long":messengerpageData.allMessage[i].long,
                        "locality":messengerpageData.allMessage[i].locality
                    });
...
function reloadContactList() {
                                 ...
                                 getAllContactsCrossVerified().then(function(){
                                                                    getRecentContact().then(function () {
                                                                                            ...
                                                                                            var c2 = "00";
                                                                                            if(a.name !== undefined)
                                                                                            c1 = a.name.toUpperCase();
                                                                                            if(b.name !== undefined)
                                                                                            c2 = b.name.toUpperCase();
                                                                                            return (c1 < c2) ? -1 : (c1 > c2) ? 1 : 0;
                                                                                            }
                                                                                            messengerpageData.recentContact.sort(sortUp);
                                                                                            messengerpageData.remianAllContact.sort(sortUp);

                                        locChatManager.updateRecentContact().then(function(){
                                                                                            Appyscript.hideIndicator();

                                                                                   var contactHtml = ` <li class="item-divider dividercss {{@global.styleAndNavigation.button[0]}} {{@global.styleAndNavigation.button[1]}}" style="font-weight:bold;background-color:{{@global.styleAndNavigation.button[2]}}; color:{{@global.styleAndNavigation.button[3]}}">{{@global.pageLanguageSetting.Contacts_Tab_My_Contacts}}</li>

                                                                                            {{#recentContact}}
                                                                                  <li onclick="openChatWindow('{{this.name}}','{{this.profilepic}}','{{this.userId}}','{{this.country}}','false',this)">
                                                                    ...
                                                                    {{else}}
                                                                                 <div class = "nearby-loc {{@global.styleAndNavigation.content[0]}} {{@global.styleAndNavigation.content[1]}}" style="color:{{@global.styleAndNavigation.content[2]}};">{{compareDistance distance locality}}</div>
                                                                    {{/js_compare}}

                                                                    {{/if}}

                                                                                            </div>
                                                                                            <i class="icon-chat-4" style="color:{{@global.styleAndNavigation.heading[2]}}"></i>
                                                                                            </div>
                                                                                            </a>
                                                                                            </li>
                                                                                            {{/recentContact}}
                                                                                            ...
                                                                                            <li class="item-divider dividercss {{@global.styleAndNavigation.button[0]}} {{@global.styleAndNavigation.button[1]}}" style="font-weight:bold;background-color:{{@global.styleAndNavigation.button[2]}}; color:{{@global.styleAndNavigation.button[3]}}">{{@global.pageLanguageSetting.Contacts_Tab_All_Contacts}}</li>
                                                                                            {{#remianAllContact}}
                                                                                    ...
                                                                                    <img src="{{#if this.profilepic}} {{this.profilepic}}{{else}}{{@global.style.reseller}}/newui/images/user-pic-news.png{{/if}}" class="avatar avatar-rounded chatlistimgprofile">
                                                                                            ...
                                                                                            {{#js_compare "this.name == this.phoneno"}} {{else}} <div class="item-title">{{this.name}}</div>{{/js_compare}}<div class="item-subtitle {{@global.styleAndNavigation.content[0]}} {{@global.styleAndNavigation.content[1]}}" style="color:{{@global.styleAndNavigation.content[2]}}">{{this.phoneno}}</div>
                                                                                             ...
                                                                                             <i class="icon-chat-4"
                                                                                   style="color:{{@global.styleAndNavigation.heading[2]}}"></i>
                                                                                            </div>
                                                                                            </a>
                                                                                            </li> {{/remianAllContact}}`;
...
function initializeUser() {
    ...
    locChatManager.defaltSettings()
    if(messengerpageData.locationEnabled && messengerFirebase != undefined ) {
      locChatLocationManager.triggerGeolocationForLatandLong()
...
function loginSinchNotPresent() {
    ...
    if (loginData.code == "Sucess") {
        ...
        if (isOnline()) {
            ...
            Appyscript.showIndicator();
            $$.ajax({
                url: webserviceUrl + "Messenger.php",
...
function loadUserData(id) {
    messengerFirebase.database().ref('user/' + id).once("value", function (snapshot) {
        ...
        //radical
        var deviceTokenObj = {deviceToken : Appyscript.getDeviceToken()}
        messengerFirebase.database().ref('user/' + id).update({deviceToken : Appyscript.getDeviceToken()})
        //radical
        messengerFirebase.database().ref('appId').once("value", function (snapshot) {
               if(snapshot.val() == null){
                     messengerFirebase.database().ref("/appId").set(appId, function (error) {
                                           console.log(error)
                                    });
                    }
            }, function (errorObject) {
        ...
        messengerFirebase.database().ref("login/" + phoneNo).once('value', function (snapshot) {
            if (snapshot.val() !== null) {
                ...
                resolve(false);
                     ...
                     Appyscript.hideIndicator();
                     var _langKey =  pageData.languageSetting.firebase_rules_error || "You are not authorised to access data !";
                     Appyscript.alert(_langKey,appnameglobal_allpages);
         });
    })
}
...
function openChatWindow(name, profilepic, userid, country, isMainScreen, context) {
    ...
    if (isOnline()) {
        ...
        messengerFirebase.database().ref('messageDetail/' + userId + "/" + userid).off('child_added');[34560
              ]
...
function checkMessageFromLive(userid, context) {
    messengerFirebase.database().ref('messageDetail/' + userId + "/" + userid).once('value', function (messageSnapshot) {
        ...
        getUserData(userid).then(function (data) {
            ...
            var localMessageArray = [];
            messageSnapshot.forEach(function (innerMessageSnapshot) {
                localMessageArray.push(innerMessageSnapshot.val());
            });
...
$$(document).on("click", "#chat_tab", function () {
                $$("#right a").css("visibility", "hidden");
                setTimeout(function () {
                           $$("#refreshBtn").css("display","none");
                           $$("#right a").css("visibility", "visible");
...
function loadChatHomePage() {
    ...
    localStorage.setItem("userChatRoomLoggedin", true);
    //Comment  below line because i need to update userdata at user initialize time - Rohit Chauhan
    //messengerpageData.userData = JSON.parse(localStorage.getItem("userdata"));
    ...
    $$.get("pages/messenger-home.html", function (template) {
        ...
        if (messengerpageData.allMessage.length) {
            $$("#right a").css("visibility", "visiable");
              $$("#refreshBtn").css("display","none");
        } else {
            $$(".messenger-tab a").eq(1).click();
        }
...
function sendLocation(groupId) {
    if (isOnline()) {
        ...
        var iframeHtml = '<iframe width="200" height="200" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://maps.google.com/maps?f=q&amp;source=s_q&amp;hl=en&amp;geocode=&amp;q=' + location.lat + ',' + location.lng + '&amp;output=embed"></iframe>';
        $$(".close-popup").eq(0).click();
...
function sendMessage(friednUserid, message, groupid, fileUniqueId) {
    if (isOnline()) {
        ...
        friednUserid = friednUserid || sessionStorage.getItem("chatuserid") || "false";

        if(friednUserid==='false'){
        ...
        if (chatType == "single") {
            ...
            messengerFirebase.database().ref('messageDetail/' + userId + "/" + friednUserid).push(messageData);
...
function appendMessage(messageText, messageType, uniqueId, contentTypeMessenger, audioLocalUrl, extra) {
    ...
    if (messageType == "sent") {
        ...
        if (contentTypeMessenger == "Image" || contentTypeMessenger == "Video") {
            ...
            $$("#" + uniqueId).parent().parent().attr("class", " message message-received message-pic width100");
            $$("#" + uniqueId).parent().addClass("width90");
            if (size == "") { } else {
                $$("#" + uniqueId).attr("size", size);
            }
    ...
    if (contentTypeMessenger == "Audio") {
        if (messageType == "sent") {
            ...
            $$($$("#" + uniqueId).parent()).parent().removeAttr("class").addClass("message message-received message-audio width100");
        }
        ...
        Appyscript.modal({
            ...
            buttons: [{
                ...
                onClick: function () {
                    $$("#mediaoption").hide();
                    $$("#emojioption").hide();
                     Appyscript.cameraPermission('captureimagecameraImage','Appyscript.permissionDenied')
                ...
                onClick: function () {
                    $$("#mediaoption").hide();
                    $$("#emojioption").hide();
                      Appyscript.cameraPermission('captureimagegalImage','Appyscript.permissionDenied')
...
var captureimagecameraImage = function () {
    let groupId = global_groupID
...
var captureimagegalImage = function () {
     let groupId = global_groupID
    navigator.camera.getPicture(onSuccess, onFail, {
        quality: 100,
        destinationType: Camera.DestinationType.DATA_URL,
                ...
                if (localData.unread != 0) {
                    if (snapshot.getKey() == id) {
                        ...
                        if ($$(conversationlistArray[i]).find("span").length == 0) {
                            ...
                            $$("#messagedatehome" + id).text(timeSince(localData.date));
                        } else {
...
var validatePhoneNo = function (phoneNo) {
    if (phoneNo == "undefined" || phoneNo == null || phoneNo == "" || phoneNo.length > 15 || phoneNo.length < 8) {
        return false;
    } else {
        return true;
    }
        ...
        Appyscript.modal({
            ...
            buttons: [{
                ...
                onClick: function () {
                    $$("#mediaoption").hide();
                    $$("#emojioption").hide();
                    Appyscript.cameraPermission('captureimagecameraVideo','Appyscript.permissionDenied')
                ...
                onClick: function () {
                    $$("#mediaoption").hide();
                    $$("#emojioption").hide();
                   Appyscript.cameraPermission('captureimagegalVideo','Appyscript.permissionDenied')
...
Appyscript.inviteUser = function (value) {
    ...
    if (Appyscript.device.android) {
        ...
        urlstr = "https://itunes.apple.com/lookup?bundleId=" + bundelId;
        if (isOnline()) {
            $$.ajax({
                url: urlstr,
                type: 'GET',
...
function insertUserDataInSql(userDataSql) {
    var queryStr = "INSERT INTO messengerUserData (userId, name, phoneNo, profilePic, country, lat, long, locality,isBlockUser)VALUES ('" + userDataSql.userid + "','" + userDataSql.name + "','" + userDataSql.phoneNo + "','" + userDataSql.profilePic + "','" + userDataSql.country + "','" + userDataSql.isBlockUser + "','" + userDataSql.lat + "','" + userDataSql.long + "','" + userDataSql.locality + "');";
...
function checkMessengerTable() {
    openAppyPieDatabase();
    return new Promise(function (resolve, reject) {
        var queryStr = 'CREATE TABLE messengerUserData(userId varchar(255) PRIMARY KEY,name varchar(255),phoneNo varchar(255),profilePic varchar(255),country varchar(255), lat varchar(255), long varchar(255), locality varchar(255), isBlockUser BOOLEAN);';
        queryMessengerSqlite(queryStr).then(function (data) {
            resolve("Not");
        }, function (error) {
            resolve("Exist");
                            ...
                            if (filterData[0]) {
                                ...
                                if (JSON.parse(localStorage.userdata).data.phoneno == resultSet.rows.item(i).phoneNo) { } else {
                                    messengerpageData.recentContact.push({
                                        ...
                                        "userId": resultSet.rows.item(i).userId,
                                        "lat": resultSet.rows.item(i).lat,
                                        "long": resultSet.rows.item(i).long,
                                        "locality": resultSet.rows.item(i).locality
                                    });
                                }
                            }
                        }
...
Appyscript.hideIndicator();
  ...
  //radical hotfix for setTimeout issues
                            var messengerLoader_isVisible = false;
                            function messengerLoader_show(){
                            messengerLoader_isVisible = true;
                            ...
                            function messengerLoader_hide(){
                            messengerLoader_isVisible = false;
                            ...
                            Appyscript.onPageAfterAnimation('messenger-phoneVerfication', function (page) {
                                                            messengerLoader_show()
                                                            setTimeout(function(){messengerLoader_hide()},3500);
                                                            });


//radical messenger push notification

Appyscript.messengerHandleNotification = function(_id,_content){
var currentChatId = $($("[message-userid]")).attr("message-userid");
if(currentChatId == _id){
//radical : notify user through some go-to-end of chat notification like in whatsapp.
}else{
//radical : when user is on another chat screen
if(currentChatId !== undefined && mainView.activePage.name != "messenger-home")
{
if (Appyscript.device.android) {
AppyPieNative.showSnackBarOnTop(_content);
}else
{
window.location = "messengertoast:" + _content;
}//radical : when user is not in messenger screen.
}else if(mainView.activePage.name != "messenger-home"){
if (Appyscript.device.android) {
AppyPieNative.showSnackBarOnTop(_content);
}else
{
...
//TODO: LocChat Feature

var selectedTab = null;
var locChatManager = {

                         calDistance:function(user_data)
                         {

                         if(messengerpageData.userData.data.lat == undefined && messengerpageData.userData.data.lat == undefined) {return undefined; }

                         var lat1 = messengerpageData.userData.data.lat;
                         var lat2 = user_data.lat;
                         var lon1 = messengerpageData.userData.data.long;
                         var lon2 = user_data.long

                         var R = 6371; // Radius of the earth in km
                         var dLat = locChatManager.deg2rad(lat2-lat1);  // deg2rad below
                         var dLon = locChatManager.deg2rad(lon2-lon1);
                         var a = Math.sin(dLat/2) * Math.sin(dLat/2) + Math.cos(locChatManager.deg2rad(lat1)) * Math.cos(locChatManager.deg2rad(lat2)) * Math.sin(dLon/2) * Math.sin(dLon/2);
                         ...
                         updateRecentContact:function() {
                         return new Promise(function(resolve, reject){
                                            messengerpageData.recentContact.forEach(function(item){
                                                                                    if(item.lat != undefined && item.long != undefined) {
                                                                                       item.distance = locChatManager.calDistance(item)

                                                                                    }
                                                                                    locChatManager.getFriendDetails(item)

                                            })

                                            messengerpageData.recentContact.sort(function(d1, d2){
                                                 if(d1.distance == undefined || d2.distance == undefined) {
                                                     return 1;

                                                 }

                                                 return (d1.distance < d2.distance) ? -1 : (d1.distance > d2.distance) ? 1 : 0;
                                            })
                         ...
                         getFriendDetails:function(item) {
                             var i;
                             for(i = 0; i < messengerpageData.contacts.length; i++) {
                                 var userContactDetails = messengerpageData.contacts[i]
                                 if(userContactDetails.phoneno == item.phoneno) {
                                    item.profilepic = userContactDetails.profilepic
                                    item.country = userContactDetails.country
                                    item.userId = userContactDetails.userId
                                    return;
                         ...
                         updateChatDataModel:function() {
                         return new Promise(function(resolve, reject){
                                            messengerpageData.allMessage.forEach(function(item){
                                                                                 if(item.lat != undefined && item.long != undefined) {
                                                                                 item.distance = locChatManager.calDistance(item)
                         ...
                         defaltSettings:function() {

                         if(pageData.list[0].location_base_chat == 0) {
                         locChatLocationManager.disableLocationService()

                         }else if(pageData.list[0].location_base_chat == 1) {
                         locChatLocationManager.enableLocationService()
                         if(localStorage.getItem("gpsStatus") != undefined) {
                         messengerpageData.gpsEnabled = localStorage.getItem("gpsStatus")
                         }
...
var locChatLocationManager = {
    ...
    disableLocationService:function() {
      messengerpageData.locationEnabled = false;

    },

    triggerGeolocationForLatandLong:function() {

        if(messengerpageData.gpsEnabled == "false" || messengerpageData.gpsEnabled == false) {return; }

        if( navigator.geolocation )
        {
         // Call getCurrentPosition with success and failure callbacks
          navigator.geolocation.getCurrentPosition(locChatLocationManager.success, locChatLocationManager.fail);
        }
        else
        {
    ...
    success:function(position)
    {
       ...
       locChatLocationManager.getReverseGeocodingData(position.coords.latitude ,position.coords.longitude)
    ...
    fail:function(error)
    {
      console.log(error);
        ...
        geocoder.geocode({ 'latLng': latlng }, function (results, status) {
             ...
             locChatLocationManager.updatelocationtofirebase(lat, lng, addrStr);

             });
        },

    updatelocationtofirebase:function(lat, long, addr) {
            var updates = {
            "lat":lat,
            "long":long,
            "locality":addr
            ...
            var user_id =  JSON.parse(localStorage.getItem("userdata")).userid;
            console.log(user_id)

             messengerpageData.userData.data.lat = lat;
             messengerpageData.userData.data.long = long;
             messengerpageData.userData.locality = addr

             if((messengerpageData.gpsEnabled == "false" || messengerpageData.gpsEnabled == false)) {
            ...
            if(user_id != undefined && user_id != '') {
                database.ref().child('user').child(user_id).update(updates)
    ...
    gpsSetting:function() {
     ...
     if(pageData.list[0].location_disable == 0) {
          $(".gps_block").hide()
          return;

        }else if(pageData.list[0].location_disable == 1){
          $(".gps_block").show()

        }


    if(messengerpageData.gpsEnabled == "true" || messengerpageData.gpsEnabled == true) {
              $("#switchgps").prop("checked", true)
              makeGpsSwitchOn()

            }else {
              $("#switchgps").prop("checked", false)
              makeGpsSwitchOff()
...
AppyTemplate.registerHelper('compareDistance',function (a,b) {
                            if(parseInt(a) < parseInt(messengerpageData.friendSearchArea)){
                               return b + "," +"  "+ a + " km"
...
AppyTemplate.registerPartial('user', '<li><h2>{{firstName}} {{lastName}}</h2><p>{{bio}}</p></li>')
...
function eventOnGps() {
    if(messengerpageData.gpsEnabled == false || messengerpageData.gpsEnabled == "false") {
       ...
       messengerpageData.gpsEnabled = false;
       makeGpsSwitchOff()

    }

    localStorage.setItem("gpsStatus", messengerpageData.gpsEnabled)

}

function makeGpsSwitchOff() {
    if($$("#gpsSwitch").hasClass("switchOn")) {
       $$("#gpsSwitch").removeClass("switchOn");
       $$("#gpsSwitch").addClass("switchOff")
       $$("#gpsSwitch").text("Off")
    }

}

function makeGpsSwitchOn() {
    if($$("#gpsSwitch").hasClass("switchOff")) {
        $$("#gpsSwitch").removeClass("switchOff");
        $$("#gpsSwitch").addClass("switchOn")
        $$("#gpsSwitch").text("On")
    }
...
function openPeopleSearchList() {
    Appyscript.popupPage("messenger-searcharea")

}

function closePeopleSearchList(){
    Appyscript.popupClose()

}

function showNearbyFriendsOnMap() {

    if( !isabletoOpenMap()) {
      Appyscript.alert("This app need to access your location");
     return;

    }

    if (isOnline()) {
    $$.get("pages/messenger-map-page.html", function (template) {
           var compiledTemplate = AppyTemplate.compile(template);
...
function isabletoOpenMap() {
    if(localStorage.localLatitude != undefined || localStorage.localLongitude != undefined || localStorage.localLatitude != '' || ocalStorage.localLongitude != '') {return true }
...
$$(document).on('pageInit', 'div[data-page="messenger-map"]', function (page) {
Appyscript.showIndicator();
...
if (isOnline()) {
    ...
    map = new google.maps.Map(document.getElementById("messengerMapId"), {
    zoom: 16,
    center: myLatLng,
    ...
    for (i = 0; i < messengerpageData.recentContact.length; i++) {

    var markerData = messengerpageData.recentContact[i]
    if( !checkMarkerAvailable(markerData) ) {continue;  }
    ...
    var label2 = document.createElement("label");
                $(label1).html(markerData.name + "<br>");
                $(label2).text(markerData.locality + " " +  markerData.distance + " " + "KM");
    $(boxText).append(label1)
    $(boxText).append(label2)
    boxList.push(boxText);

    google.maps.event.addListener(marker, 'click', (function (marker, j) {
        return function () {
                infowindow.setContent(boxList[j]);
        ...
        google.maps.event.addDomListener(boxList[j],'click',(function(marker, j) {
                                                                 return function() {
                                                                 openChatWindowForUser(marker.id, this)
...
function openChatWindowForUser(phoneNo,context) {

    var i;
    for(i = 0; i < messengerpageData.contacts.length; i++) {
        var userContactDetails = messengerpageData.contacts[i]
        if(userContactDetails.phoneno == phoneNo) {
         openChatWindow(userContactDetails.name,userContactDetails.profilepic,userContactDetails.userId,userContactDetails.country,false,context)
        return;
...
function eventOnSearchAreaTab(element) {
updateFriendSearchArea($(element).find('label').text())
selectedTab = $(element).find('input').attr("id");
$$("#friendSearchAreaId").text($(element).find('label').text())
closePeopleSearchList()

}

function updateFriendSearchArea(text) {
    if(text == "10 Miles") {
       messengerpageData.friendSearchArea = (10 * 1.609344).toFixed(2)

    }

    if(text == "25 Miles") {
       messengerpageData.friendSearchArea = (25 * 1.609344).toFixed(2)

    }

    if(text == "50 Miles") {
       messengerpageData.friendSearchArea = (50 * 1.609344).toFixed(2)
    }

    if(text == "100 Miles") {
       messengerpageData.friendSearchArea = (100 * 1.609344).toFixed(2)
    }
    if(text == "200 Miles") {
       messengerpageData.friendSearchArea = (200 * 1.609344).toFixed(2)
    }
    if(text == "Anywhere") {
       messengerpageData.friendSearchArea = everyWhereArea;
    }
...
function checkMarkerAvailable(markerData) {
  if(markerData.lat != undefined && markerData.long != undefined && markerData.lat != '' && markerData.long != '') {return true; }
