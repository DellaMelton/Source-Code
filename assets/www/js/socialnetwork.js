...
var socialNetworkingUrl = webserviceUrl+'SocialNetwork.php';
var socialNetworkingPostUrl = site_url;
...
Appyscript.SocialNetDataFollowers = function(a){
    ...
    { var newlistfollowinglist={"data":[]};
        ...
        $$.ajax({
                ...
                type: 'post',
                //123  headers: {'accessToken': deviceEncryptedToken},
                async: true,
        ...
        $$.ajax({
           ...
           //123  headers: {'accessToken': deviceEncryptedToken},
                ...
                for(i=0;i<data.data.length;i++){
                 $$("#container"+data.data[i].id).remove();
                var videourl='';
                var imagel='';
                var video_shareurl='';
    ...
    if(new Date($$("#socialbdate").val()) > new Date()){
        ...
        if(isOnline())
        {
            ...
            $$.ajax({
                    ...
                    var SocialNetData ={};
                    Appyscript.hideIndicator();
                    Appyscript.alert(AppyTemplate.global.pageLanguageSetting.sc_profile_upload_success,AppyTemplate.global.pageLanguageSetting.alert_social_network)
                    //                $$.get("pages/socialnetwork-stream.html", function (template) {
                    //                       var compiledTemplate = AppyTemplate.compile(template);
                    //                       var html = compiledTemplate(SocialNetData);
                    //                       mainView.router.load({content: html, animatePages: true});
...
Appyscript.Socialcomment = function(postid) {
    ...
    if(isOnline())
    {
        ...
        var  _srcValue=$$("#answerImage").attr("src").replace("data:image/jpeg;base64,","").replace("https://cdn.pixabay.com/photo/2015/02/24/15/41/dog-647528_1280.jpg","");

        if(_srcValue ==="" && FinalSocialcomment===""){
        Appyscript.hideIndicator();
            return false;
        }
        ...
        $$.ajax({
                url: socialNetworkingUrl,
                data: '{"method":"saveComment","appId":"' + appId + '","image":"'+_srcValue+'","userId":"' + localStorage.getItem("userid") + '","postId":"'+postid+'","comment":"'+FinalSocialcomment+'","lang":"' + localStorage.lang + '","pageIdentifier":"' + pageIdentifie + '"}',
              ...
              //123  headers: {'accessToken': deviceEncryptedToken},
                ...
                Appyscript.hideIndicator();
                 var imageData="";
                 if(data.commentDetail.image!==""){
                    imageData = `<div style='clear:both;'>
                                <img onclick =openGalleryImage('${data.commentDetail.image}'); src='${data.commentDetail.image}' class='commentImage'  />
                            </div>`;
                 }
                ...
                compiledBGTemplate=AppyTemplate.compile("<li style='border-color:{{@global.styleAndNavigation.fieldBorderColor}}'  id='commentid"+data.commentDetail.id+"'  > <div class='title-container likeList'><div class='profile-snap'><img style='background:url(https://snappy.appypie.com/media/splash/1351667928.jpg) no-repeat center center' alt='' src='"+data.commentDetail.avatar+"'></div> <div class='profile-content'> <a href='#' class='{{@global.styleAndNavigation.heading[0]}} {{@global.styleAndNavigation.heading[1]}}' style='color:{{@global.styleAndNavigation.heading[2]}}; text-align:{{@global.styleAndNavigation.heading[3]}}'><strong>"+data.commentDetail.username+"</strong></a><span class='{{@global.styleAndNavigation.content[0]}} {{@global.styleAndNavigation.content[1]}}' style='color:{{@global.styleAndNavigation.content[2]}};'> "+data.commentDetail.message+" </span>"+imageData+" <span class='comment_time {{@global.styleAndNavigation.content[0]}} {{@global.styleAndNavigation.content[1]}}' style='color:{{@global.styleAndNavigation.content[2]}}; text-align:{{@global.styleAndNavigation.heading[3]}}'> "+data.commentDetail.addedon+"</span><div class='post-ban'><a  class='iconz-option-vertical'   postuserid='"+data.commentDetail.userId+"' commentid='"+data.commentDetail.id+"' id='commentpopblock'  style='color:{{@global.styleAndNavigation.iconColor}};'></a>   </div> </div></div> </li>");
                bghtml=compiledBGTemplate();
...
//Appyscript.onPageInit('socialnetwork-Page',function(page){
                      ...
                      if (mainView.activePage.name =='socialnetwork-Page') {
                       ...
                       //Function was getting twice once on
                      }
                      setTimeout(function(){Appyscript.Socialpostpage(1);},500);
                     /* setTimeout(function() {
                          if(pageData.socialGenralSetting.share_on_off == 0){
                            $("li").find("#hideShareKey").hide();
                          }else{
...
//function showAlertForPayment()
//{
//
//    if(pageData.inAppAndroidBundleId == "" && pageData.inAppPublicKey == "")
//    {
...
function showAlertForPaymentSocial(typeOfPayment) {
...
function callbackAfterPaymentOnSocial(status, transactionId, productId, payType,pageTypeid,paymentTypeIap,subscriptionPrice,subscriptionCurrency)
        {
        console.log("**********************callbackAfterPaymentOnSocial***********************************************************")
         if(status=="success")
              {
              loginFromSocialNetwork = true;
                 ...
                 Appyscript.showIndicator();
                    ...
                    else
                    {
                     ...
                     uploadSocialTransectionDetailAfterPayment(transactionId,pageTypeid, productId,paymentTypeIap,subscriptionPrice,subscriptionCurrency)
                    }
             }
...
var socialNetworkPaymentStatus;
 function uploadSocialTransectionDetailAfterPayment(transactionId,pageTypeid, productId,paymentTypeIap,subscriptionPrice,subscriptionCurrency)
 {
...
var socialNetworkIAPData = {};
function openPaymentPageForSocialIap()
{
 socialNetworkIAPData = { monthlySubscriptionPrice:pageData.monthlySubscriptionPrice,
                                    yearlySubscriptionPrice:pageData.yearlySubscriptionPrice,
                                    oneTimeSubscriptionPrice:pageData.oneTimeSubscriptionPrice,
                                   oneTimeSubscriptionCurrency:pageData.oneTimeSubscriptionCurrency,
                                   yearlySubscriptionCurrency:pageData.yearlySubscriptionCurrency,
 ...
 $$.get("popups/socialnetwork-check-iap.html", function(template) {

var compiledTemplate = AppyTemplate.compile(template);
var html = compiledTemplate(socialNetworkIAPData);
...
function checkSocialIAPPayment()
     {
...
Appyscript.onPageAfterAnimation('socialnetwork-Page',function(page){
        if (mainView.activePage.name =='socialnetwork-Page' && data.appData.layout !="bottom") {
           Appyscript.homePageInit()
        }
});
...
Appyscript.Socialsendtoserver =function(event) {
    ...
    if ( $$("#statusbox").val().trim() == '' && socialvideosendurl=="" && sendsoimages.length==0) {
...
socialimagebase64.push(element.simage)
        ...
        params.pageIdentifier = pageIdentifie;
        ...
        console.log("dsdds" + options.params.status.trim());
        var statusTrimValue = options.params.status.trim();
     /*   if(statusTrimValue == "")
        {
             Appyscript.alert(AppyTemplate.global.pageLanguageSetting.social_please_enter_status ,AppyTemplate.global.pageLanguageSetting.alert_social_network);
             Appyscript.hideIndicator();
        ...
        ft.upload(sspath, socialNetworkingPostUrl+'/webservices/SocialNetworkPost.php', win, fail,
                  options);
    ...
    if(data2.data.image.length>0){
        ...
        if(data2.data.image.length==1){
            ...
            imagel2+= "<div class='block-1'><img src="+data2.data.image[0]+"  onclick=Appyscript.openImageWithPathIndexHeader('"+data2.data.image[0]+"','"+imageno+"','"+AppyTemplate.global.socialpagetitle.replace(/\s/g, "&nbsp;")+"'); alt=''  class='banner-back' /></div>";
                                                                            ...
                                                                            imagel2+= "<div class='block-1'><img src='images/2X1.png'  onclick=Appyscript.openImageWithPathIndexHeader('"+data2.data.image[0]+"','"+imageno+"','"+AppyTemplate.global.socialpagetitle.replace(/\s/g, "&nbsp;")+"'); alt=''  class='banner-back' style='background-image:url(https://snappy.appypie.com/timthumb.php?src="+data2.data.image[0]+"&h=150&w=300);'/></div>";
                                                                     }
    ...
    if(isOnline())
    {
        ...
        $$.ajax({
               ...
               //123  headers: {'accessToken': deviceEncryptedToken},
                ...
                $$("#streampostcount").text(eval(parseInt($$("#streampostcount").text())-1)+ AppyTemplate.global.pageLanguageSetting.post_without_colon_social_net);
                $$("#streampostcount2").text(eval(parseInt($$("#streampostcount2").text())-1)+ AppyTemplate.global.pageLanguageSetting.post_without_colon_social_net);
...
mypostallcountlikeslenght=data.myTotalLikePost;
 ...
 $$("#streampostcountlike").text(mypostallcountlikeslenght+ AppyTemplate.global.pageLanguageSetting.post_without_colon_social_net);
                var socialfollow='';
                var socialfollowing='';
                ...
                $$.ajax({
                    ...
                    $$("#streampostcount").text(mypostallcountlenght+ AppyTemplate.global.pageLanguageSetting.post_without_colon_social_net)
                       ...
                       // mypostvideourl+="<video width='100%' poster='"+data[i].video[0][0]+"' controls><source src='"+data[i].video[0][1]+"' type='video/mp4'></video>";
                        ...
                        myprofilepostimg+= "<div class='block-1'><img src="+data[i].image[0]+" onclick=Appyscript.openImageWithPathIndexHeader('"+data[i].image[0]+"','"+imageno+"','"+AppyTemplate.global.socialpagetitle.replace(/\s/g, "&nbsp;")+"'); alt=''  class='banner-back' /></div>";
                                ...
                                myprofilepostimg+= "<div class='block-1'><img src='images/2X1.png'  onclick=Appyscript.openImageWithPathIndexHeader('"+data[i].image[0]+"','"+imageno+"','"+AppyTemplate.global.socialpagetitle.replace(/\s/g, "&nbsp;")+"'); alt=''  class='banner-back' style='background-image:url(https://snappy.appypie.com/timthumb.php?src="+data[i].image[0]+"&h=150&w=300);'/></div>";

                        }

                        }else if(data[i].image.length==2){

                        myprofilepostimg+= "<div class='block-2'><img src='images/1X1.png' onclick=Appyscript.openImageWithPathIndexHeader('"+data[i].image[0]+","+data[i].image[1]+"','0','"+AppyTemplate.global.socialpagetitle.replace(/\s/g, "&nbsp;")+"'); alt=''  class='banner-back' style='background-image:url(https://snappy.appypie.com/timthumb.php?src="+data[i].image[0]+"&h=300&w=300);'/><img onclick=Appyscript.openImageWithPathIndexHeader('"+data[i].image[0]+","+data[i].image[1]+"','1','"+AppyTemplate.global.socialpagetitle.replace(/\s/g, "&nbsp;")+"'); src='images/1X1.png' alt='' class='banner-back' style='background-image:url(http://snappy.appypie.com/timthumb.php?src="+data[i].image[1]+"&h=300&w=300);'></div>";
...
videourl+= "<div class='sn_video_container' onclick=Appyscript.opensocialvideoplayer('"+data[i].video[0][1]+"','"+AppyTemplate.global.socialpagetitle.replace(/\s/g, "&nbsp;")+"');> <img src='images/2X1.png' style='background:url("+data[i].video[0][0]+")'/> <div class='sn_play_btn icon-play-circle2-1'></div> </div>";
                ...
                mylikeposts+= "<div class='block-1'><img src="+data[i].image[0]+"  onclick=Appyscript.openImageWithPathIndexHeader('"+data[i].image[0]+"','"+imageno+"','"+AppyTemplate.global.socialpagetitle.replace(/\s/g, "&nbsp;")+"'); alt=''  class='banner-back' /></div>";
                      ...
                      mylikeposts+= "<div class='block-1'><img src='images/2X1.png'  onclick=Appyscript.openImageWithPathIndexHeader('"+data[i].image[0]+"','"+imageno+"','"+AppyTemplate.global.socialpagetitle.replace(/\s/g, "&nbsp;")+"'); alt=''  class='banner-back' style='background-image:url(https://snappy.appypie.com/timthumb.php?src="+data[i].image[0]+"&h=150&w=300);'/></div>";
                                 ...
                                 $$.ajax({
                                     ...
                                     //123  headers: {'accessToken': deviceEncryptedToken},
                                         ...
                                         if(data.msg == "Unfollow"){
                                                 unfollowMsg = AppyTemplate.global.pageLanguageSetting.unfollow_social_network;
                                                 }
                                                 else
                                                 {
                                                 unfollowMsg = AppyTemplate.global.pageLanguageSetting.follow_social_network;
                                                 }
                                         $$("#socailfollowers").text(unfollowMsg);


                                         //
                                         $$("#socailfollowers"+friendId).text(unfollowMsg);
...
function trimSuccess2(result) {
    ...
    //$$("#muploadvid").append("<li id='socialvidup' style='border-color:{{@global.styleAndNavigation.fieldBorderColor}}'><img  class='img-style' src='images/image-1x1.png' uploadpicurl="+result+" style='background:url("+result+") no-repeat center center'><div class='overlay'><button id='uploadpicid"+cvid+"' onclick='Appyscript.SocialRemovevid(this);' class='icon-cancel' style='color:{{@global.styleAndNavigation.iconColor}}'></button></div> </li >");
    compiledBGTemplate=AppyTemplate.compile("<li id='socialvidup' style='border-color:{{@global.styleAndNavigation.fieldBorderColor}}'><img  class='img-style' src='images/image-1x1.png' uploadpicurl="+result+" style='background:url("+result+"?key"+((new Date()).getTime())+") no-repeat center center'><div class='overlay'><button id='uploadpicid"+cvid+"' onclick='Appyscript.SocialRemovevid(this);' class='icon-cancel' style='color:{{@global.styleAndNavigation.iconColor}}'></button></div> </li >");
    bghtml=compiledBGTemplate();
    $$("#muploadvid").append(bghtml);
    $$("#addsocialvideo").hide();
...
Appyscript.socialuserprofileDetails = function(userprofilefriendid){
    ...
    var isHideSignPhone = data.login.fixedFields.isHideSignPhone;
    var hideDOB = data.login.fixedFields.isHideDate_4;
    var hideGender = data.login.fixedFields.isHideGender_1;
    //var hideAbout = data.login.fixedFields.isHideText_2;
     ...
     $$.ajax({
       ...
       //123  headers: {'accessToken': deviceEncryptedToken},
             ...
             Appyscript.hideIndicator();
                ...
                $$("#streampostcount2").text(usermypostcountshow+"  "+ AppyTemplate.global.pageLanguageSetting.post_without_colon_social_net);
             var checkimge2=[];
             for(i=0;i<data.length;i++){
             var imagel='';
             var myprofilepostimg='';
           ...
           myprofilepostimg+= "<div class='block-1'><img src="+data[i].image[0]+" onclick=Appyscript.openImageWithPathIndexHeader('"+data[i].image[0]+"','"+imageno+"','"+AppyTemplate.global.socialpagetitle.replace(/\s/g, "&nbsp;")
             ...
             +"'); alt=''  class='banner-back' style='background-image:url(https://snappy.appypie.com/timthumb.php?src="+data[i].image[0]+"&h=150&w=300);'/></div>";
             }
...
$$.ajax({
    ...
    //123  headers: {'accessToken': deviceEncryptedToken},
     ...
     Appyscript.loadmoreusermypostPhoto(1);
}
else
{
Appyscript.alert(internetconnectionmessage, appnameglobal_allpages);
...
AppyTemplate.registerHelper('socialindividualpost',function (data) {
                            ...
                            if(data.length==1){
                              ...
                              if(pageData.styleAndNavigation.displayFullImage==1){
                            myprofilepostimgind+= "<div class='block-1'><img src='images/2X1.png'  onclick=Appyscript.openImageWithPathIndexHeader('"+data[0]+"','"+imageno+"','"+AppyTemplate.global.socialpagetitle.replace(/\s/g, "&nbsp;")+"'); alt=''  class='banner-back' style='background-image:url(https://snappy.appypie.com/timthumb.php?src="+data[0]+"&h=150&w=300);'/></div>";
...
if($$($$(".swiper-slide.active")[0]).find("a")[0].attributes[0].nodeValue =="#tabs3"){
 ...
 {
 setTimeout(function(){
            usermyphotoload=usermyphotoload+1;
            console.log("Photoload"+usermyphotoload)
            Appyscript.loadmoreusermypostPhoto(usermyphotoload);
...
function selectPhotoComments_SN() {
    Appyscript.modal({
     ...
     buttons: [{
               ...
               },
               {
                   ...
                   onClick: function() {
                       if (Appyscript.device.android)
                        Appyscript.storagePermission('runEventGallery_SN', 'Appyscript.permissionDenied')
                       else
                        runEventGallery_SN();
                       }
                   }, {
...
function getPhotoFromCamera_SN() {
    if(Appyscript.device.android)
    {
        Appyscript.cameraPermission('Appyscript.customphotocaptureimagecamera_SN','Appyscript.permissionDenied')
    }
    else
    {
        Appyscript.customphotocaptureimagecamera_SN()
    }
}

//get custom photo from camera
Appyscript.customphotocaptureimagecamera_SN = function(event) {
    ...
    function onSuccessImgForum(mediaFiles) {
        ...
        $$('#answerImage').attr('src', "data:image/jpeg;base64," + mediaFiles).show();
    }
...
function runEventGallery_SN(){
    var options = {
        quality: 100,
        sourceType: Camera.PictureSourceType.PHOTOLIBRARY,
        destinationType: navigator.camera.DestinationType.DATA_URL
    }
    navigator.camera.getPicture(fromEventGallery_SN, errImageUpload_SN, options);

    function fromEventGallery_SN(imageURI){
        $$('#answerImage').attr('src', "data:image/jpeg;base64," + imageURI).show();
    }

    function errImageUpload_SN(message){
    }
}


Appyscript.loadmoreusermypostPhoto = function(countload) {
Appyscript.showIndicator()
$$.ajax({
     url: socialNetworkingUrl,
     data: '{"method":"myImages","appId":"' + appId + '","userId":"' + userprofilefriendid + '","page":"' + countload + '","loginuserId":"' + localStorage.getItem("userid") + '","lang":"' + localStorage.lang + '"}',
     type: 'post',
     async: true,
     ...
     for (m = 0; m < data[i].images.length; m++) {
     checkimge2.push(data[i].images[m]);
     imagel += "<li><a href='#' class='nit6' onclick='Appyscript.SocialNetDataPost(" + data[i].id + ");'><img src='images/image-1x1.png' style='background:url(" + data[i].images[m] + ");' class='img-style'/></a></li>";
     }
     }
     $$("#socialstreamimage2").append(imagel);
       Appyscript.autoHeightSocialPage2(2)
     }
     if (checkimge2.length > 0) {
     $$("#socialuserprofilesimages2icon").hide();
     }
