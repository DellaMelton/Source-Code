...
//                console.log("Appyscript.openDoc : "+ DocData);
                root.items = [];
                $$.each(DocData.items, function(index, element){
//                  if(element.shared){
                      if(!element.labels.trashed){
                          if(element.owners[0].emailAddress == GdriveEmailid){
                             try{
                                 if(element.parents[0].isRoot)
                                  {
//                                    console.log("ROOT ELEMENT : "+ element.id);
                                    if(element.mimeType.indexOf("application/vnd.google-apps.folder") != -1)
                                    {
                                        element.items = [];
                                        root.items.push(addChild(element.id, element));
                                    }else{
                                        root.items.push(element);
                                    }
                                  }
                                  else
                                  {
...
//            console.log("Root Node : "+ JSON.stringify(root));

                $$.get("pages/document-listing.html", function(template) {
                       var compiledTemplate = AppyTemplate.compile(template);
                       var html = compiledTemplate(root);
                       mainView.router.load({
                                            content: html,
                                            animatePages: true
                                            });
                      ...
                      if(!element.labels.trashed){
                          if(element.owners[0].emailAddress == GdriveEmailid){
                             try{
                                 if(!element.parents[0].isRoot)
                                  {
                                    if(element.parents[0].id.indexOf(id) != -1){
//                                        console.log("Not ROOT ELEMENT : "+ element.id);
                                        if(element.mimeType.indexOf("application/vnd.google-apps.folder") != -1)
                                        {
                                            element.items = [];
                                            parent.items.push(addChild(element.id, element));
                                        }else{
...
Appyscript.openDocDrive= function(tokken) {
    ...
    if(isOnline())
    {
        ...
        $$.ajax({
                url: "https://api.onedrive.com/v1.0/drive/root/view.delta?expand=thumbnails,children(expand=thumbnails(select=large,c200x150_Crop))&&access_token="+tokken,
                type: 'get',
                async: true,
                success: function(data) {
...
//                                                    console.log("elementtitle   "+newFilterAddress+" elementembedLink "+element.embedLink+" elementmimeType "+element.mimeType+" elementcreatedDate "+element.createdDate)
                                                ...
                                                }else if(element.mimeType.indexOf("application/vnd.google-apps.folder") != -1){
//                                                    console.log("Folder Log : "+element);
//                                                    console.log("Folder Log : "+JSON.stringify(element));
                                                    var newFilterAddress = element.title.replace(/(<([^>]+)>)/ig, "");
//                                                    console.log("elementtitle   "+newFilterAddress+" elementembedLink "+element.embedLink+" elementmimeType "+element.mimeType+" elementcreatedDate "+element.createdDate)
                                                    Documentlisting+='<li data-name="'+ newFilterAddress+'" onclick=addChildHtml("'+element.id+'"); style="background-color:'+AppyTemplate.global.styleAndNavigation.listmenu[3]+'"> <div class="dc_identi"> <img src="images/image-1x1.png" class="img-style dc_overlay" /> <i class="appyicon-foldericon" style="color:'+dociconcolor[doccssobj[element.mimeType]]+'"></i> </div> <div class="dc_title '+AppyTemplate.global.styleAndNavigation.filenameFont+' '+AppyTemplate.global.styleAndNavigation.filenameSize+'"  style="color:'+AppyTemplate.global.styleAndNavigation.filenameColor+'"> '+newFilterAddress+' <span  class="'+AppyTemplate.global.styleAndNavigation.dateTimeFont+' '+AppyTemplate.global.styleAndNavigation.dateTimeSize+'" style="color:'+AppyTemplate.global.styleAndNavigation.dateTimeColor+'">Created at:'+new Date(element.createdDate).getMonth() + '-' + new Date(element.createdDate).getDate() + '-' + new Date(element.createdDate).getFullYear()+'</span> </div> </li>'
                                                }
                                            }
                                        }
                                    }
...
var childArr;
var nodeHistory = [];
function addChildHtml(node){
    var child = {};
    if(!child[0] && nodeHistory[0]){
        child = nodeHistory.filter(function(v){
               return v.id==node;
            })
    }

    if(!child[0]){
            child = childArr.filter(function(v){
               return v.id==node;
             })
              $$.each(childArr, function(index, element){
                    if(element.mimeType.indexOf("application/vnd.google-apps.folder") != -1)
                        nodeHistory.push(element);
              });
    }

    if(!child[0]){
            child = root.items.filter(function(v){
                   return v.id==node;
                })
               if(child[0])
                nodeHistory.push(child[0]);
    }

    $$.get("pages/document-listing.html", function(template) {
                           var compiledTemplate = AppyTemplate.compile(template);
                           var html = compiledTemplate(child[0]);
                           mainView.router.load({
                                                content: html,
                                                animatePages: true
                                                });
