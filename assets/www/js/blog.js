...
//            Appyscript.RssPage1(a);
        ...
        $$.each(data.feed.entries, function(index, value) {
            ...
            var thisRow = {
                ...
                "desc": desData,
                "contentImg": "https://snappy.appypie.com/images/no-image.jpg",
                "featured": 0
            }
            if (index == 0) {
                thisRow.featured = 1;
            ...
            if (value.description.indexOf("<img") != -1) {
                if (thisRow.desc.indexOf("img") > 0) {
                    ...
                    if (imgJPG.match(/\.(jpeg|jpg|gif|png)$/) != null) {
                        thisRow.contentImg = imgJPG;
                    } else {
                        if (value.enclosures.length > 0) {
                            thisRow.contentImg = value.enclosures[0].url;
                        }
            ...
            try {
                var getLink = thisRow.contentImg;
                if (getLink.indexOf("https") == -1) {
                    getLink = getLink.replace("http", "https");
                }

                thisRow.contentImg = getLink;
    ...
    $$.get(blogData.value, function(data) {
        ...
        $$(data).find("item").each(function(i) {
            ...
            var thisRow = {
               ...
               // "contentImg": "https://supersnappy.appypie.com/images/no-image.jpg",
                "contentImg":  site_url + "/images/no-image.jpg",
                "featured": 0
            }
            if (i == 0) {
                thisRow.featured = 1;
...
var blogPageScroll = true;
var compiledBlogTemplate = null;
Appyscript.scrollBlogPage = function(self) {
    if (blogData.identifire == "blogger" || blogData.identifire == "tumbler" || blogData.identifire == "wordpress") {
        return;
    }
    var scrollContentDiv = document.getElementById("blogPageScrollContent");
    if (blogPageScroll) {

        if ((scrollContentDiv.scrollTop + 20 + scrollContentDiv.clientHeight) >= scrollContentDiv.scrollHeight) {
            blogPageScroll = false;
            setTimeout(function() {
                pageidN = pageidN + 1;
                Appyscript.getCustomBlogData(pageidN);
...
Appyscript.getCustomBlogData = function() {
    ...
    var wsUrl = webserviceUrl + "Blog.php";
    var appid = localStorage.getItem("appid");
    //var pageIdentifire = localStorage.getItem("BlogPageIndenty");
    console.log("===== blogData.identifire : " + pageIdentifie);
    var json = '{"method":"articleList","appId":"' + appid + '","pageIdentifire":"' + pageIdentifie + '", "page":"' + pageidN + '","count":"10"}';
    if (isOnline()) {
        $$.ajax({
            url: wsUrl,
            ...
            success: function(jsonData, textStatus) {
                ...
                if (pageidN > 1) {
                    ...
                    if (tempData.list.length > 0) {
                        blogPageData.list = blogPageData.list.concat(tempData.list);
                        $$.get("pages/blog.html", function(template) {
                            var compiledBlogTemplate = AppyTemplate.compile(template);
                            var html = compiledBlogTemplate(blogPageData);
                            $$(".blog-list").html($$('<div>' + html + '</div>').find(".blog-list").html())
                ...
                if (listData == null) {
                    ...
                    $$.each(listData, function(ind, val) {
                        if (val.contentImg.trim() == "") {
                            //listData[ind].contentImg = "https://supersnappy.appypie.com/images/no-image.jpg";
                            listData[ind].contentImg = site_url + "/images/no-image.jpg";
                        }
...
Appyscript.BlogDetailsPage = function(index) {
    ...
    console.log("===== data details : " + data.desc);
    ...
    data.articleLocation = blogPageData.list[index].articleLocation;
...
Appyscript.BlogFontsSize = function(a) {
    ...
    Appyscript.modal({
        ...
        verticalButtons: true,
        buttons: [{
                text: pageData.languageSetting.small,
                onClick: function() {
                    $$(".blog-page").removeClass("small").removeClass("normal").removeClass("large").addClass("small");
                    localStorage.popup = false
                }
            },
            {
                text: pageData.languageSetting.normal,
                onClick: function() {
                    $$(".blog-page").removeClass("small").removeClass("normal").removeClass("large").addClass("normal");
                    localStorage.popup = false
                }
            },
            {
                text: pageData.languageSetting.large,
                onClick: function() {
                    $$(".blog-page").removeClass("small").removeClass("normal").removeClass("large").addClass("large");
                    localStorage.popup = false
                }
            },
            {
...
Appyscript.BlogSharecustom = function(val) {
    ...
    if (pageData.settings.enableShare == 1) {
    ...
    copyBlogCode(document.getElementById("decvalue"));
    ...
    if (pageData.settings.enableShare == 1) {
        ...
        if (pageData.settings.enableShareContent == 0) {
            Appyscript.shareText(header + ' ' + datalink);
        } else {
...
function copyBlogCode(element_id) {
...
Appyscript.DoneBlogPastComment = function(id) {
    ...
    var json = {
        "method": "addComment",
        "appId": appid,
        "pageIdentifire": pageIdentifie,
        "articleId": postCommentId,
        "email": localStorage.getItem("email"),
        "comment": commentTxt,
        "enableCommentsAuto": "1"
    }
        ...
        $$.ajax({
            ...
            success: function(jsonData, textStatus) {
                ...
                if (pageData.settings.enableCommentsAuto == "1") {
                    ...
                    Appyscript.alert(pageData.languageSetting.comment_successfully_post, '', function() {
                        $$("#text").val("");
                    });
                    ...
                    Appyscript.alert(pageData.languageSetting.congratulation_blog, '', function() {
                        $$("#text").val("");
                    });
                    Appyscript.hideIndicator();
                }
