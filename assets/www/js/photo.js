...
var customgalleryurl = webserviceUrl + 'Gallery.php';
//custom photo
var imageUrl = '',
    picsLikes = '',
    picsComment = '',
...
Appyscript.getFacebookAuthToken = function() {
    var AcessToken='';
    var facebookcount= 20;
    ...
    $.ajax({
           ...
           console.log(AcessToken,'this is it')
...
Appyscript.PhotoPage = function(a, b) {
    ...
    if (isOnline()) {
        ...
        photoData.index = a;
        if (b == "picasa") {
            var postURL = 'https://picasaweb.google.com/data/feed/api/user/' + photoData.value + '/album/' + photoData.albumName;
            Appyscript.PhotoJSON(postURL)
            ...
            Appyscript.getFacebookAuthToken();
        } else if (b == "flickr") {
...
Appyscript.onPageInit('photo-Page', function(page) {
    ...
    $$(".plus").click(function() {
        ...
        if (pageData.list[photoData.index].configSettings.enableUserLogin == 1) {
        ...
        $$(".infinite-scroll").on('infinite', function() {
            ...
            $$.get(nextURL, function(data) {
                ...
                $$.each(data.data, function(key, value) {
                    var a = {
                        "photoImage": value.picture,
                        "largeImage": value.source
                    }
            ...
            $$.each(data.photos.photo, function(key, value) {
                ...
                if (typeof value.url_m !== "undefined") {
                    a = {
                        ...
                        "photoImage": value.url_m,
                        "largeImage": value.url_m
                    };
        ...
        $$.get(postURL, function(data) {
            ...
            $$.each(data.photosets.photoset, function(key, value) {
                var a = {
                    ...
                    "photoTitle": value.title._content,
                    "photoImage": value.primary_photo_extras.url_m
                }
                getData.list.push(a);
            });
    ...
    if (isOnline()) {
        ...
        $$.ajax({
            ...
            success: function(jsondata) {
                ...
                var a = {
                    "photoTitle": "",
                    "photoImage": data2.value[0].large.url,
                    "largeImage": data2.value[0].large.url
                }
    ...
    if (isOnline()) {
        ...
        if (photoData.identifire == "customalbum") {
            ...
            $$.each(photoData.list, function(key, value) {
                var a = {
                    "photoTitle": value.photoTitle,
                    "photoImage": value.photoImage,
                    "largeImage": value.photoImage
                }
                getData.list.push(a);
        ...
        if (photoData.identifire == "photo360album") {
            ...
            $$.each(photoData.list, function(key, value) {
                var a = {
                    "photoTitle": value.photoTitle,
                    "photoImage": value.photoImage,
                    "photoImagePreview": value.photoImagePreview
                }
                getData.list.push(a);
                imageUrls += value.photoImage + ",";
            });
            ...
            success: function(data) {
                ...
                if (photoData.identifire == "picasa") {
                    ...
                    $$(data).find("entry").each(function(i) {
                        var a = {
                            "photoTitle": "",
                            "photoImage": $$(this).find("media").attr("url"),
                            "largeImage": $$(this).find("media").attr("url")
                        }
                        getData.list.push(a);
                ...
                if (photoData.identifire == "pinterest") {
                    ...
                    $$.each(data.data.pins, function(key, value) {
                        var a = {
                            "photoTitle": "",
                            "photoImage": value.images["237x"].url,
                            "largeImage": value.images["237x"].url
                        }
                        getData.list.push(a);
                ...
                if (photoData.identifire == "facebook") {
                    ...
                    if (data.data.length == 0) {
                        Appyscript.hideIndicator();
                        Appyscript.alert("Does not found any Photo", appnameglobal_allpages);
                        return false;
			   ...
			   */
                ...
                if (photoData.identifire == "instagram") {
                    ...
                    $$.each(dataProfileFromInsta.data, function(key, value) {
                        ...
                        var a = {
                            "photoTitle": textValue,
                            "photoImage": value.images.thumbnail.url,
                            "largeImage": value.images.standard_resolution.url
                        }
                        getData.list.push(a);
                ...
                if (photoData.identifire == "onedrive") {} else {
                    openPage("photo", getData);
                }
...
//Appyscript.open360Photo = function(photoImage, photoName){
////	var div = document.getElementById('container');
//
//
// 				   $$.get("pages/photo-360.html", function (template) {
//                                   var compiledTemplate = AppyTemplate.compile(template);
...
Appyscript.openPhoto = function(position, a) {
    ...
    flagCheck = false;
    /* Some People were double clicking within a fraction of second*/
...
function loadMore() {
    ...
    $$.get(globalPostURL, function(instadata, status) {
        ...
        if (localData.pagination.next_url == undefined) {} else {
...
Appyscript.getPhotoTokenOnedrive = function(refresh_token, client_id, client_secret) {
    if (isOnline()) {
        ...
        $$.ajax({
            ...
            data: 'client_id=' + client_id + '&redirect_uri=https://snappy.appypie.com/app/onedrive&client_secret=' + client_secret + '&refresh_token=' + refresh_token + '&grant_type=refresh_token',
            type: 'post',
...
Appyscript.Opencustomphotogallery = function() {
    ...
    if (isOnline()) {
        ...
        $$.ajax({
            ...
            data: Appyscript.validateJSONData('{"method":"galleryList","appId":"' + appId + '","pageId":"' + identifirecustomphoto + '","offset":"0","limit":"1000","userEmail":"' + userEmail + '"}'),
            type: 'post',
...
Appyscript.Opencustomphotolist = function(id, name) {
    ...
    if (isOnline()) {
        ...
        $$.ajax({
            url: customgalleryurl,
            data: Appyscript.validateJSONData('{"method":"galleryList","appId":"' + appId + '","pageId":"' + identifirecustomphoto + '","offset":"0","limit":"1000","albumId":"' + id + '","userEmail":"' + userEmail + '"}'),
            type: 'post',
            //321  headers: {'accessToken': deviceEncryptedToken},
            async: true,
            success: function(data) {
...
function openphotoframecustomphotowithcaption(photoindex, picsLikes, picsComment, picsText, checkshare, imageTitle, isFromInstagram) {
    ...
    myPhotoBrowser = Appyscript.photoBrowser({
        ...
        exposition: true,
        lazyLoading: true,
        navbarTemplate: '<div class="navbar hidenavbar"> <div class="navbar-inner " style="background-color: {{@global.style.headerBarBackgroundColor}}; color: {{@global.style.headerBarTextColor}};"> <div class="left sliding"> <a onclick="closeAppypiegalary();"  style="color:{{@global.style.headerBarIconColor}};"> <i class="icon icon-left-open-2"></i> </a> </div> <div class="center sliding"> <div class="topHeader {{@global.style.headerBarSize}} {{@global.style.headerBarFont}}"></div> </div> <div class="right" > <a  ' + shareicon + ' class="appyslim-ui-options" style="color:{{@global.style.headerBarIconColor}}; transform: rotate(90deg);" onclick="Appyscript.customphotosaveshare()" style="color:#ffffff;"></a></div> </div> </div>',
...
Appyscript.customphotocaptureimagecamera2 = function(event) {
    ...
    function onSuccess(imageURI) {
        ...
        getFileContentAsBase64(imageURI, function(base64Image) {
            ...
            sendsoimagescustomphoto.push({
                'simage': btoa(base64Image)
            });
...
if (isOnline()) {
    ...
    if (sendsoimagescustomphoto.length == 1) {
        ...
        $$.ajax({
            url: customgalleryurl,
            data: '{"method":"uploadImages","appId":"' + appId + '","pageId":"' + identifirecustomphoto + '","albumId":"' + customphotoalbumid + '","images":"' + socialimagebase64customphoto + '","captions":"' + customphotocaption.join("#!p@$%") + '","deviceId":"' + Appyscript.getDeviceId() + '","uploadBy":"' + email + '","autoApprove":"' + photoData.configSettings.autoApprovePhotos + '"}',
            type: 'post',
           //321  headers: {'accessToken': deviceEncryptedToken},
            async: true,
            success: function(data) {
                ...
                if (photoData.configSettings.autoApprovePhotos == 0) {
                    Appyscript.alert(AppyTemplate.global.pageLanguageSetting.gallery_alert_with_review, appnameglobal_allpages);
...
Appyscript.customphotosaveshare = function() {
    ...
    if (photoData.configSettings.allowUsersReport == 1 && pageData.photoShare == "On") {
        photoactionsheet = [{
                                    text: data.languageSetting.common_save,
                                    onClick: function () {
                                    ...
                                    Appyscript.photopagedelete(photodatadddd.data[myPhotoBrowser.activeIndex])
...
function galleryTakeCustom() {
    ...
    if (mainView.activePage.name == "photo-Page") {} else {
        mainView.router.back();
    }
    window.imagePicker.getPictures(
        function(results) {
...
Appyscript.photopagedelete=function(link)
{  var identifirecustomphoto = pageIdentifie;
       if (pageIdentifie.indexOf("--") != -1) {
           identifirecustomphoto = pageIdentifie.split("--")[0];
       }
    console.log('{"method":"deleteImage","appId":"' + appId + '","pageId":"'+pageIdentifie+'","albumId":"'+customphotoalbumid+'","imageId":"'+socialimagebase64customphoto+'"}');
    if(isOnline())
    {
    $$.ajax({
            url: customgalleryurl,
            data: Appyscript.validateJSONData('{"method":"deleteImage","appId":"' + appId + '","pageId":"'+identifirecustomphoto+'","albumId":"'+customphotoalbumid+'","imageId":"'+link.id+'"}'),

            type: 'post',
            async: true,
            success: function(data) {
            var dmsg = JSON.parse(data);
            var newmessage='';
             if(dmsg.msg ==="no image found")
             { newmessage = pageData.languageSetting.image_already_deleted;}
             if(dmsg.msg ==="image deleted successfully")
             { newmessage = pageData.languageSetting.image_delete_success;}
            Appyscript.alert(newmessage , appnameglobal_allpages,function(){ Appyscript.hideIndicator(); closeAppypiegalary(); });
            },
            error: function(){
            Appyscript.hideIndicator();
            Appyscript.alert(something_went_wrong_please_try_again,appnameglobal_allpages);
