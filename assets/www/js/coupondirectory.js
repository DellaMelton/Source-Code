var CouponDirWebURL=webserviceUrl + 'CouponDirectory.php';
var globalcatidcdir='';
...
var userIdValue;
var deeplinkDirDetail;
var pageDataCouponDir;
var pageDataDeeplink = {}
/******************************************************************/
/* This function is used to open coupondirectory menu page.*/
Appyscript.opencoupondirectoryMenuPage = function() {
    ...
    $$.get("popups/coupon-directoryMenu.html", function(template) {
...
/* This function is used to coupondirectory edit profile */
Appyscript.coupondirectoryEditProfile = function() {

    if (localStorage.getItem("email") == null) {
        Appyscript.loginPage('true');
        callbackLogin = Appyscript.coupondirectoryEditProfile;
        return;
    }
    AppyTemplate.global.styleAndNavigation = pageData.styleAndNavigation;
    callbackLogin = null;
    ...
    jsondata.saveBtnTxt = pageData.languageSetting.dir_save_btn;

    $$.get("popups/coupondirectory-edit-profile.html", function(template) {
        var compiledTemplate = AppyTemplate.compile(template);
...
Appyscript.saveProfilePicCouponDir = function() {
    ...
    if (isOnline()) {
        Appyscript.updateDirProfile(localStorage.getItem("appid"), name, localStorage.getItem("email"), profilePicPath, "coupondirectory","","");
    } else {
...
function profileUpdateCallBackcouponDir(isProfileUpdate, name, imgurl) {
    Appyscript.hideIndicator();
    if (isProfileUpdate == "success") {
        ...
        Appyscript.alert("Updated", appnameglobal_allpages);
...
$$(document).on('pageInit', 'div[data-page="coupondirectory-myCoupon"]', function(page) {
    if (mainView.activePage.name == "coupondirectory-myCoupon") {
        if (localStorage.getItem("coupondirectbookmarked" + pageData.couponDirectoryPageId) && localStorage.getItem("coupondirectbookmarked" + pageData.couponDirectoryPageId) == '') {
            localStorage.setItem("coupondirectbookmarked" + pageData.couponDirectoryPageId, "");
        }
...
function coupondirectoryOpenCouponList() {
    Appyscript.showIndicator();
    Appyscript.popupClose();
    $$.ajax({
        url: CouponDirWebURL,
        data: '{"method":"getScratchedWithoutRedeemCoupons","appId":"' + localStorage.getItem("appid") + '","deviceId":"' + localStorage.uniqueid + '","lang":"' + data.appData.lang + '","couponDirectoryPageId":"' + pageData.couponDirectoryPageId + '"}',
        type: 'post',
        //321 headers: {'accessToken': deviceEncryptedToken},
        async: true,
        success: function(data) {
            data = JSON.parse(data);
            console.log("OpenlistData    " + JSON.stringify(data));
            Appyscript.hideIndicator();
                    coupondetailpagedataonllistonly = data.coupons;
                    coupondetailpagedataonlmainlispage = data.coupons
                    coupondetailpagedata = data.coupons;
                    data.heading = pageData.languageSetting.cdir_my_coupons;
                    $$.get("pages/coupondirectory-mycoupon.html", function(template) {
                        var compiledTemplate = AppyTemplate.compile(template);
 ...
 Appyscript.showIndicator();
    $$.ajax({
            ...
            CoupanDetailsdirectory(data.couponsWithSubCategory.coupons[0].id,'0')
                ...
                {
                data.heading= check.getAttribute("data-heading");
                }
...
var coupansharepwaurl = pageDataService.pwaUrl;
function CoupanDetailsdirectory(couponId,index){
    ...
    if(isOnline())
    {
       $$.ajax({
                ...
                console.log("dddddddddddd  "+JSON.stringify(jsonData));
                ...
                coupon_unluckycdir = pageDataService.withdraw.coupon_unlucky;
                scratchImageGlobalcdir =  pageDataService.withdraw.image;
                coupansharepwaurl = pageDataService.pwaUrl;
                jsonData.defaultCurrency = pageData.setting.couponDirectoryDefaultCurrency;
                jsonData.hide_text = pageData.hide_text;

                if(pageDataService.deeplinkInfo != undefined){
                    deeplinkDirDetail=pageDataService.deeplinkInfo.deeplink;
                }
                // expirecoupon();
...
Appyscript.cdircouponServiceStamp = function(a) {
    ...
    pageDataService.heading=serviceHeading;
...
function coupanRedeemValuecdir() {
  ...
  Appyscript.alert(pageData.languageSetting.pin_blank);
    ...
    if(localStorage.getItem("userid")){    userIdValue=localStorage.getItem("userid");}else{   userIdValue='';}
        ...
        if(pageData.couponDirectoryPageId == undefined){
            pageData.couponDirectoryPageId = couponDetail[1];
        }
...
function scratchCart_cdir() {
    ...
    (function() {
     ...
     console.log("filled  "+filledInPixels + '%');
     ...
     setTimeout(function() {
                ...
                $$.ajax({
                        ...
                        if (textStatus == 200 || textStatus == 'success') {
                         console.log("ffffff " + JSON.stringify(jsonData));
...
Appyscript.coupansharedirectory(coupansharepwaurl);
    } else {
...
/*****************************************************/
//CouponDirectory DEEP Link With Direcory.
/*****************************************************/
var deeplinkValDirecorty;
var couponDetailDirectory;
var pageDataDetail;
var header;
function openDirectoryList() {
    pageDataCouponDir = pageData;
    lang = data.appData.lang;
    deeplinkValDirecorty = deeplinkDirDetail;


    console.log("deeplinkDirDetail==  " + deeplinkDirDetail);
    couponDetailDirectory = deeplinkValDirecorty.split("#", 4);

     if(couponDetailDirectory.length <4){
             opendeeplinkpage(deeplinkValDirecorty)
                  return;
      }
    ...
    var pageNameDir = couponDetailDirectory[1].split("_",1);
    console.log("deeplinkDirDetail  " + couponDetailDirectory);
    localStorage.setItem("couponDir","directoryCoupon");
    var jsonPostecom = '{"method":"getListingWithPageData","appId":"' + localStorage.getItem("appid") + '","dirPageId":"' + couponDetailDirectory[1] + '","catId":"' + couponDetailDirectory[2] + '","listId":"' + couponDetailDirectory[3] + '","lang":"' + lang + '"}';
    console.log("jsonPostecom " + jsonPostecom + " ghg  " + webserviceUrl + 'CouponDirectory.php');
    Appyscript.showIndicator();

    $$.ajax({
        url: webserviceUrl + 'CouponDirectory.php',
        ...
        success: function(jsonData, textStatus){
            ...
            pageData = responcedata.pageData;
            pageDataDetail = responcedata.pageData;
            if(pageData.listing.list != undefined){
                AppyTemplate.global.pageLanguageSetting = pageData.languageSetting;
                AppyTemplate.global.styleAndNavigation = pageData.styleAndNavigation;
                ...
                AppyTemplate.global.header = pageData.listing.list[0].header;

                Appyscript.hideIndicator();
                 $$.get("pages/services-DetailCoupon.html", function(template) {
                        var compiledTemplate = AppyTemplate.compile(template);
                        ...
                        $$('#pagesCSS').attr('href', 'css/' + pageNameDir + '.css');
                        bindServiceDetails(0);
                        claimButtonShowHideCoupon();
                        mapinitializeCoupon();
                    });
             }else{
                Appyscript.hideIndicator();
...
/*****************************************************/
//END Coupon Link Directory
/*****************************************************/

function setUpPageData(){

    let _pageData = pageDataDeeplink || {}
    if(_pageData.pageId == "coupondirectory"){
         pageData = _pageData
         AppyTemplate.global.couponcurentcycode = pageData.setting.couponDirectoryDefaultCurrency
         AppyTemplate.global.styleAndNavigation = pageData.styleAndNavigation;
         AppyTemplate.global.pageLanguageSetting = pageData.languageSetting;
...
$$(document).on('pageAfterBack', 'div[data-page="coupondirectory-detailcoupon"]', function(e) {
    setUpPageData()
});