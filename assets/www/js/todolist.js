...
if(finalalertdata!="")
{
Appyscript.alert("Do you want to reset your Data?");
}
	 var saveData = [];	
...
function shareTodolist() {
    var sharedata="";
    var shareDataPre = localStorage.getItem("todolistData" + todolistIndex + pageIdentifie);
    var shareDatajson= JSON.parse(shareDataPre);
    var valuetask;
    for(var i=0;i<shareDatajson.length;i++)
    {
      if(shareDatajson[i].checked){
            valuetask=AppyTemplate.global.pageLanguageSetting.lang_complete;
        }else{valuetask=AppyTemplate.global.pageLanguageSetting.lang_task_pending}
        sharedata+= "Task: "+shareDatajson[i].taskDesc +"\n" +" Complete: "+ valuetask +"\n" +" Note: "+ shareDatajson[i].noteText +"\r\r";
    }
        ...
        $$(page.container).on('change', '.listMenu input', function(){
           ...
           $$(page.container).on('click', '.listMenu .icon-edit, .listMenu  .icon-chat-3', function(){
            ...
            $$(page.container).on('click', '.okbtn', function(){
                  ...
                  if(content.length<1)
                  {
                      Appyscript.alert(AppyTemplate.global.pageLanguageSetting.lang_task_text);
