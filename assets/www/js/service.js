...
var swiperServices;
var ServiceURL = webserviceUrl + 'Directory.php';
...
function initializeService() {
    ...
    localStorage.setItem("couponDir","directory");

    AppyTemplate.global.setting = pageData.setting;
    AppyTemplate.global.languageSetting = pageData.languageSetting;
    ...
    ServiceURL = webserviceUrl + 'Directory.php';
...
Appyscript.createDirSubpage = function(a, catId, categoryName, sortByDirName, autosearch) {
       ...
       var catName = categoryName.split(" ");
        ...
        var newcatname = "";
                for (var i = 0; i < catName.length; i++) {
                    newcatname = newcatname + catName[i].replace(/['",]+/g, ' ');
                }
        console.log(newcatname);
        categoryName = newcatname.split(",").join(" ");
      }
    if (autosearch == "autosearch") {
        ...
        document.getElementById('search').value = "";
        $$("#res").html("");
         if(pageData.setting.dirDisableLocServices==0 && sortByDirName == "distance"){
             sortByDirName = "alphabet";
         }else{
            sortByDirName = sortByDirName;
         }
        if(sortByDirName == "distance" && pageData.setting.dirDisableLocServices==0)
               {
                   ...
                   catId+'","count":"500","pageNo":"1","searchText":"","searchListId":"","catName":"'+categoryName+'","listSort":"'+sortByDirName+'","latitude":"' + latval + '","longitude":"' + lngval + '"}'
               }
    } else {
        var index = a.getAttribute("data-index");
        if(sortByDirName == "distance" && pageData.setting.dirDisableLocServices==0){
         jsonval = '{"method":"catListingWithSubCategory","appId":"' + localStorage.getItem("appid") + '","latitude":"' + localStorage.getItem("localLatitude") + '","longitude":"' + localStorage.getItem("localLongitude") + '","listType":"cat","dirPageId":"' + pageIdentifie + '","catId":"' +
                     catId + '","count":"500","pageNo":"1","searchText":"","searchListId":"","catName":"' + $$(a).attr("data-heading") + '","listSort":"alphabet"}'
         }else{
         jsonval = '{"method":"catListingWithSubCategory","appId":"' + localStorage.getItem("appid") + '","latitude":"' + localStorage.getItem("localLatitude") + '","longitude":"' + localStorage.getItem("localLongitude") + '","listType":"cat","dirPageId":"' + pageIdentifie + '","catId":"' +
                     catId + '","count":"500","pageNo":"1","searchText":"","searchListId":"","catName":"' + $$(a).attr("data-heading") + '","listSort":"' + sortByDirName + '"}'
         }
    ...
    var aaaa= performance.now();
        ...
        $$.ajax({
            ...
            success: function(jsonData, textStatus) {
               ...
               var bbbb= performance.now();
               AppyTemplate.global.timevalue=bbbb-aaaa;
...
function showPopup (latitude, longitude, address, openthirdparty) {
    var localLatitude = localStorage.getItem("localLatitude");
    var localLongitude = localStorage.getItem("localLongitude");

 if(pageData.setting.dirArMapView!="0")
        {
           Appyscript.modal({
                   title:pageData.languageSetting.conitnue_to_view_in,
                   buttons:[{
                       text:pageData.languageSetting.ar,       onClick: function(){AppyPieNative.getDirectionAR(localLatitude,localLongitude,latitude,longitude)}
                   },{
                       text:pageData.languageSetting.map,       onClick: function(){GetDirectionFun1(latitude, longitude, '', address, '', openthirdparty)}
                   },
                   {
    ...
    if(pageData.setting.dirDisableLocServices == 1 ){
        ...
        Appyscript.modal({
            ...
            buttons: [{
                ...
                onClick: function() {
                    showPopup(latitude, longitude, address, openthirdparty);
...
AppyTemplate.registerPartial('dirListing', '<li id="serviceinnerSubHeaderPage"  class="listing-item" style="border-bottom:2px solid {{@global.styleAndNavigation.borderColor}}">' +
    ...
    '{{#root_Compare @global.setting.displayQuickActionIcons "==" "1"}} <span class="hyper-addons">{{#if latitude}}' +
    '{{#root_Compare @global.setting.dirMapDisplay "==" "1"}}{{#root_Compare @global.setting.checkonEnable "==" "1"}}{{#root_Compare @global.setting.dirDisableLocServices "==" "1"}}<a href="#" class="icon-ok-4" style="background-color:{{@global.styleAndNavigation.icon[0]}}; color:{{@global.styleAndNavigation.icon[1]}};" onclick="Appyscript.checkinPage(this,{{listId}},{{catId}},\'{{dirPageId}}\',{{latitude}},{{longitude}},{{mapThirdParty}});"  headercheckin="{{filterAddressString header}}" addresscheckin="{{filterAddressString address}}"></a>{{/root_Compare}}{{/root_Compare}}{{/root_Compare}}{{/if}}' +
    '{{#root_Compare @global.setting.dirMapDisplay "==" "1"}}{{#root_Compare @global.setting.dirDisableLocServices "==" "1"}} {{#if latitude}}<a href="#" class="icon-location-2" style="background-color:{{@global.styleAndNavigation.icon[0]}}; color:{{@global.styleAndNavigation.icon[1]}};" onclick="Appyscript.showServicePageMapNewservice(\'{{latitude}}\',\'{{longitude}}\',\'{{@index}}\',\'{{filterAddressString address}}\',\'{{filterAddressString header}}\',\'{{mapThirdParty}}\',\'{{@root.isFromPage}}\',this);"   headerval="{{filterAddressString header}}" addressval="{{filterAddressString address}}"></a>{{/if}}{{/root_Compare}}{{/root_Compare}}' +
    '{{callDir directoryInfo}}' +
    '{{#root_Compare @global.setting.listingShare "==" "1"}}<a href="#" class="icon-share-1" style="background-color:{{@global.styleAndNavigation.icon[0]}}; color:{{@global.styleAndNavigation.icon[1]}};" indexValue="{{@index}}" onclick="Appyscript.share_Img_Header_Description(this,\'{{getDirectoryUrl directoryInfo}}\',\'{{getCallData directoryInfo}}\',\'{{getEmailData directoryInfo}}\');"  images="{{#if mediaImageUrl}}{{mediaImageUrl[0]}} {{/if}}" header="{{#if header}}{{header}} {{/if}}" detailshare = "{{#if detail}} {{escape detail}} {{/if}}" summary="{{#if summary}}{{escape summary}} {{/if}}" summaryshare="{{#if summary}}{{escape summary}} {{/if}}" detail="" ></a>{{/root_Compare}}' +
    '{{#root_Compare @global.setting.reviewSetting "==" "1"}}<a href="#" class="rating-icon pageToPopup" style="background-color:{{@global.styleAndNavigation.icon[0]}}; color:{{@global.styleAndNavigation.icon[1]}};" onclick="Appyscript.directoryReviewRatingData(this,{{listId}},{{catId}},\'{{catName}}\')"  headervalreview="{{filterAddressString header}}" id="Ratng{{listId}}"><i class="iconz-star"></i>{{#if dirRating}}{{dirRating}}{{else}}0{{/if}}</a>{{/root_Compare}}{{#if dirCoupan}} <a href="#" class="coupan-icon iconz-gift pageToPopup" style="background-color:{{@global.styleAndNavigation.icon[0]}}; color:{{@global.styleAndNavigation.icon[1]}};" couponid="{{couponId}}" onclick="openCoupanPage(this)" onclick="Appyscript.showDataCoupon({{listId}},\'{{#if dirCoupan}}{{dirCoupan}} {{/if}}\',{{catId}},this)" ></a>{{/if}} {{#if loyaltyId}} <a href="#" class="appyicon-loyalty iconz-gift pageToPopup" style="background-color:{{@global.styleAndNavigation.icon[0]}}; color:{{@global.styleAndNavigation.icon[1]}};" loyaltyid="{{loyaltyId}}" onclick="openLoyaltyPage(this)"></a>{{/if}}' +
    '</span> {{/root_Compare}}' +
    '<img src="images/transparent-bg.png" data-listId="{{listId}}" data-claimStatus = "{{claimStatus}}" data-pageDataType="{{@root.isFromPage}}" data-heading="{{@root.catName}}" class="dirDetailService" style="background-image:url({{#if mediaImageUrl}}{{mediaImageUrl[0]}}{{else}}{{@global.setting.dirDefaultImg}} {{/if}});"> <div class="newloader" style=""></div>' +
    '</li>');
...
AppyTemplate.registerPartial('dirDetailListing', '<ul class="main-cat-listing">' +
    ...
    '<img src="images/transparent-bg.png" class="swiper-lazy"  onclick="Appyscript.detailSwaperImageOnclick(\'{{@global.setting.dirDefaultImg}}\',\'image\',\'{{../header}}\')" data-background="{{@global.setting.dirDefaultImg}}" \>' +
    ...
    ' {{#root_Compare @global.setting.displayQuickActionIcons "==" "1"}} <span class="hyper-addons">{{#root_Compare @global.setting.displayQuickActionIcons "==" "1"}}{{#if latitude}}{{#root_Compare @global.setting.dirMapDisplay "==" "1"}}{{#root_Compare @global.setting.checkonEnable "==" "1"}}{{#root_Compare @global.setting.dirDisableLocServices "==" "1"}}<a href="#" class="icon-ok-4" style="background-color:{{@global.styleAndNavigation.icon[0]}}; color:{{@global.styleAndNavigation.icon[1]}};" onclick="Appyscript.checkinPage(this,{{listId}},{{catId}},\'{{dirPageId}}\',{{latitude}},{{longitude}},{{mapThirdParty}},\'deatilPage\');" headercheckin="{{filterAddressString header}}" addresscheckin="{{filterAddressString address}}"></a>{{/root_Compare}}{{/root_Compare}}{{/root_Compare}}{{/if}}{{#root_Compare @global.setting.dirMapDisplay "==" "1"}}' +
    '{{#root_Compare @global.setting.dirDisableLocServices "==" "1"}} {{#if latitude}}<a href="#" id="99999" class="icon-location-2" style="background-color:{{@global.styleAndNavigation.icon[0]}}; color:{{@global.styleAndNavigation.icon[1]}};" onclick="Appyscript.openpopupmap(\'{{latitude}}\',\'{{longitude}}\',\'{{dirIndex}}\',\'{{address}}\',\'{{listId}}\',\'{{mapThirdParty}}\',\'deatilPage\',this);"  headersinglemap="{{filterAddressString header}}" addresssinglemap="{{filterAddressString address}}"></a>{{/if}}{{/root_Comapre}}{{/root_Compare}}' +
    '{{callDir directoryInfo}}' +
    '{{#root_Compare @global.setting.listingShare "==" "1"}}<a href="#" class="icon-share-1" style="background-color:{{@global.styleAndNavigation.icon[0]}}; color:{{@global.styleAndNavigation.icon[1]}};"  indexValue="{{@index}}" onclick="Appyscript.share_Img_Header_Description(this,\'{{getDirectoryUrl directoryInfo}}\',\'{{getCallData directoryInfo}}\',\'{{getEmailData directoryInfo}}\');"  images="{{#if mediaImageUrl}}{{mediaImageUrl[0]}} {{/if}}" header="{{#if header}}{{header}} {{/if}}" detailshare = "{{#if detail}} {{escape detail}} {{/if}}" summary="{{#if summary}}{{escape summary}} {{/if}}" summaryshare="{{#if summary}}{{escape summary}} {{/if}}" detail=""  ></a>{{/root_Compare}}{{#root_Compare @global.setting.reviewSetting "==" "1"}}<a href="#" class="rating-icon pageToPopup" style="background-color:{{@global.styleAndNavigation.icon[0]}}; color:{{@global.styleAndNavigation.icon[1]}};" onclick="Appyscript.directoryReviewRatingData(this,{{listId}},{{catId}},\'{{catName}}\')"  headervalreview="{{filterAddressString header}}"><i class="iconz-star"></i>{{#if dirRating}}{{dirRating}}{{else}}0{{/if}}</a>{{/root_Compare}}{{#if dirCoupan}} <a href="#" class="coupan-icon iconz-gift pageToPopup" style="background-color:{{@global.styleAndNavigation.icon[0]}}; color:{{@global.styleAndNavigation.icon[1]}};" couponid="{{couponId}}" onclick="openCoupanPage(this)" onclick="Appyscript.showDataCoupon({{listId}},\'{{#if dirCoupan}}{{dirCoupan}} {{/if}}\',{{catId}},this)" ></a>{{/if}} {{#if loyaltyId}} <a href="#" class="appyicon-loyalty iconz-gift pageToPopup" style="background-color:{{@global.styleAndNavigation.icon[0]}}; color:{{@global.styleAndNavigation.icon[1]}};" loyaltyid="{{loyaltyId}}" onclick="openLoyaltyPage(this)"></a>{{/if}}{{/root_Compare}}</span> {{/root_Compare}}' +
    ' </li>' +
    '</ul>' +
    '<ul class="detailInfolist">{{#root_Compare @global.setting.dirSummaryListingDetailsPage "==" "1"}}' +
    '{{#if summary}}<li enableIcon="{{enableAddressIcon}}" style="border-color:{{@global.styleAndNavigation.borderColor}}"><div class="runing_txt {{@global.styleAndNavigation.content[0]}} {{@global.styleAndNavigation.content[1]}}" style="color:{{@global.styleAndNavigation.content[2]}}">' +
    '{{summary}}' +
    '</div></li>{{/if}}' +
    '{{/root_Compare}}' +
    '{{#if detail}}<li enableIcon="{{enableAddressIcon}}" style="border-color:{{@global.styleAndNavigation.borderColor}}"><div id="descService" class="runing_txt {{@global.styleAndNavigation.content[0]}} {{@global.styleAndNavigation.content[1]}}   {{#root_Compare @global.setting.dirAbstractListingInfo "==" "1"}} type-content  {{/root_Compare}}" style="color:{{@global.styleAndNavigation.content[2]}}; width:100%">' +
    '{{detail}}' +
    '</div> {{#root_Compare @global.setting.dirAbstractListingInfo "==" "1"}} <a class="type-button" onClick="showlessdirecttorytext(this)"  style="color:{{@global.styleAndNavigation.activeColor}}">{{@global.languageSetting.more}}</a> {{/root_Compare}}</li>{{/if}}' +
    '</ul>' +
    '<ul class="detailInfolist">' +
    '{{#root_Compare @global.setting.reviewSetting "==" "1"}}' +
    '{{#if dirRating}}' +
    '<li enableIcon="{{enableAddressIcon}}" style="border-color:{{@global.styleAndNavigation.borderColor}}">' +
    '<i class="iconz-star" style="color:{{@global.styleAndNavigation.icon[1]}}"></i><div class="innerlist">' +
    '<div onclick="Appyscript.directoryReviewRatingData(this,{{listId}},{{catId}},\'{{catName}}\')"  headervalreview="{{filterAddressString header}}"><p class="{{@global.styleAndNavigation.subheading[0]}} {{@global.styleAndNavigation.subheading[1]}}" style="color:{{@global.styleAndNavigation.subheading[2]}}"><strong>{{@global.languageSetting.user_reviews_ratings_dir}}</strong></p> <p><e class="{{@global.styleAndNavigation.subheading[0]}} {{@global.styleAndNavigation.subheading[1]}}" style="color:{{@global.styleAndNavigation.subheading[2]}}">{{@global.languageSetting.rating_dir}}: </e><e class="blueColor {{@global.styleAndNavigation.content[0]}} {{@global.styleAndNavigation.content[1]}}" style="color:{{@global.styleAndNavigation.content[2]}}"><strong  class="blueColor {{@global.styleAndNavigation.content[0]}} {{@global.styleAndNavigation.content[1]}}" style="color:{{@global.styleAndNavigation.content[2]}}">{{dirRating}} {{@global.languageSetting.fromDir}} 0-5</strong></e></p><p><e class="{{@global.styleAndNavigation.subheading[0]}} {{@global.styleAndNavigation.subheading[1]}}" style="color:{{@global.styleAndNavigation.subheading[2]}}">{{@global.languageSetting.reviews_Dir}}: </e><strong  class="blueColor {{@global.styleAndNavigation.content[0]}} {{@global.styleAndNavigation.content[1]}}" style="color:{{@global.styleAndNavigation.content[2]}}">{{totalReview}}</strong></p> </div></div></li>' +
    '{{else}}' +
    '<li enableIcon="{{enableAddressIcon}}" style="border-color:{{@global.styleAndNavigation.borderColor}}">' +
    ...
    '<div > <p class="{{@global.styleAndNavigation.subheading[0]}} {{@global.styleAndNavigation.subheading[1]}}" style="color:{{@global.styleAndNavigation.subheading[2]}}"><strong>{{@global.languageSetting.user_reviews_ratings_dir}}</strong></p> <p><e class="{{@global.styleAndNavigation.subheading[0]}} {{@global.styleAndNavigation.subheading[1]}}" style="color:{{@global.styleAndNavigation.subheading[2]}}">{{@global.languageSetting.rating_dir}}: </e><e class="blueColor {{@global.styleAndNavigation.content[0]}} {{@global.styleAndNavigation.content[1]}}" style="color:{{@global.styleAndNavigation.content[2]}}"><strong  class="blueColor {{@global.styleAndNavigation.content[0]}} {{@global.styleAndNavigation.content[1]}}" style="color:{{@global.styleAndNavigation.content[2]}}"><!--0 {{@global.languageSetting.fromDir}} -->0/5</strong></e></p><p><e class="{{@global.styleAndNavigation.subheading[0]}} {{@global.styleAndNavigation.subheading[1]}}" style="color:{{@global.styleAndNavigation.subheading[2]}}">{{@global.languageSetting.reviews_Dir}}: </e><strong  class="blueColor {{@global.styleAndNavigation.content[0]}} {{@global.styleAndNavigation.content[1]}}" style="color:{{@global.styleAndNavigation.content[2]}}">{{@global.pageLanguageSetting.no_review_available}}  </strong></p> </div></div></li>' +
    ' {{/if}}' +
    '{{/root_Compare}}' +
    ' {{#directoryInfo}}' +
    ' {{#if value}}' +
    ' <li enableIcon="{{enableIcon}}" style="border-color:{{@global.styleAndNavigation.borderColor}}">' +
    ...
    '{{#js_compare "this.type === \'deepLink\' "}}' +
        '<div class="innerlist" onclick="Appyscript.openDeeplinkDetailPage(\'{{value}}\')" >' +

        '<span class="{{@global.styleAndNavigation.subheading[0]}} {{@global.styleAndNavigation.subheading[1]}}" style="color:{{@global.styleAndNavigation.subheading[2]}}; display: block;">{{label}} </span>' +
        '</div>' +
    ...
    ' {{/directoryInfo}}' +
    '{{#if dirCoupan}}<li enableIcon="{{enableAddressIcon}}" style="border-color:{{@global.styleAndNavigation.borderColor}}">' +
    ...
    '</div>' +
    ' </li>{{/if}}' +
    '{{#if loyaltyId}}<li enableIcon="{{enableAddressIcon}}" style="border-color:{{@global.styleAndNavigation.borderColor}}">' +
        '<i class="appyicon-loyalty" style="color:{{@global.styleAndNavigation.icon[1]}}"></i><div class="innerlist">' +
        '<a loyaltyid="{{loyaltyId}}" onclick="openLoyaltyPage(this)" class="{{@global.styleAndNavigation.content[0]}} {{@global.styleAndNavigation.content[1]}}" style="color:{{@global.styleAndNavigation.content[2]}}">{{@global.global_view_loyalty}}</a>' +
        '</div>' +
     ' </li>{{/if}}' +
    ' {{#pdfUrl}}' +
    '<li enableIcon="{{enableAddressIcon}}" style="border-color:{{@global.styleAndNavigation.borderColor}}"  {{#if @global.setting.dirOpenPdfBrowser}} onclick="showlinkiappsystem(\'{{value}}\')" {{else}} onclick="Appyscript.checksinHeaderForWebView(this)" {{/if}} value="{{value}}" header="{{../../../header}}">' +
    ...
    '{{#if trackStreamUrl}}' +
    '<li enableIcon="{{enableAddressIcon}}" style="border-color:{{@global.styleAndNavigation.borderColor}}"><i class="iconz-star" style="color:{{@global.styleAndNavigation.icon[1]}}"></i><div class="innerlist"><a onclick="Appyscript.dirAudioPlay(\'{{trackStreamUrl}}\');" class="{{@global.styleAndNavigation.content[0]}} {{@global.styleAndNavigation.content[1]}}" style="color:{{@global.styleAndNavigation.content[2]}}">' + '{{header}}</a>' +
    '</li>{{/if}}' +
    ...
    ' <div class="innerlist" {{#root_Compare @global.setting.dirMapDisplay "==" "1"}} onclick="Appyscript.openpopupmap(\'{{latitude}}\',\'{{longitude}}\',\'{{dirIndex}}\',\'{{filterAddressString address}}\',\'{{listId}}\',\'{{mapThirdParty}}\',\'deatilPage\',this);"{{/root_Compare}}><span class="{{@global.styleAndNavigation.subheading[0]}} {{@global.styleAndNavigation.subheading[1]}}" style="color:{{@global.styleAndNavigation.subheading[2]}}">{{@global.pageLanguageSetting.address_dir}}: </span><a {{#root_Compare @global.setting.dirMapDisplay "==" "1"}} {{/root_Compare}}  headersinglemap="{{filterAddressString header}}" addresssinglemap="{{filterAddressString address}}" class="{{@global.styleAndNavigation.content[0]}} {{@global.styleAndNavigation.content[1]}}" style="color:{{@global.styleAndNavigation.content[2]}}">' +
    ...
    '{{#root_Compare @global.setting.dirMapDisplay "==" "1"}}' +
    ' {{#if latitude}} {{#root_Compare @global.setting.dirDisableLocServices "==" "1"}}<li class="swiper-no-swiping"  style="border-color:{{@global.styleAndNavigation.borderColor}}"><div id="googleMap{{listId}}" style="width:100%;height:200px;" ></div></li>{{/root_Compare}}{{/if}}' +
    '{{/root_Compare}}' +
    ' {{/if}}' +
    ' </ul>' +
    '</li>' +
    ' </ul>' +
    ' <div id="claimform" style="visibility:hidden" class="hyper-gray-wrapper"><button class=" hyper-sent-btn popupToPage {{@global.styleAndNavigation.button[0]}} {{@global.styleAndNavigation.button[1]}}" style="background-color: {{@global.styleAndNavigation.button[2]}}; color: {{@global.styleAndNavigation.button[3]}};" data-animate="true"  data-back="back"  onclick="Appyscript.openClaimForm({{listId}},{{catId}},\'{{categoryName}}\',\'{{filterAddressString header}}\')">{{@global.global_view_claim}}</button></div>' +
    '{{#js_compare "this.formType != \'noform\' "}}<div id="selectform" class="hyper-gray-wrapper">{{#root_Compare @global.setting.formSetting "==" "1"}}<button class=" hyper-sent-btn {{@global.styleAndNavigation.button[0]}} {{@global.styleAndNavigation.button[1]}}" style="background-color: {{@global.styleAndNavigation.button[2]}}; color: {{@global.styleAndNavigation.button[3]}};" data-animate="true"  data-back="back"  onclick="Appyscript.openRequestFormAndFormBuilder(\'{{#if formType}}{{formType}}{{/if}}\',\'{{#if formBuilderForm}}{{formBuilderForm}}{{/if}}\',\'{{#if formBuilderPageId}}{{formBuilderPageId}}{{/if}}\',{{listId}},{{catId}},\'{{#if dirPageId}}{{dirPageId}}{{/if}}\',\'{{#if header}}{{filterAddressString header}}{{/if}}\',\'{{#if listOwner}}{{listOwner}}{{/if}}\')">{{#if formName}}{{formName}}{{else}}{{@global.languageSetting.send_request}}{{/if}}</button>{{/root_Compare}}</div>{{/js_compare}}');
...
Appyscript.createDirlisting = function(a, subcatId, subcatname, sortByDirName) {
    ...
    console.log("Dharam========" + index);
       ...
       subcatIdvalue=subcatId;
        ...
        if(pageIdentifie)
         {
        pageIdentifie_val=pageIdentifie;
          }
        else{
          pageIdentifie=pageIdentifie_val;
        }
...
Appyscript.createDirDetailService = function() {
    ...
    for (var i = 0; i < pageDataDetail.listing.list.length; i++) {
     ...
     if(pageDataDetail.listing)
               {
               if(pageDataDetail.listing.list)
               {
               AppyTemplate.global.servicelistid= pageDataDetail.listing.list[0].listId;
               }
               }
               AppyTemplate.global.Pagename=pageData.pageTitle;
    console.log("pageDataDetail===   "+ JSON.stringify(pageDataDetail));
    $$.get("pages/services-Detail.html", function(template) {
        ...
        if(pageDataDetail.listing.list[index].adBanner && pageData.setting.dirAdBanner == "1"){
            ...
            var addBannerId = pageDataDetail.listing.list[index].adBanner
                             if ( Object.keys(addBannerId).length != 0) {
        ...
        localStorage.setItem("dirIndex", index);
        if (index > 0) {
...
function openDirectoryDetailWithIndex(index1, isFrom, reload) {
    ...
    if(listindexval && pageData.setting.defaultListingView != "map" && pageData.setting.isMapData)
    {
       index = listindexval;
    }
    else{
    ...
    if (isFrom == "pageDataList") {
        ...
        if(pageDataSubCatList){
            pageDataDetail = pageDataSubCatList;
            }
            else{
            pageDataDetail=pageDataDetail;
            }
        /*if(pageDataSubCatList.subCat.list)
        {
            index=index-pageDataSubCatList.subCat.list.length;
    ...
    } else if(isFrom == "pageDataListDetails"){
        pageDataDetail = pageDataDetail;
    }else if(isFrom == "" && pageData.categoryList.list.length==1){
       pageDataDetail = pageData.sigleCategoryList;
    }
           if(pageDataDetail){
           if(pageDataDetail.listing)
           {
...
function mapinitialize() {
    ...
    if ((parseFloat(serlatitude)) && pageData.setting.dirMapDisplay == 1) {
        ...
        listindexval=index;
        console.log("listindexval1234" + listindexval)
        if (header.indexOf("&") != "-1") {
        ...
        setTimeout(function(){
            var gid = 'googleMap' + listId;
             AppyTemplate.global.servicelistid=listId;
            console.log("gid========" + gid);
        ...
        $$.ajax({
            ...
            success: function(jsonData, textStatus) {
                ...
                $$.get("pages/services-listing.html", function(template) {
                    ...
                    if (mainView.activePage.name == "services-listingPage" ) {
                        mainView.router.reloadContent(html);
                    } else {
                        mainView.router.load({
                            content: html,
...
Appyscript.searchAndFilterData = function(istype, searchfrom) {
    sliderMaxValue = pageData.setting.defaultRengeSearch;
    distanceUnits = pageData.setting.defaultDistance;
    var activeTab = $$(".listing-tabs a.active").attr("value");
    if (istype=='textSearch' && (filterType=="distance" || filterType=="ratting"))
    {
    istype = 'filter'
    }
    ...
    if (activeTab == "Distance") {
        ...
        if (istype=='textSearch')
        {
        filterType = "textSearch";
        }

        console.log("filter ======== "+istype)
        ...
        if (searchfrom == 'mainpagesearch') {
            ...
            searchText = document.getElementById('txtSearchsub').value;
    ...
    console.log("rating or distance filter added ======= "+filterType)
   ...
   if(filterType=="distance")
    {
       if (pageData.setting.dirDisableLocServices == "0" || pageData.setting.hideFilter == "1") {
          //  Appyscript.filterSearch("textSearch", sliderMinValue, sliderMaxValue, distanceUnits, catidvalue, rCountWithComma, lattitude, longitude, searchText, searchfrom);
            Appyscript.filterSearch("textSearch", sliderMinValue, sliderMaxValue, distanceUnits, catidvalue, rCountWithComma, lattitude, longitude, searchText, searchfrom);
        }
        else
        {
...
function addItemAlertDir(address, lattitude, longitude) {
    ...
    var sliderMaxValue = pageData.setting.defaultRengeSearch;
    var selChekId = "";
    var rCountWithComma = "";
    var searchText = "";
    setTimeout(function() {
...
Appyscript.directorySaveDeleteBookMark = function(a, listid, isFrom) {
    ...
    if(localStorage.getItem("couponDir") == "directoryCoupon"){
        pageIdentifie = couponDetailDirectory[1];
    }
    if (localStorage.getItem('bookmark_' + pageIdentifie) == null || localStorage.getItem('bookmark_' + pageIdentifie) == "") {
        localStorage.setItem('bookmark_' + pageIdentifie, JSON.stringify(bookmarkJsonData));
    }
...
var bookmarkDetails =[];
     ...
     if (pageDataList.listing) {
            ...
            bookmarkDetails[i] = pageDataList.listing.list[i].listId;

                }
            }

    bookmarkDetails = bookmarkDetails.toString();

    console.log("Check Bookmark",bookmarkDetails);
         if(isOnline())
         {
         var  postData= '{"method":"directoryBookMark","appId":"' + localStorage.getItem("appid") + '","dirPageId":"' + pageIdentifie + '","pageId":"'+bookmarkDetails+'","lang":"'+data.appData.lang+'"}';
         ...
         $$.ajax({
                 ...
                 pageDataList.listing = "";

                 console.log("Fialure in Bookmark")
...
//        Appyscript.openMapViewGoogle(address, latitude, longitude, header);
        Appyscript.modal({
                ...
                buttons:[{
                    ...
                    text:pageData.languageSetting.map,       onClick: function(){Appyscript.openMapViewGoogle(address, latitude, longitude, header)}
                },
                {
                    text: pageData.languageSetting.cancel,
                    onClick: function () {}
...
//        AppyPieNative.showServicePageMapNew(dirData);
           Appyscript.modal({
                        ...
                        buttons:[{
                            ...
                            text:pageData.languageSetting.map,       onClick: function(){AppyPieNative.showServicePageMapNew(dirData)}
                        },
                        {
                                text: pageData.languageSetting.cancel,
                                             onClick: function () {}
...
Appyscript.openMenuPage = function() {
    ...
    var _getEmail=localStorage.getItem("email") || "" ;
    if(_getEmail!=""){
...
Appyscript.addListingData = function(inapptrnsactionid, statusval) {
    ...
    if (pageData.paymentMethod == "iap") {
        ...
        //showPaymentAlertservice(pageData.oneTimeSubscriptionPrice, pageData.oneTimeSuFbscriptionCurrency, pageData.inAppAndroidBundleId, pageData.inAppPublicKey);
       //showPaymentAlertserviceAll(pageData);
        checkDirectoryIAPPayment();
        return;

    }
    else{
    Appyscript.addListingDirData();
    }
    }


Appyscript.addListingDirData = function(inapptrnsactionid, statusval) {
...
function addListingClick(type, listId) {
   ...
   var urlAddListLabaelData = '';
    ...
    if(pageData.languageSetting.Others){
    otherLabel = pageData.languageSetting.Others;
    }else{
    otherLabel ="Others"
    }
            ...
            if (Appyscript.device.android) {
                ...
                if (type == "add") {
                    ...
                    Appyscript.uploadMultipleFilesDirectory(catIdSelectedStr, heading, summary, body, emailLabelSend, emailDataSend, urlLabaelSend, urlDataSend,
                        ...
                        listId, imageFinalUrlArray.toString(), lat, long, JSON.stringify(pdfJson),coupanIdSelected,otherLabel, others);

                } else {

                    var bodyImageFinalArray = new Array();
                    var ImageFinalArray = new Array();
                    var othersUpdate = document.getElementById('othersUpdate').value;
                    ...
                    Appyscript.uploadMultipleFilesDirectory(catIdSelectedStr, heading, summary, body, emailLabelSend, emailDataSend, urlLabaelSend, urlDataSend,
                        ...
                        listId, ImageFinalArray.toString(), lat, long, JSON.stringify(pdfJson),'',otherLabel, othersUpdate);
...
//    navigator.device.capture.captureImage(captureSuccessDirectory, serviceCaptureErrorDirectory, options);
    navigator.camera.getPicture(captureSuccessDirectory, serviceCaptureErrorDirectory, {
            quality: 70,
...
function librarySuccessDir(imageURI) {
    ...
    $$(addImgA).parent().append('<span><img src="' + imageData + '" id="imageSL' + addImgIndex + '" onclick=""><a onclick="removeThis(this,' + addImgIndex + ')">X</a></span><span class="add-more" onclick="addMoreClick2(this,' + indexval + ')">+</span>');
    $$(addImgA).remove();
                ...
                setTimeout(function() {
                    ...
                    for (var k = 0; k < directoryInfoArray.length; k++) {
                        ...
                        if (directoryInfoArray[k].type == "email") {
                            ...
                            if(directoryInfoArray[k].type == "otherInfo"){
                            document.getElementById("othersUpdate").value =directoryInfoArray[k].value;
                            }
                    ...
                    for (var m = 0; m < serviceImageArray.length; m++) {
                        ...
                        var indexval = m + 1;
                        $$('.items-scroller .items').append('<span><img src="' + serviceImageArray[m] + '" id="imageSL' + imgAppendIndex + '" onclick=""><a onclick="removeThis(this,' + imgAppendIndex + ')">X</a></span>');
...
function openCoupanPage(couponId) {
    var query = webserviceUrl + 'Coupon.php';
...
function coupanRedeemValue() {
        ...
        if (couponserch == couponCode) {
            ...
            var baseurl = webserviceUrl + 'Coupon.php';
...
function coupanRedeemShowMessage() {
    ...
    var reedmConfirmation = pageData.languageSetting.redeem_confirmation_common;
...
function openLoyaltyPage(loyaltyId) {
    var query = webserviceUrl + 'loyality.php';
        ...
        if (getDate.toString().length > 1) {
            ...
            if (d2 <= d3 && d2 >= d1) {
                ...
                if (localStorage.getItem("checkidentifier", pageIdentifie) != pageIdentifie) {
                    ...
                    loyaltypage.push({
                        invoiceID: "Invoice",
                        invoiceImage: "https://zhenglab.ucr.edu/images/generic_woman.gif",
                        invoiceDate: "Date",
                        cardNo: "0"
                    });
...
function serviceQrScanner(a) {
    ...
    if (pageDataServiceLoyalty.data.validationPinType == "uniquecode") {
        ...
        var wsUrl = webserviceUrl + "loyality.php";
        Appyscript.showIndicator();
        $$.ajax({
            url: wsUrl,
...
//    navigator.device.capture.captureImage(serviceCaptureSuccessDirectoryy, serviceCaptureErrorDirectory, options);
    Appyscript.showIndicator();
    navigator.camera.getPicture(serviceCaptureSuccessDirectoryy, serviceCaptureErrorDirectory, {
        quality: 70,
        destinationType: Camera.DestinationType.FILE_URI,
        sourceType: Camera.PictureSourceType.CAMERA,
        encodingType: Camera.EncodingType.JPEG,
...
function serviceCaptureSuccessDirectoryy(imageURI) {
    ...
    setTimeout(function() {
        ...
        console.log("wkehjerfjkr" + imageURI);
        serviceGetImageiOSAndAnd(fromloyality);
    }, 3000);
    if (Appyscript.device.android) {
        console.log(mediaFiles);
        imageData = mediaFiles;
...
//Appyscript.onPageBack('services-CardPage', function(page) {
//    AppyTemplate.global.styleAndNavigation =  pageData.styleAndNavigation;
//    openDirectoryDetailWithIndex(0, "sublistList", 1);
//});
...
function showAdBanner(banner, isFeatured){
    ...
    var _banner = banner.banner.split("://");
    if(_banner.length>1 && _banner[0] == "http"){
    banner.banner = "https://"+_banner[1];
    }
    console.log("bAnner11"+banner.banner);
...
// navigator.device.capture.captureImage(fromCamera, errImageUpload, options);
    navigator.camera.getPicture(fromCamera, errImageUpload, {
            quality: 70,
            destinationType: Camera.DestinationType.FILE_URI,
            sourceType: Camera.PictureSourceType.CAMERA,
            encodingType: Camera.EncodingType.JPEG,
...
function getFileFromUrl(url, flag){
 ...
 return new Promise(function (resolve, reject) {
                    ...
                    window.resolveLocalFileSystemURL("file://" + url, gotFile, fail);
                    }
...
function dir_createCoupon() {
 ...
 /* savings end here */
    ...
    if (validType == "redeem") {
                if(c_pId != 3)
                      validationObj.validation_process = "2";
                      else
                      validationObj.validation_process = "0";
                }
    var vData = {};
...
function dir_postCoupon(fd, from) {
    ...
    xhr.open("POST", webserviceUrl + 'Coupon.php');
    xhr.setRequestHeader("accessToken", deviceEncryptedToken);
...
/*****************************************/
//OPEN DEEP LINK Details Page
/*****************************************/
var lang;
var deeplinkVal;
var couponDetail;
var couponDetailPage;
var serviceHeading;
Appyscript.openDeeplinkDetailPage = function(value) {
    ...
    lang = data.appData.lang;
    deeplinkVal = value;
    couponDetail = deeplinkVal.split("#");
    if(couponDetail.length>1)
    {
    couponDetailPage = couponDetail[1].split("_");
    console.log("  bbbb "+couponDetail[2] + "  ddd  "+couponDetailPage[0]);
    }
    else{

    couponDetailPage= couponDetail[0];
    }
    if(couponDetailPage==deeplinkVal){
       opendeeplinkpage(value);
       return;
    }
    else{
    localStorage.setItem("uniquecodedata",couponDetail[2]);
    var jsonPostecom = '{"method":"getListingWithPageData","appId":"' + localStorage.getItem("appid") + '","couponDirectoryPageId":"' + couponDetail[1] + '","couponId":"' + couponDetail[2] + '","lang":"' +lang+ '","deviceId":"' + coupondeviceid + '"}';
    console.log("jsonPostecom " + jsonPostecom + " ghg  " + site_url + '/webservices/Directory.php');
    Appyscript.showIndicator();

    $$.ajax({
        url: webserviceUrl + 'Directory.php',
        data: Appyscript.validateJSONData(jsonPostecom),
        type: "post",
       //321  headers: {'accessToken': deviceEncryptedToken},
        success: function(jsonData, textStatus) {
            responcedata = JSON.parse(jsonData);
            AppyTemplate.global.pageLanguageSetting = responcedata.pageData.languageSetting;
            AppyTemplate.global.styleAndNavigation = responcedata.pageData.styleAndNavigation;
            console.log("responcedata::::: " + JSON.stringify(responcedata.pageData));
            Appyscript.hideIndicator();
            if(responcedata.pageData.couponDetails.length){
                responcedata.pageData.couponDetails[0].defaultCurrency = responcedata.pageData.setting.couponDirectoryDefaultCurrency;
                responcedata.pageData.couponDetails[0].title=responcedata.pageData.couponDetails[0].heading;
                serviceHeading=responcedata.pageData.couponDetails[0].heading;
                couponredeem = responcedata.pageData.couponDetails[0].couponRedeem;
                coupon_type = responcedata.pageData.couponDetails[0].couponType;
                pageDataService.title=pageDataService.heading;
                couponValidation = responcedata.pageData.couponDetails[0].validationProcess.validation;
                couponredeemcdir = responcedata.pageData.couponDetails[0].couponRedeem;
                scratchoff =  responcedata.pageData.couponDetails[0].scratchOff;
                expiredcdir = responcedata.pageData.couponDetails[0].couponExpire;
                couponValidTocdir = responcedata.pageData.couponDetails[0].validDate;
                couponValidFromcdir = responcedata.pageData.couponDetails[0].issueDate;
                couponValidTo = responcedata.pageData.couponDetails[0].validDate;
                couponValidFrom = responcedata.pageData.couponDetails[0].issueDate;
                coupon_scratch_textcdir =  responcedata.pageData.couponDetails[0].withdraw.scratch_text;
                coupon_unluckycdir = responcedata.pageData.couponDetails[0].withdraw.coupon_unlucky;
                scratchImageGlobalcdir =  responcedata.pageData.couponDetails[0].withdraw.image;
                couponHeading = responcedata.pageData.couponDetails[0].heading;
                brief_desc = responcedata.pageData.couponDetails[0].briefDescription;
                couponCode = responcedata.pageData.couponDetails[0].code;
                couponBgImaged = responcedata.pageData.couponDetails[0].coverImage;
                couponCodecdir = responcedata.pageData.couponDetails[0].code;
                ...
                $$.get("pages/coupondirectorydeeplink-detail.html", function(template) {
                    var compiledTemplate = AppyTemplate.compile(template);
                    var html = compiledTemplate(responcedata.pageData.couponDetails[0]);
                    mainView.router.load({
                        content: html,
                        animatePages: true
                    });
                    $$('#pagesCSS').attr('href', 'css/' + couponDetailPage[0] + '.css');
                });
            }else{
                 Appyscript.alert(responcedata.pageData.languageSetting.coupon_invalid, appnameglobal_allpages);
            }
...
Appyscript.onPageBack('coupondirectory-detailcoupondeeplink', function(page) {
        AppyTemplate.global.styleAndNavigation =  pageData.styleAndNavigation;
        openDirectoryDetailWithIndex(localStorage.getItem("dirIndex"), "pageDataListDetails", 1);
});

Appyscript.onPageInit('coupondirectory-detailcoupondeeplink', function(page) {
    handleWebcdir = true;
    if (couponredeem == 1 || expired == 1) {
        $$(".couponCode3").css("visibility", "hidden");
    } else {
...
/*****************************************************/
//Coupon DEEP Link With Direcory
/*****************************************************/

function claimButtonShowHideCoupon() {
    console.log("claimButtonShowHideCoupon  " + pageData.setting.dirShowHideClaim+"    claimStatusss    "+ pageData.listing.list[0].claimStatus)
    if (pageData.setting.dirShowHideClaim == "1" && pageData.listing.list[0].claimStatus == "1") {
        $$("[id='claimform']").show();
    } else {
        $$("[id='claimform']").hide();
    }
...
var listindexval ;
function mapinitializeCoupon() {
    var serlatitude = $$(".swiper-services .service-slide").attr("data-lattitude");
    if ((parseFloat(serlatitude)) && pageData.setting.dirMapDisplay == 1) {
        var serlongitude = $$(".swiper-services .service-slide").attr("data-longitude");
        var listId = $$(".swiper-services .service-slide").attr("data-id");
        var address = $$(".swiper-services .service-slide").attr("data-address");
        var mapThirdParty = $$(".swiper-services .service-slide").attr("data-mapThirdParty");
        var index = $$(".swiper-services .service-slide").attr("data-index");
        var header = AppyTemplate.global.headrtitle;
        var header = AppyTemplate.global.headrtitle;
        listindexval=index;
        console.log("listindel1234=====  " + listindexval);
        setTimeout(function(){
            var gid = 'googleMap' + listId;
             AppyTemplate.global.servicelis
             tid=listId;
            console.log("gid========  " + gid);
            var mapDiv = document.getElementById(gid);
            var map = new google.maps.Map(mapDiv,{
...
Appyscript.onPageBack('services-DetailPagecoupon', function(page) {
    pageData = pageDataCouponDir;
    AppyTemplate.global.styleAndNavigation =  pageData.styleAndNavigation;
    $$('#pagesCSS').attr('href', 'css/coupondirectory.css');
});
...
function checkForm(index){
 ...
 function checkDirectoryIAPPayment()
 {
 ...
 function uploadDirectoryTransectionDetailAfterPayment(transactionId,pageTypeid, productId,paymentTypeIap,subscriptionPrice,subscriptionCurrency)
 {
 ...
 function callbackAfterPaymentOnDirectory(status, transactionId, productId, payType,pageTypeid,paymentTypeIap,subscriptionPrice,subscriptionCurrency )
 {
     ...
     if(status=="success")
     {
       ...
       Appyscript.showIndicator();
          ...
          else
          {
           ...
           uploadDirectoryTransectionDetailAfterPayment(transactionId,pageTypeid, productId,paymentTypeIap,subscriptionPrice,subscriptionCurrency);
          }
     }
 ...
 // function call for geo fencing event to open directory
function openDirectoryListingDetail(dirPageId1, listId) //dirPageId1,listId
{
    console.log("dirpageId : ");

    var baseurl =  webserviceUrl + "Directory.php"
    var postDataGeo = '{"method":"getDirectoryListingDetails","appId":"' + data.appData.appId + '","dirPageId":"' + dirPageId1 + '","listId":"' + listId + '"}'; //dirPageId1 5c07654b8b03bea564b2228a
    if (isOnline()) {
        Appyscript.showIndicator();

        $$.ajax({
            url: baseurl,
            data: Appyscript.validateJSONData(postDataGeo),
            type: "post",
            async: true,
            //321  headers: {'accessToken': deviceEncryptedToken},
            success: function(jsonData, textStatus) {
                var jsonData = JSON.parse(jsonData);
                console.log("posted successfully**+++++*** " + JSON.stringify(jsonData));
                console.log("list     "+jsonData.listing.list.length)
                if (jsonData.listing.list.length > 0) {
                    setTimeout(function() {
                        geoFenceMatchhitDeatil(jsonData);
                        Appyscript.hideIndicator();
                    }, 2000);
 ...
 function geoFenceMatchhitDeatil(dataRecieved)
 {

 AppyTemplate.global.pageLanguageSetting = dataRecieved.dirPageData.languageSetting;
 AppyTemplate.global.styleAndNavigation = dataRecieved.dirPageData.styleAndNavigation;
 AppyTemplate.global.setting = dataRecieved.dirPageData.setting;
 AppyTemplate.global.global_view_coupon = dataRecieved.dirPageData.languageSetting.view_coupon;
 AppyTemplate.global.global_view_loyalty = dataRecieved.dirPageData.languageSetting.dir_loyalty_card_click;
 AppyTemplate.global.global_view_claim = dataRecieved.dirPageData.languageSetting.claim;

 pageData = dataRecieved.dirPageData
 pageDataDetail = new Object();
 console.log("hhhh  "+dataRecieved.dirPageData.sigleCategoryList)
 console.log("hhh********h  "+dataRecieved.dirPageData.sigleCategoryList.length)
 if(dataRecieved.dirPageData.sigleCategoryList.length){
     //pageDataDetail.catName = dataRecieved.dirPageData.sigleCategoryList.catName;
     pageDataDetail.subCat =dataRecieved.dirPageData.sigleCategoryList.subCat;
 }

 pageDataDetail.catName = dataRecieved.catName;
 pageDataDetail.setting = dataRecieved.dirPageData.setting;
 pageDataDetail.languageSetting = dataRecieved.dirPageData.languageSetting;
 pageDataDetail.listing = dataRecieved.listing;
 pageDataDetail.layout = 1;

 if(pageDataDetail.listing.list[0].header != undefined){
    pageDataDetail.listing.list[0].header = pageDataDetail.listing.list[0].header.replace(/ /g, '\u00a0');
 }
 AppyTemplate.global.headrtitle = pageDataDetail.listing.list[0].header;
 index = 0;
 claimStatus = pageDataDetail.listing.list[0].claimStatus;
 claimlistId = pageDataDetail.listing.list[0].listId;
 AppyTemplate.global.header = pageDataDetail.listing.list[0].header;
 pageDataDetail.header = pageDataDetail.listing.list[0].header;
 if(pageDataDetail.listing.list[0].address != undefined){
    pageDataDetail.listing.list[0].address = pageDataDetail.listing.list[0].address.replace(/ /g, '\u00a0');
 }
 ...
 Appyscript.dirAudioPlay = function(trackURL){
 var jsonString="@@@@"+pageDataDetail.catName+"$$@@$$"+" $$@@$$"+trackURL+ "$$@@$$" + "";
 if (Appyscript.device.android) {
 jsonString = "@@--@@" + pageDataDetail.catName + "#####" + trackURL + "#####" + "";
 } else {
 jsonString = "@@@@" + pageDataDetail.catName + "$$@@$$" + "" + "$$@@$$" + trackURL + "$$@@$$" + "";
 }

 Appyscript.playAudioUrls(jsonString,"1","customPlayList","","","",pageDataDetail.catName);
 }


AppyTemplate.registerHelper('headerHelper', function(catNameChk) {
                                      var cName = catNameChk.replace("\""," ");
                                                 return cName;
                                                               });

AppyTemplate.registerHelper('getEmailData', function(directoryInfo) {
    ...
    if (directoryInfo) {
        for (var i = 0; i < directoryInfo.length; i++) {
             ...
             if (directoryInfo[i].type == "email") {
                            if (email) {
                                email = email + "  " + directoryInfo[i].value;
                            } else {
                                email = directoryInfo[i].value;
                            }
                        }
...
AppyTemplate.registerHelper('getCallData', function(directoryInfo) {
    var phoneArr = "";

    if (directoryInfo) {
        for (var i = 0; i < directoryInfo.length; i++) {
            if (directoryInfo[i].type == "call") {
                if (phoneArr) {
                    phoneArr = phoneArr + "  " + directoryInfo[i].value;
                } else {
                    phoneArr = directoryInfo[i].value;
                }
            }
...
function openQRCodeService() {
    var fromPage = 'services';
    AppyPieNative.validateLoyaltyViaScanner(fromPage, pageData.pageTitle);
}

function qrScannerServiceSuccess(value){
        let qrCodeValue  = value.split(':');
        qrCodeValue = qrCodeValue[1];
        let userIdValue = localStorage.getItem("userid") || ""
        if (qrCodeValue.trim() != couponCode.trim()){
             Appyscript.alert(AppyTemplate.global.pageLanguageSetting.INVALID_QR_CODE || "Invalid QR Code. Please try again.");
             return
        }
       ...
       var postdata = '{"method":"redeemCoupon","redeemFrom":"directory","userId":"' + userIdValue + '","appId":"' + app_id + '","uniquecodedata":"' + localStorage.getItem("uniquecodedata") + '","deviceId":"' + coupondeviceid + '"}';
       var baseurl = webserviceUrl + 'Coupon.php';
       console.log("kkkkk2222fffff   " + postdata + "   baseurllllll  " + baseurl);
       $$.ajax({
           url: baseurl,
