...
Appyscript.onPageInit('fitness', function (page) {
                      var workoutStart = pageData.list[0].languageSetting.fitness_start_workout || "Start Workout"
                      var workoutStop = pageData.list[0].languageSetting.fitness_stop_workout || "Stop Workout"
                      var html = '<div class="timerWatch"><div class="innerImt"><button id="startTimer"  onclick="fitness_timer()" class="startTimer">'+workoutStart+'</button><button  id="stopTimer" style="display:none" onclick="fitness_timer_stop()" class="stopTimer {{@global.styleAndNavigation.subheading[0]}} {{@global.styleAndNavigation.subheading[1]}}">'+workoutStop+'</button><p id="demo" class="timerDiv"></p></div></div>';
...
var fitnessAppUrl = webserviceUrl+'Fitness.php';
...
function Exercise_On_Categories_Bases(id) {
    ...
    if(localData.Exercises.length == 1){
        ...
        localData.list = pageData.list;
        if(localData.video_type != undefined && localData.video_type != ""){
        if(localData.video_type == "vimeo_private"){
                    getVimeoPrivateVideoUrl(localData.exercise_video,localData.access_token).then(function (res) {
                    localData.exercise_video = res;
                    loadPageHTML(localData);
                    }, function (e){ console.log("getVimeoPrivateVideoUrl ::  " + e)
                     Appyscript.alert("Invalid Access token");
                     });
                    }else if(localData.video_type == "vimeo_public"){
                    //will take care later
                    loadPageHTML(localData)
                    }else if(localData.video_type == "youtube"){
                    loadPageHTML(localData);
                    }
                    }else{
                    loadPageHTML(localData);
                    ...
                    data.exercise_video = data.exercise_video || "";
...
function getVimeoPrivateVideoUrl(vidId, authBearerToken){
var auth = "Bearer " + authBearerToken;
return new Promise(function (resolve, reject) {

       $$.ajax({
                           url: "https://api.vimeo.com/me/videos/" + vidId,
                           type: "GET",
                           ...
                           success: function(data) {
                       		...
                       		resolve($$(vimeoData.embed.html).attr("src"));
...
//    if(Object.keys(exeDetail).length == 0)
//        exeDetail = '{"temp":"hotfix"}';

        if(JSON.stringify(exeDetail)=="{}"){
             exeDetail = '{"temp":"hotfix"}';
        }
...
//        var a = ExrDetails.list[0].list;
//        var newArr=a.filter(function(v){
//            return v.exercise_id==id
//        });

//        localData=newArr[0];
        localData.list = pageData.list;
        var _getVideoLink = localData["exercise_video"] || ""
         localData["exercise_video"] = _getVideoLink
        loadFitnessDetailPage(localData);
    ...
    function loadFitnessDetailPage(ld){
if(localData.video_type != undefined && localData.video_type != ""){
    if(ld.video_type == "vimeo_private"){
            getVimeoPrivateVideoUrl(ld.exercise_video,ld.access_token).then(function (res) {
            ld.exercise_video = res;
            loadPageHTML(ld);
            }, function (e){
             console.log("getVimeoPrivateVideoUrl ::  " + e);
                             Appyscript.alert("Invalid Access token");
            });
            }else if(ld.video_type == "vimeo_public"){
            //will take care later
            loadPageHTML(ld)
            }else if(ld.video_type == "youtube"){
            loadPageHTML(ld);
            }
            }else{
            loadPageHTML(ld);
...
function fitnessDietDetails(dietIndex) {
    var _activeIndex= dietPlanSwiper.activeIndex;
    $$.get("pages/fitness-diet-details.html", function(template) {
        ...
        //   var data = JSON.parse(fitness.replaceAll(strWith));
           var data = workoutData.list[_activeIndex].list[dietIndex];
           console.log(data)
...
Appyscript.onPageAfterBack('fitness-diet-plans', function (page) {
    $$('.timerWatch').show();
})
...
Appyscript.onPageInit('fitness-diet-plans', function(page) {
                      ...
                      itemsSize = 1;
                       ...
                       dietPlanSwiper = Appyscript.swiper('.tabs-view');
                      /*
                       var videoCategories = Appyscript.swiper('.tabs-list', {
                       slidesPerView: itemsSize,
                       centeredSlides: true,
                      ...
                      dietPlanSwiper.on("SlideChangeEnd", function(){
                                     $$(".tabs-list li").removeClass("swiper-slide-active").eq(dietPlanSwiper.activeIndex).addClass("swiper-slide-active");
...
if(pageData.list[0].height_unit=="inch"){
height = parseFloat(height * 2.54);
}
if(pageData.list[0].weight_unit=="lbs"){
weight = parseFloat(weight * 0.453592);
}

    if (formData.gender === "male") {
        meta = ((10 * parseFloat(weight)) + (6.25 * parseFloat(height)) -(5 * parseFloat(formData.age))+5 || 0); //3071.487
    } else if (formData.gender === "female" || pageData.list[0].height_unit=="cm") {
        meta = ( (10 * parseFloat(weight)) + (6.25 * parseFloat(height)) -   (5 * parseFloat(formData.age))-161 || 0);
    } else meta = null;
...
function BMR_Calculator() {
    ...
    if (fitness.validForm(formData)) {
        ...
        fitness.bmr = bmr;
        var bmr_is = pageData.list[0].languageSetting.your_bmr_is;
        $$("#fitness-bmr").text(bmr_is + bmr);
        return true;
    } else {
        //show alert;
        return false;
...
fitness.updateNutrition = function(selectObj) {
    ...
    if(!BMR_Calculator()){
        Appyscript.alert(pageData.list[0].languageSetting.fitness_fill_fields);
        return;
    }
    ...
    var formData = Appyscript.formToJSON('#Fitness_BMR');
        ...
        fitness.userData = formData;
        fitness.userData.weight=weight;
        fitness.userData.height=height;
    ...
    if ((fitness.userData.gender === "male" || fitness.userData.gender === "female") && fitness.userData.weight > 0 && fitness.userData.height > 0 && fitness.userData.age > 0 && fitness.userData.activityLevel > 0) {
        ...
        fitness.goals.loss.carbs = (fitness.goals.loss.calories * 0.3) / 4 || 0;
...
AppyTemplate.registerHelper('fitness_json',function(context) {
                            ...
                            if(age.length == 0){ Appyscript.alert(pageData.list[0].languageSetting.fitness_provide_age);}
                            else{
                            Appyscript.alert(pageData.list[0].languageSetting.fitness_provide_weight);
                            }
                            }
...
//                            document.getElementById("demo").innerHTML = t;
                            ...
                            document.getElementById("demo").innerHTML = (hours ? (hours > 9 ? hours : "0" + hours) : "00") + ":" + (minutes ? (minutes > 9 ? minutes : "0" + minutes) : "00") + ":" + (seconds > 9 ? seconds : "0" + seconds);
                            }
                            ...
                            var postData = '{"method":"addTimer","appId":"'+appId+'","pageId":"'+pageIdentifie+'","userId":"'+userId+'","deviceId":"'+deviceId+'","time":"'+t+'","type":"start","timeZone":"'+timezone_offset_minutes+'"}';
                            Appyscript.showIndicator();
                           /*$$.post(fitnessAppUrl, postData, function (getData) {
                                    console.log(getData);
                                    getData = JSON.parse(getData);
...
/*
                            ...
                            var postData = '{"method":"addTimer","appId":"'+appId+'","pageId":"'+pageIdentifie+'","userId":"'+userId+'","deviceId":"'+deviceId+'","time":"'+t+'","type":"stop","timeZone":"'+timezone_offset_minutes+'"}';
                            Appyscript.showIndicator();
                            /*$$.post(fitnessAppUrl, postData, function (getData) {
                                    console.log(getData);
                                    getData = JSON.parse(getData);
