...
var property = "",
    indexval = 0;
var realstateurl = webserviceUrl + 'Realestate.php';
var starValuereal = 0;
var dataaddprprty = {};
var amenttypepage = '';
var type = [],
...
Appyscript.openRealstatePage = function(index) {
    if (localStorage.getItem("profileImage") == "") {
        AppyTemplate.global.realimage = "https://gurucul.com/wp-content/uploads/2015/01/default-user-icon-profile.png"
    } else {
        ...
        $$.ajax({
           ...
           //321  headers: {'accessToken': deviceEncryptedToken},
            ...
            success: function(jsonData, textStatus) {
                ...
                console.log("jsonData after  " + datareal)
                Appyscript.hideIndicator();
                globalPage = true;
                $$.get("pages/customrealestate.html", function(template) {
                    var compiledTemplate = AppyTemplate.compile(template);
                    AppyTemplate.global.pageTitle = pageData.pageTitle;
...
function listaccordingsearch(values) {
    ...
    geocoder.geocode({
        ...
        if (status == google.maps.GeocoderStatus.OK) {
            ...
            Appyscript.searchpropert(values, results[0].geometry.location.lat(), results[0].geometry.location.lng());
        }
    });
}
...
function onSuccessreallanding(position) {
    ...
    if (isOnline()) {
        ...
        $$.ajax({
            url: 'https://maps.googleapis.com/maps/api/geocode/json?latlng=' + postions[0] + ',' + postions[1] + '&sensor=true' + '&key=' + data.googlePlacesApiKey,
            type: 'get',
            async: true,
            success: function(data) {
...
AppyTemplate.registerPartial('realstateListing', '<li rentsoldout="{{rentsoldout}}" propertyfor="{{propertyFor}}" data-background="{{#if image.length}}{{image[0].image}}{{else}}{{@global.defaultimage}} {{/if}}"  class="lazy lazy-fadeIn"><img src="images/transparent-bg.png" alt="" data-background="{{#if image.length}}{{image[0].image}}{{else}}{{@global.defaultimage}} {{/if}}"  class="lazy lazy-fadeIn"/><span class="distance-km {{@global.styleAndNavigation.title[0]}} {{@global.styleAndNavigation.title[1]}}">{{distanceCalreal latitude longitude}}</span><div class="top-list-icon">{{#root_Compare favourite "==" "0"}}<span class="like-btn icon-heart-1" onclick="addfav(this)" property_iid="{{id}}" type="listing"></span>{{/root_Compare}}{{#root_Compare favourite "==" "1"}}<span class="like-btn icon-heart-1 on" onclick="addfav(this)" property_iid="{{id}}" type="listing"></span>{{/root_Compare}}{{#root_Compare @global.sharingoption "==" "On"}}<span class="share-btn iconz-share-alt" descriptionID="{{id}}" onclick="RealEstateShare(this)" location="{{address}}" projectName="{{projectName}}" {{#root_Compare propertyFor "==" "0"}}property_for="For Sale"{{/root_Compare}}{{#root_Compare propertyFor "==" "1"}}property_for="For rent"{{/root_Compare}} price="({{format_currency @global.currencycode}}) {{expectedPrice}}" {{#if image.length}} property_image="{{image[0].image}}" {{else}} property_image="{{@global.defaultimage}}" {{/if}}></span>{{/root_Compare}}</div><div class="estate-listing-overlay  {{#if phonenumber}}listWithMap {{/if}}"> <span class="hyper-catNamebox"><span class="price {{@global.styleAndNavigation.title[0]}} {{@global.styleAndNavigation.title[1]}}"><span style="display:inline-block;">{{format_currency @global.currencycode}}</span> {{expectedPrice}}   <span class="newProp"> &nbsp;({{addtimelist addedon}})</span></span><div class="shorDetail {{@global.styleAndNavigation.title[0]}} {{@global.styleAndNavigation.title[1]}}">{{#root_Compare bedrooms "!=" ""}}<span><i class="iconz-bed"></i> {{getStringFromInt bedrooms}} {{@global.pageLanguageSetting.Bedrooms}}</span> {{/root_Compare}}{{#root_Compare bathrooms "!=" ""}}<span><i class="iconz-bath-bathtub"></i>{{getStringFromInt bathrooms}} {{@global.pageLanguageSetting.Bathrooms}}</span>{{/root_Compare}}<span>{{property_area}} {{area_unit}}</span></div><div class="addPan {{@global.styleAndNavigation.title[0]}} {{@global.styleAndNavigation.title[1]}}"><span class="address">{{address}}</span> <span><i class="{{propertyImage}}"></i> {{propertyType}}</span></div></span></div><span class="real-estate-addons">{{#if rentsoldout}} {{#root_Compare propertyFor "==" "1"}}<p class="forSale {{@global.styleAndNavigation.title[0]}} {{@global.styleAndNavigation.title[1]}}">{{@global.pageLanguageSetting.RENT_OUT}}</p>{{/root_Compare}}{{#root_Compare propertyFor "==" "0"}}<p class="forSale {{@global.styleAndNavigation.title[0]}} {{@global.styleAndNavigation.title[1]}}">{{@global.pageLanguageSetting.SOLD_OUT}}</p>{{/root_Compare}}{{else}}{{#root_Compare propertyFor "==" "1"}}<p class="forSale {{@global.styleAndNavigation.title[0]}} {{@global.styleAndNavigation.title[1]}}">{{@global.pageLanguageSetting.FOR_RENT}}</p>{{/root_Compare}}{{#root_Compare propertyFor "==" "0"}}<p class="forSale {{@global.styleAndNavigation.title[0]}} {{@global.styleAndNavigation.title[1]}}">{{@global.pageLanguageSetting.FOR_SALE}}</p>{{/root_Compare}}{{/if}}<a class="icon-location" onclick="movetomap(\'{{latitude}}\',\'{{longitude}}\',\'{{@index}}\',\'{{filterAddressString address}}\',\'{{projectName}}\',this);"></a>{{#if phonenumber}}<a class="iconz-phone1" onclick="callreal(this)" number="{{phonenumber}}"></a>{{/if}}</span><div class="click-area" onclick="Appyscript.realestateDetails(this)" index="{{@index}}" type="details" property_id="{{id}}" {{#if image.length}} property_image="{{image[0].image}}" {{else}} property_image="{{@global.defaultimage}}" {{/if}} favoteid="{{favourite}}"></div><div id="description{{id}}" style="display:none">{{description}}</div></li>');
...
//        return pageData.languageSetting.new;
...
//            return pageData.languageSetting.new;
//        } else {
//            return +Math.round(dayscal / 7) + pageData.languageSetting.weeks_ago
//        }
//    } else if (elapsed < msPerYear) {
//        return +Math.round(elapsed / msPerMonth) +  pageData.languageSetting.months_ago;
//    } else {
//        return pageData.languageSetting.months_ago;
//    }
//
//});


AppyTemplate.registerHelper('addtimelist', function(timeObject) {

    var date;

    if((typeof timeObject) == "object") {
                            date = new Date( parseInt(timeObject.value));

    }else {
                            date = new Date( parseInt(timeObject));
                            ...
                            if (interval > 1) {
                            return interval + AppyTemplate.global.pageLanguageSetting.years_ago;

                            }
                            interval = Math.floor(seconds / 2592000);
                            if (interval > 1) {
                            return interval + AppyTemplate.global.pageLanguageSetting.months_ago;
                            ...
                            return AppyTemplate.global.pageLanguageSetting.yesterday

                            }

                            return interval + AppyTemplate.global.pageLanguageSetting.days;


                            }
                            interval = Math.floor(seconds / 3600);
                            if (interval > 1) {
                            return interval + AppyTemplate.global.pageLanguageSetting.hours_ago;

                            }
                            interval = Math.floor(seconds / 60);
                            if (interval > 1) {
                            return interval + AppyTemplate.global.pageLanguageSetting.minute_ago;

                            }

                            return AppyTemplate.global.pageLanguageSetting.less_then_minute;

});


function callreal(a) {
     var number = String(a.getAttribute('number'));
...
//    } else {
        ...
        Appyscript.call(String(number).trim());
   // }
...
AppyTemplate.registerHelper('youtubeurl', function(url) {
    ...
    if (url.indexOf("watch?v=") != -1 || url.indexOf("youtu.be") != -1) {
        ...
        var youtubeImage = 'https://img.youtube.com/vi/' + youtubeID + '/0.jpg'
    }
...
function showmore(a) {
    ...
    var linkText = $$this.attr("ctype");

    if (linkText === "read_more") {
        linkText = pageData.languageSetting.read_less || "Read Less";
        $$content.removeClass("hideContent");
        $$content.addClass("showContent");
        $$this.attr("ctype","read_less");
    } else {
        linkText = pageData.languageSetting.READ_MORE;
        $$content.removeClass("showContent");
        $$content.addClass("hideContent");
        $$this.attr("ctype","read_more");
    };
...
function setcurrentmarker(position) {
    ...
    var query = 'https://maps.googleapis.com/maps/api/geocode/json?latlng=' + positions[0] + ',' + positions[1] + '&sensor=true' + '&key=' + data.googlePlacesApiKey;
    Appyscript.showIndicator();
    if (isOnline()) {
        $$.ajax({
            type: 'GET',
...
function geocodePosition(pos) {
    ...
    var query = 'https://maps.googleapis.com/maps/api/geocode/json?latlng=' + pos.lat() + ',' + pos.lng() + '&sensor=true' + '&key=' + data.googlePlacesApiKey;
    Appyscript.showIndicator();
    if (isOnline()) {
        $$.ajax({
            type: 'GET',
...
AppyTemplate.registerPartial('favrealstateListing', '<li rentsoldout="{{rentsoldout}}" propertyfor="{{propertyFor}}" data-background="{{#if image.length}}{{image[0].image}}{{else}}{{@global.defaultimage}} {{/if}}"  class="lazy lazy-fadeIn"><img src="images/transparent-bg.png" alt="" data-background="{{#if image.length}}{{image[0].image}}{{else}}{{@global.defaultimage}} {{/if}}"  class="lazy lazy-fadeIn"/><span class="distance-km {{@global.styleAndNavigation.title[0]}} {{@global.styleAndNavigation.title[1]}}">{{distanceCalreal latitude longitude}}</span><div class="top-list-icon"><span class="like-btn icon-heart-1 on" onclick="addfav(this)" property_iid="{{id}}" type="favrte"></span>{{#root_Compare @global.sharingoption "==" "On"}}<span class="share-btn iconz-share-alt" descriptionID="{{id}}" onclick="RealEstateShare(this)" location="{{address}}" projectName="{{projectName}}" {{#root_Compare propertyFor "==" "0"}}property_for="For Sale"{{/root_Compare}}{{#root_Compare propertyFor "==" "1"}}property_for="For rent"{{/root_Compare}} price="({{format_currency @global.currencycode}}) {{expectedPrice}}" {{#if image.length}} property_image="{{image[0].image}}" {{else}} property_image="{{@global.defaultimage}}" {{/if}}></span>{{/root_Compare}}</div><div class="estate-listing-overlay {{#if phonenumber}}listWithMap {{/if}}"> <span class="hyper-catNamebox"><span class="price {{@global.styleAndNavigation.title[0]}} {{@global.styleAndNavigation.title[1]}}"><span style="display:inline-block;">{{format_currency @global.currencycode}} </span>{{expectedPrice}}   <span class="newProp"> &nbsp;({{addtimelist addedon}})</span></span><div class="shorDetail {{@global.styleAndNavigation.title[0]}} {{@global.styleAndNavigation.title[1]}}">{{#root_Compare bedrooms "!=" ""}}<span><i class="iconz-bed"></i> {{bedrooms}}{{@global.pageLanguageSetting.Bedrooms}}</span> {{/root_Compare}}{{#root_Compare bathrooms "!=" ""}}<span><i class="iconz-bath-bathtub"></i> {{bathrooms}} ba {{@global.pageLanguageSetting.Bathrooms}}</span>{{/root_Compare}}<span>{{property_area}} {{area_unit}}</span></div><div class="addPan {{@global.styleAndNavigation.title[0]}} {{@global.styleAndNavigation.title[1]}}"><span class="address">{{address}}</span> <span><i class="{{propertyImage}}"></i> {{propertyType}}</span></div></span></div><span class="real-estate-addons">{{#if rentsoldout}} {{#root_Compare propertyFor "==" "1"}}<p class="forSale {{@global.styleAndNavigation.title[0]}} {{@global.styleAndNavigation.title[1]}}">{{@global.pageLanguageSetting.RENT_OUT}}</p>{{/root_Compare}}{{#root_Compare propertyFor "==" "0"}}<p class="forSale {{@global.styleAndNavigation.title[0]}} {{@global.styleAndNavigation.title[1]}}">{{@global.pageLanguageSetting.SOLD_OUT}}</p>{{/root_Compare}}{{else}}{{#root_Compare propertyFor "==" "1"}}<p class="forSale {{@global.styleAndNavigation.title[0]}} {{@global.styleAndNavigation.title[1]}}">{{@global.pageLanguageSetting.FOR_RENT}}</p>{{/root_Compare}}{{#root_Compare propertyFor "==" "0"}}<p class="forSale {{@global.styleAndNavigation.title[0]}} {{@global.styleAndNavigation.title[1]}}">{{@global.pageLanguageSetting.FOR_SALE}}</p>{{/root_Compare}}{{/if}}<a class="icon-location" onclick="movetomap(\'{{latitude}}\',\'{{longitude}}\',\'{{@index}}\',\'{{filterAddressString address}}\',\'{{projectName}}\',this);"></a>{{#if phonenumber}}<a class="iconz-phone1" onclick="callreal(this)" number="{{phonenumber}}"></a>{{/if}}</span><div class="click-area" onclick="Appyscript.realestateDetails(this)" index="{{@index}}" type="details" property_id="{{id}}" {{#if image.length}} property_image="{{image[0].image}}" {{else}} property_image="{{@global.defaultimage}}" {{/if}} favoteid="{{favourite}}"></div><div id="description{{id}}" style="display:none">{{description}}</div></li>');
...
function RealEstateShare(a) {
    ...
    var name = a.getAttribute("projectName");
     var description = a.getAttribute("descriptionID");
        var descriptionText = $$("#description"+description).text();
    var shareData = "";
    if (Appyscript.device.android) {
        if (image) {
        ...
        if (descriptionText) {
                    shareData += pageData.languageSetting.discription + " : " + descriptionText + "\n";
                }
    } else {
        if (image) {
...
Appyscript.onPageInit('realestate-addMedia', function(page) {
    ...
    $$("#sellproperty").is(".on") ? $("#sellCheck").html('<i class="iconz-bed"></i>'+AppyTemplate.global.pageLanguageSetting.SELL_PRICE +' ('+ AppyTemplate.global.currencycode +')') :$("#sellCheck").html('<i class="iconz-bed"></i>'+ AppyTemplate.global.pageLanguageSetting.EXPECTED_RENT+' ('+ AppyTemplate.global.currencycode +')') ;
});


Appyscript.realestateAddListing = function(inapptrnsactionid, statusval) {
    updateloc = "false";
    property = "";
    ...
    postJobData = null;
    if (pageData.configration.paymentMethod == "inApp" ) {
    console.log("Realestate payment method chk .....")
            pageData.oneTimeSubscriptionPrice = pageData.configration.oneTimeSubscription
            pageData.oneTimeSubscriptionCurrency = pageData.configration.currency_code
            pageData.inAppAndroidBundleId = pageData.configration.androidProductId

            pageData.monthlySubscriptionPrice = pageData.configration.realEstateMonthlySubscription
            pageData.monthlySubscriptionCurrency = pageData.configration.currency_code
            pageData.androidMonthlyBundleId = pageData.configration.realEstateAndroidMonthlyProductId

            pageData.yearlySubscriptionPrice = pageData.configration.realEstateYearlySubscription
            pageData.yearlySubscriptionCurrency = pageData.configration.currency_code
            pageData.androidYearlyBundleId = pageData.configration.realEstateAndroidYearlyProductId
            pageData.paymentMethod = pageData.configration.paymentMethod
            pageData.inAppPublicKey = pageData.configration.androidPublicKey
    ...
    checkRealestateIAPPayment();
    return;

    }
    else{

    Appyscript.realestateAddListingPage();
    }
    }

    Appyscript.realestateAddListingPage = function(inapptrnsactionid, statusval) {
...
//        var global.checkLists = $$("#sellproperty").is(".on");
        AppyTemplate.global.checkLists=$$("#sellproperty").is(".on")
...
//    }
       if ($$("#prprtydescrptn").val() == "") {
        ...
        params.totalFloor = dataaddprprty.totalfloors;
        params.propertyFloor = dataaddprprty.floor;
        ...
        ft.upload(realstatepath, webserviceUrl + 'RealestatePost.php', winreal, fail, options);
...
//    navigator.device.capture.captureImage(captureSuccessrealestate, captureErrorrealestate, options);
    ...
    navigator.camera.getPicture(captureSuccessrealestate, captureErrorrealestate, {
        quality: 70,
        ...
        $$.ajax({
            ...
            success: function(jsonData, textStatus) {
                ...
                AppyTemplate.global.propertydetail = datareal;
                 ...
                 if(datareal.data.image.length>0)
                                {
                                for(i=0;i<datareal.data.image.length;i++){
                                console.log(datareal.data.image[i].type);
                                if(datareal.data.image[i].type=="image")
...
AppyTemplate.registerHelper('getStringFromInt',function (value) {
...
function checkRealestateIAPPayment()
{
...
function uploadRealestateTransectionDetailAfterPayment(transactionId,pageTypeid, productId,paymentTypeIap,subscriptionPrice,subscriptionCurrency)
{
...
function callbackAfterPaymentOnRealestate(status, transactionId, productId, payType,pageTypeid,paymentTypeIap,subscriptionPrice,subscriptionCurrency )
{
    console.log("**********************callbackAfterPaymentOnRealestate***********************************************************")
    if(status=="success")
    {
      ...
      Appyscript.showIndicator();
         ...
         else
         {
          ...
          uploadRealestateTransectionDetailAfterPayment(transactionId,pageTypeid, productId,paymentTypeIap,subscriptionPrice,subscriptionCurrency);
         }
    }
